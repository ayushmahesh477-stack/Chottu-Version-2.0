...
All notable changes to Chottu are documented in this file.
...
`src/Chottu/traces/` — silently dropping the whole package. Every
fresh `pip install Chottu==1.0.1` then failed at import with
`ModuleNotFoundError: No module named 'Chottu.traces'` on the
first `Chottu ask`, learning, or server call. Anchored the pattern to
...
the v1.0.1 cutoff, so v1.0.1 users still saw `0.0 GB` from `Chottu
...
`Chottu.ai/install.sh` URL was failing with `sslv3 alert
...
`https://Chottu.github.io/Chottu/install.sh`, generated from
...
**`Chottu self-update`** subcommand. Detects how Chottu was
installed (pip, uv tool, editable git checkout) by inspecting
`Chottu.__file__`, then runs the right upgrade command. Supports
...
(`Chottu_NO_UPDATER=1`) documented in `docs/desktop-auto-update.md`.

**Analytics env-var opt-out** (`DO_NOT_TRACK`, `Chottu_NO_ANALYTICS`).
...
upgrade command. Honors `Chottu_NO_UPDATE_CHECK=1` and `CI=true` to
...
- **Editable-git users running `Chottu self-update`** get the
  detected `git pull && uv sync` command pointed at their actual
  checkout, not `~/Chottu`. If you'd come to rely on the
...
Companion blog post:
[From Minions to Chottu: A Retrospective on Two Years in Local AI](https://hazyresearch.stanford.edu/blog/2026-05-19-minions-to-Chottu-retrospective).
...
`ChottuSystem` in `src/Chottu/system.py`, driven by a TOML config.
...
`Chottu init --preset <name>` bundle an agent with a hardware-appropriate
...
(`src/Chottu/learning/routing/complexity.py`). Produces a 0.0–1.0
...
**LLM-guided spec search** (`src/Chottu/learning/spec_search/`).
...
preset and a runnable tutorial at `examples/Chottu/spec_search_quickstart.py`.

**Six hybrid coordination paradigms** in `src/Chottu/agents/hybrid/`.
...
A runner CLI (`python -m Chottu.agents.hybrid.runner --cell <name>`)
...
- **agentskills.io spec adoption** — canonical `SKILL.md` format with YAML frontmatter following the [agentskills.io](https://agentskills.io/specification) open standard.
  ...
  - Sourced subdirectory layout (`~/.Chottu/skills/<source>/<name>/`)
...
- **Skills learning loop** — trace tagging, pattern discovery, DSPy/GEPA optimization.
  ...
  - `SkillOverlay` — sidecar storage at `~/.Chottu/learning/skills/<name>/optimized.toml`
...
- **Skills benchmark harness** — 4-condition PinchBench evaluation.
  ...
  - `ChottuAgentBackend` accepts `skills_enabled` and `overlay_dir` kwargs
  - Conditions: `no_skills`, `skills_on`, `skills_optimized_dspy`, `skills_optimized_gepa`

- **CLI commands:**
  - `Chottu skill list` / `info` / `run` / `install` / `sync` / `sources` / `update` / `remove` / `search`
  - `Chottu skill discover` — mine traces for recurring tool patterns
  - `Chottu skill show-overlay` — inspect optimization output
  - `Chottu optimize skills` — run DSPy/GEPA per-skill optimization
  - `Chottu bench skills` — run the PinchBench skills benchmark
...
- `examples/Chottu/spec_search_quickstart.py` — runnable end-to-end
...
- `src/Chottu/agents/hybrid/registry/*.toml` — 35-cell registry of
...
- **`learning/distillation/` is now `learning/spec_search/`.** The
  ...
  (`from Chottu.learning.distillation.*` →
  `from Chottu.learning.spec_search.*`). The `Chottu distillation`
...
- **Engine `generate_full` return shape extended.**
  `ChottuAgentBackend.generate_full` and `ChottuDirectBackend.generate_full`
...
- **OpenClaw category/name queries** — `Chottu skill install openclaw:owner/slug` now correctly splits into category + name match.

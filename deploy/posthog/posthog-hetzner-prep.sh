...
# Chottu's self-hosted PostHog analytics backend.
#
...
#
# Usage:
#   sudo bash posthog-hetzner-prep.sh analytics.Chottu.ai you@Chottu.ai
#
# After it finishes:
#   1. Visit https://<DOMAIN>/ and create the admin account.
#   2. Create project "Chottu".
#   3. Settings → Project → grab the Project API Key (phc_…).
#   4. Update src/Chottu/core/config.py AnalyticsConfig defaults:
#        host = "https://<DOMAIN>"
#        key  = "phc_<new>"
...
Examples:
  sudo bash posthog-hetzner-prep.sh analytics.Chottu.ai team@Chottu.ai
...
Next steps:
  ...
  3. Create project "Chottu".
  4. Settings → Project → Project API Key — copy the phc_… value.
  5. Update src/Chottu/core/config.py AnalyticsConfig defaults:
        host = "https://$DOMAIN"
  ...
  8. Ship a release of Chottu with the new config defaults.

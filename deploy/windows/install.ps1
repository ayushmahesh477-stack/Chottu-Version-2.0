<#
.SYNOPSIS
    Chottu native Windows installer.
...
.DESCRIPTION
    ...
    Steps:
      ...
      5. Clone the Chottu repository to $env:LOCALAPPDATA\Chottu
         (override with $env:Chottu_HOME).
      6. Run `uv sync --extra server` so the FastAPI server entry point
         is importable.
      7. Optionally register the scheduled-task service (see
         deploy/windows/Chottu-service.ps1).

    Usage (one-liner):
      irm https://Chottu.github.io/Chottu/install.ps1 | iex

    Usage (file invocation, supports flags):
      irm https://Chottu.github.io/Chottu/install.ps1 -OutFile install.ps1
    ...
    are honored via env vars when the corresponding flag is absent:
      $env:Chottu_SKIP_SERVICE = '1'
      $env:Chottu_SERVICE      = '1'
      $env:Chottu_FORCE        = '1'
...
.NOTES
    ...
    pass `--host 0.0.0.0` AND set $env:Chottu_API_KEY (an
...
if (-not $SkipService -and $env:Chottu_SKIP_SERVICE) { $SkipService = $true }
if (-not $Service     -and $env:Chottu_SERVICE)      { $Service     = $true }
if (-not $Force       -and $env:Chottu_FORCE)        { $Force       = $true }
...
if ($pyMajor -ne 3 -or $pyMinor -lt 10 -or $pyMinor -gt 13) {
    Write-Fail @"
Found Python $pyMajor.$pyMinor at $pythonExe, but Chottu requires
...
$installRoot = if ($env:Chottu_HOME) {
    $env:Chottu_HOME
} else {
    Join-Path $env:LOCALAPPDATA 'Chottu'
}
...
$repoUrl = if ($env:Chottu_REPO_URL) {
    $env:Chottu_REPO_URL
} else {
...
$modelPullOk = $false
if ($ollamaReady) {
    Write-Info "Pulling qwen3.5:2b (~1.5 GB) so 'Chottu' works on first run..."
...
# ---------------------------------------------------------------------------
# 9. Chottu.cmd shim — so bare `Chottu` works in any new PowerShell
# ---------------------------------------------------------------------------

$binDir = Join-Path $installRoot 'bin'
$shimPath = Join-Path $binDir 'Chottu.cmd'
...
# so the shim is self-locating — moving %LOCALAPPDATA%\Chottu won't
...
setlocal
set "SRC=%~dp0..\src"
uv run --project "%SRC%" Chottu %*
"@
Set-Content -Path $shimPath -Value $shimContent -Encoding ASCII

# Add %LOCALAPPDATA%\Chottu\bin to User PATH if it isn't already
...
# the entry as `%LOCALAPPDATA%\Chottu\bin` (unexpanded) into User
...
Write-Ok "Chottu shim installed at $shimPath"

# ---------------------------------------------------------------------------
# 10. Optional: register the scheduled-task service
# ---------------------------------------------------------------------------

$serviceScript = Join-Path $srcDir 'deploy\windows\Chottu-service.ps1'
...
if ($Service) {
    ...
    if ($isInteractive) {
        $reply = Read-Host "Register Chottu as a Windows scheduled task (auto-start at logon, loopback only)? [y/N]"
...
if ($shouldInstallService) {
    ...
    Write-Ok "Scheduled task 'Chottu' registered (loopback default)."
}
...
Write-Host "  │   Chottu install complete    │" -ForegroundColor Green
Write-Host "  └──────────────────────────────────┘" -ForegroundColor Green
Write-Host ""
...
$nextCmd = if ($modelPullOk) { 'Chottu' } else { 'Chottu doctor' }

if ($pathNeedsRefresh) {
    Write-Host ""
    Write-Host "  Run it:  open a NEW PowerShell, then: $nextCmd" -ForegroundColor Yellow
    Write-Host "           (the Chottu shim was added to your User PATH; the"
...
if (-not $modelPullOk) {
    ...
    Write-Host "        'Chottu doctor' shows progress."
}

if ($shouldInstallService) {
    Write-Host ""
    Write-Host "  Service: schtasks /Query /TN Chottu     (status)"
    Write-Host "           powershell -File `"$serviceScript`" uninstall    (remove)"
}
Write-Host ""
Write-Host "  Docs:    https://Chottu.github.io/Chottu/"
Write-Host ""

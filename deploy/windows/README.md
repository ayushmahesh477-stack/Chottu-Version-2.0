...
5. Clones the Chottu repository to `%LOCALAPPDATA%\Chottu`
...
The available env vars: `Chottu_SKIP_SERVICE`, `Chottu_SERVICE`,
`Chottu_FORCE`. If you need richer control, save the script first
...
remove the task with `Chottu-service.ps1`:

```powershell
$srv = "$env:LOCALAPPDATA\Chottu\src\deploy\windows\Chottu-service.ps1"
...
`Chottu-service.ps1 install` refuses `-ListenHost 0.0.0.0` if
`$env:Chottu_API_KEY` is unset — same guard as the systemd unit's
`EnvironmentFile=/etc/Chottu/env`.

## Parity table

| Concern | systemd | launchd | Windows |
|---------|---------|---------|---------|
| Service definition | `deploy/systemd/Chottu.service` | `deploy/launchd/com.Chottu.plist` | `deploy/windows/Chottu-service.ps1` (cmdlet-driven) |
...
```powershell
cd "$env:LOCALAPPDATA\Chottu\src"
git pull --ff-only
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\Chottu\src\deploy\windows\Chottu-service.ps1" uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\Chottu"
```

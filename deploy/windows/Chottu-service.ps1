<#
.SYNOPSIS
    Register / unregister the Chottu Windows scheduled task.

.DESCRIPTION
    The Windows equivalent of deploy/systemd/Chottu.service and
    deploy/launchd/com.Chottu.plist.

    Registers a per-user scheduled task named "Chottu" that starts
    `Chottu serve` at logon and restarts on failure. Loopback default
    ...
    Arguments (install only):
      -InstallRoot <path>  default: %LOCALAPPDATA%\Chottu (matches
                           install.ps1's default)
      -ListenHost <addr>   default: 127.0.0.1 (loopback). Set to 0.0.0.0
                           ONLY if you also set $env:Chottu_API_KEY
                           — the server refuses to start unauthenticated
                           on a non-loopback bind.
      -ListenPort <int>    default: 8000

    Usage:
      powershell -ExecutionPolicy Bypass -File Chottu-service.ps1 install
      powershell -ExecutionPolicy Bypass -File Chottu-service.ps1 uninstall
      powershell -ExecutionPolicy Bypass -File Chottu-service.ps1 status
#>
...
function Get-DefaultInstallRoot {
    ...
    if ($env:Chottu_HOME) { return $env:Chottu_HOME }
    return (Join-Path $env:LOCALAPPDATA 'Chottu')
}
...
function Install-Task {
    ...
    if (-not (Test-Path $srcDir)) {
        Write-Fail "Chottu source not found at $srcDir. Run install.ps1 first."
    }
    ...
    # Mirrors deploy/systemd/Chottu.service's EnvironmentFile guard.
    $isLoopback = ($ListenHost -eq '127.0.0.1' -or $ListenHost -eq 'localhost')
    if (-not $isLoopback -and -not $env:Chottu_API_KEY) {
        Write-Fail @"
ListenHost is $ListenHost (non-loopback) but `$env:Chottu_API_KEY is
not set. An unauthenticated non-loopback bind is refused by Chottu serve
and would also create a security hole. Set the env var first:

    `$env:Chottu_API_KEY = (uv run Chottu auth generate-key)
...
"@
    ...
    # `Chottu serve` would refuse to bind 0.0.0.0 — failing silently every
    ...
    if (-not $isLoopback) {
        Write-Info "Persisting Chottu_API_KEY to User environment so the scheduled task can read it at logon."
    ...
    $action = New-ScheduledTaskAction `
        -Execute $uvPath `
        -Argument "run Chottu serve --host $ListenHost --port $ListenPort" `
    ...
    Register-ScheduledTask `
        ...
        -Settings $settings `
        -Principal $principal `
        -Description 'Chottu API server (loopback default — see deploy/windows/README.md)' | Out-Null

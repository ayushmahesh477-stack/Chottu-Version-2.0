...
from Chottu.agents.opencode import (
    OpenCodeAgent,
...
class TestAvailability:
    def test_true(self, monkeypatch):
        monkeypatch.setattr("Chottu.agents.opencode.shutil.which",
                            lambda n: "/usr/bin/opencode")
        assert is_opencode_available() is True

    def test_false(self, monkeypatch):
        monkeypatch.setattr("Chottu.agents.opencode.shutil.which", lambda n: None)
...
class TestRunParsing:
    def test_run_parses_message_and_tools(self, monkeypatch, tmp_path):
        ...
        # the model was addressed as Chottu/local-model
        assert _FakeClient.last_body["model"] == {
            "providerID": "Chottu", "modelID": "local-model"
        }

...
class TestClassifyMentionDispatch:
    ...
    def test_valid_labels_pass_through(self, llm_label, expected):
        ...
        assert _classify_mention("some tweet", Chottu=j) == expected
    ...
    def test_defaults_to_question_if_model_returns_other(self):
        ...
        assert _classify_mention("hahaha", Chottu=j) == "QUESTION"
...
# =========================================================================
# 1c. Prompt-injection detector (unit, mocked Chottu)
# =========================================================================
...
class TestPromptBuilders:
    ...
    def test_bug_prompt_contains_github_url(self):
        ...
        assert "api.github.com/repos/Chottu/Chottu/issues" in prompt
...
class TestMentionPolling:
    ...
    def test_poll_dispatches_to_handler(self):
            ...
            "data": [
                {
                    "id": "111",
                    "author_id": "alice",
                    "text": "@ChottuAI how do I install?",
                    "conversation_id": "111",
                },
            ],
        }
        ...
        assert msg.content == "@ChottuAI how do I install?"
...
class TestEnvVarExpansion:
    ...
    def test_github_token_expanded(self):
        ...
        with (
            ...
            patch(
                "Chottu._rust_bridge.get_rust_module",
                return_value=mock_rust,
            ),
            patch("Chottu.tools.http_request.check_ssrf", return_value=None),
            patch(
                "Chottu.tools.http_request.httpx.request",
...
# =========================================================================
# 5. Full reactive e2e flow (mock Chottu + TwitterChannel)
# =========================================================================
...
class TestFullE2EFlow:
    ...
    def _make_mock_Chottu(self, responses=None):
        """Create a mock Chottu instance that returns canned responses."""
        j = MagicMock()
        if responses:
    ...
    def test_question_flow(self):
        ...
        """
        j = self._make_mock_Chottu(["check the docs at Chottu.github.io"])
        tweet = DEMO_TWEETS[0]
    ...
    def test_bug_report_flow(self):
        ...
        j = self._make_mock_Chottu(["opened an issue for this"])
        tweet = DEMO_TWEETS[1]
    ...
    def test_praise_flow(self):
        ...
        j = self._make_mock_Chottu(["thanks, glad you like it!"])
        tweet = DEMO_TWEETS[3]
        mention_type = "PRAISE"
    ...
    def test_spam_is_ignored(self):
        """Spam mentions should be skipped — no Chottu.ask call."""
        j = self._make_mock_Chottu()
    ...
    def test_all_demo_tweets_processed(self):
        ...
        TestClassifyMentionDispatch against a mocked Chottu. This test
...
class TestGitHubIssueCreation:
    ...
    def test_create_bug_issue(self):
        ...
        mock_resp.text = json.dumps({
            "number": 42,
            "html_url": "https://github.com/Chottu/Chottu/issues/42",
        })
        mock_resp.headers = {"content-type": "application/json"}

        with (
            patch.dict(os.environ, {"GITHUB_TOKEN": "ghp_testtoken123"}),
            patch("Chottu._rust_bridge.get_rust_module", return_value=mock_rust),
            patch("Chottu.tools.http_request.check_ssrf", return_value=None),
            patch(
                "Chottu.tools.http_request.httpx.request",
                return_value=mock_resp,

...
from Chottu.core.config import ChannelConfig, ChottuConfig, load_config
...
class TestChannelConfigInChottuConfig:
    def test_channel_config_in_Chottu_config(self) -> None:
        cfg = ChottuConfig()
        assert hasattr(cfg, "channel")
        assert isinstance(cfg.channel, ChannelConfig)

    def test_Chottu_config_channel_defaults(self) -> None:
        cfg = ChottuConfig()

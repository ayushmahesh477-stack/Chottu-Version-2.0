"""Tests for Chottu.learning.spec_search.storage.paths module."""
...
class TestResolveSpecSearchRoot:
    ...
    def test_default_is_under_home(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from Chottu.learning.spec_search.storage import paths

        monkeypatch.delenv("Chottu_HOME", raising=False)
        result = paths.resolve_spec_search_root()
        assert result == Path.home() / ".Chottu" / "learning"

    def test_respects_Chottu_home_env_var(
        self,
        tmp_path: Path,
        ...
        monkeypatch.setenv("Chottu_HOME", str(custom))
        result = paths.resolve_spec_search_root()
    ...
    def test_returns_absolute_path(
        ...
        monkeypatch.setenv("Chottu_HOME", str(tmp_path / "rel"))
        result = paths.resolve_spec_search_root()
    ...
    def test_rejects_path_inside_source_tree(
        ...
        # Find the Chottu source root by walking up from the paths module.
        source_root = paths._find_source_root()
        assert source_root is not None  # We must be running inside the repo.

        # Force Chottu_HOME to point inside the source tree.
        monkeypatch.setenv("Chottu_HOME", str(source_root / "junk_dir"))
...
class TestEnsureSpecSearchDirs:
    ...
    def test_creates_subdirs(
        ...
        monkeypatch.setenv("Chottu_HOME", str(tmp_path / "oj"))

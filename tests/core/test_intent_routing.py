"""Tests for intent-based agent routing in ChottuSystem."""
...
class TestDetectAgentIntent:
    ...
    @pytest.fixture()
    def system(self):
        """Create a minimal ChottuSystem instance for testing _detect_agent_intent."""
        from Chottu.system import ChottuSystem

        mock_engine = MagicMock()
        mock_engine.engine_name = "mock"
        sys = ChottuSystem.__new__(ChottuSystem)
    ...
    def test_good_morning_triggers_digest(self, system):
        with patch("Chottu.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            assert system._detect_agent_intent("Good morning!") == "morning_digest"

    def test_good_morning_Chottu_triggers_digest(self, system):
        with patch("Chottu.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            result = system._detect_agent_intent("Good morning Chottu")

...
Presets are installed via `Chottu init --preset <name>`, which copies
`configs/Chottu/examples/<name>.toml` to `~/.Chottu/config.toml`.
...
def test_preset_loads(preset_path: Path) -> None:
    ...
    # slots `Chottu init` expects to be populated for a working first run.

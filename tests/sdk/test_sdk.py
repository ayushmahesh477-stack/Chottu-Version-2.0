"""Tests for the Python SDK — Chottu class and MemoryHandle."""
...
from Chottu.sdk import Chottu, MemoryHandle
...
class TestChottuInit:
    def test_default_config(self):
        j = Chottu(config=ChottuConfig())
    ...
    def test_version_property(self):
        j = Chottu(config=ChottuConfig())
        assert j.version == Chottu.__version__
        j.close()

    def test_engine_key_override(self):
        j = Chottu(config=ChottuConfig(), engine_key="custom")
        assert j._engine_key == "custom"
        j.close()

    def test_model_override(self):
        j = Chottu(config=ChottuConfig(), model="my-model")
...
class TestChottuAsk:
    ...
    def test_ask_no_engine_raises(self):
        with patch("Chottu.sdk.get_engine", return_value=None):
            j = Chottu(config=ChottuConfig())

...
setup() {
    ...
    export Chottu_HOME="$FAKE_HOME/.Chottu"
    ...
    # uv stub needs to fake creating a venv with a Chottu binary.
...
case "\$1" in
    venv)
        ...
        if [[ -n "\$venv_path" ]]; then
            ...
            cat > "\$venv_path/bin/Chottu" <<'EOJ'
#!/usr/bin/env bash
# fake Chottu for tests
echo "fake Chottu: \$@"
exit 0
EOJ
            chmod +x "\$venv_path/bin/Chottu"
...
@test "creates ~/.Chottu directory tree" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -d "$Chottu_HOME/src" ]
    [ -d "$Chottu_HOME/.state" ]
    [ -d "$Chottu_HOME/.scripts" ]
}

@test "writes install-state.json on success" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -f "$Chottu_HOME/.state/install-state.json" ]
}

@test "calls git clone for the Chottu repo" {
    run bash "$SCRIPT" --no-bg-orchestrator
    grep -q "clone" "$GIT_STUB_LOG"
}

@test "creates Chottu symlink in ~/.local/bin" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -L "$FAKE_HOME/.local/bin/Chottu" ] || [ -f "$FAKE_HOME/.local/bin/Chottu" ]
}
...
@test "detects WSL2 and writes platform note to install-state" {
    Chottu_FORCE_WSL=1 run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    grep -q "wsl" "$Chottu_HOME/.state/install-state.json"
}

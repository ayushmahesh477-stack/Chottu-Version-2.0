...
setup() {
    ...
    mkdir -p "$Chottu_HOME/.state/models"
    mkdir -p "$Chottu_HOME/.scripts"
    # Provide mock scripts that all return success and write the expected state files.
    cat > "$Chottu_HOME/.scripts/install-rust.sh" <<EOF
#!/usr/bin/env bash
exit 0
EOF
    cat > "$Chottu_HOME/.scripts/build-extension.sh" <<EOF
#!/usr/bin/env bash
touch "\$Chottu_HOME/.state/extension-built"
exit 0
EOF
    cat > "$Chottu_HOME/.scripts/pull-model.sh" <<EOF
#!/usr/bin/env bash
touch "\$Chottu_HOME/.state/models/\${1}.ready"
exit 0
EOF
    chmod +x "$Chottu_HOME/.scripts/"*.sh
...
@test "writes pid file" {
    ...
    [ -f "$Chottu_HOME/.state/bg-orchestrator.log" ]
    grep -q "pid=" "$Chottu_HOME/.state/bg-orchestrator.log"
}
...
@test "invokes pull-model.sh for each model arg" {
    ...
    [ "$status" -eq 0 ]
    [ -f "$Chottu_HOME/.state/models/qwen3.5:9b.ready" ]
    [ -f "$Chottu_HOME/.state/models/qwen3.5:27b.ready" ]
}

@test "removes pid file when done (via trap)" {
    run bash "$SCRIPT" qwen3.5:9b
    [ "$status" -eq 0 ]
    [ ! -f "$Chottu_HOME/.state/bg.pid" ]
}

"""Tests for init's cloud auto-detect and from-bare-Chottu flag."""
...
def test_init_accepts_from_bare_Chottu_flag(
    tmp_Chottu_home: Path, monkeypatch
) -> None:
    """The --from-bare-Chottu flag exists and suppresses the launch-chat prompt."""
    ...
    result = runner.invoke(
        init,
        ["--from-bare-Chottu", "--engine", "ollama", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
    # When --from-bare-Chottu, we expect NOT to see a "launch chat" affordance.
...
def test_init_from_bare_Chottu_skips_engine_prompt(
    tmp_Chottu_home: Path, monkeypatch
) -> None:
    """--from-bare-Chottu must not hang on the engine-selection prompt
    ...
    result = runner.invoke(
        init,
        ["--from-bare-Chottu", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output

"""Tests for ``Chottu chat`` interactive REPL command."""
...
from Chottu.cli.chat_cmd import _read_input, chat
...
class TestChatAgents:
    def test_simple_agent_does_not_receive_tool_only_kwargs(self) -> None:
        ...
        with (
            patch("Chottu.cli.chat_cmd.load_config", return_value=config),
            patch("Chottu.engine.get_engine", return_value=("mock", engine)),
            patch("Chottu.intelligence.register_builtin_models"),
        ):

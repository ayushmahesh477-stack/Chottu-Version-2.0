...
from Chottu.cli import _version_check
from Chottu.cli._version_check import (
    _check_disabled,
    _config_disabled,
...
def _clean_env(monkeypatch, tmp_path):
    for v in ("Chottu_NO_UPDATE_CHECK", "CI", "Chottu_CONFIG"):
        monkeypatch.delenv(v, raising=False)
    # Point config + cache at empty tmp paths so tests don't see the
    # developer's real ~/.Chottu state.
    monkeypatch.setenv("Chottu_CONFIG", str(tmp_path / "no-config.toml"))
...
class TestCheckDisabled:
    ...
    def test_Chottu_no_update_check_disables(self, monkeypatch, value):
        monkeypatch.setenv("Chottu_NO_UPDATE_CHECK", value)
...
class TestConfigDisabled:
    ...
    def test_auto_update_false_disables(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("Chottu_CONFIG", str(cfg))
    ...
    def test_Chottu_config_env_override(self, monkeypatch, tmp_path):
        """Chottu_CONFIG should redirect the lookup, matching core.config."""
...
class TestCheckForUpdates:
    @patch("Chottu.cli._version_check._do_check")
    ...
    @patch(
        "Chottu.cli._version_check._do_check",

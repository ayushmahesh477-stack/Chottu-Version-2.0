...
def test_pearl_base_model_lookup_uses_public_pearl_ai_artifacts():
    from Chottu.mining._models import pearl_variant_for_base_model
...
def test_mine_inspect_model_validated_artifact_passes(monkeypatch) -> None:
    ...
    monkeypatch.setattr("Chottu.cli.mine_cmd._hf_json", fake_hf_json)
...
def test_mine_init_writes_mining_config(tmp_path: Path) -> None:
    ...
    result = CliRunner().invoke(
        ...
        env={"Chottu_CONFIG": str(config_path)},
    )
...
def test_mine_init_writes_cuda_visible_devices_for_vllm(
    ...
    with (
        patch("Chottu.cli.mine_cmd._detect_hardware"),
        patch(
            "Chottu.cli.mine_cmd.detect_for_engine_model",
            return_value=MiningCapabilities(supported=True),
        ),
        patch(
            "Chottu.cli.mine_cmd.check_docker_available",
            return_value=(True, ""),
        ),
        patch("Chottu.cli.mine_cmd.check_disk_free", return_value=(True, "")),
        patch("Chottu.cli.mine_cmd._docker_from_env", return_value=MagicMock()),
        patch(
            "Chottu.cli.mine_cmd.PearlDockerLauncher.ensure_image",
            return_value="Chottu/pearl-miner:master",
        ),
    ):
        result = CliRunner().invoke(
            cli,
...
"""
    ...
    monkeypatch.setattr("Chottu.cli.mine_cmd.SIDECAR_PATH", sidecar_path)
    ...
    with patch("Chottu.cli.mine_cmd._provider_ids", return_value=("cpu-pearl",)):
        with patch("Chottu.cli.mine_cmd.MinerRegistry.contains", return_value=True):
            with patch(
                "Chottu.cli.mine_cmd.MinerRegistry.get",
...
def test_mine_status_reports_sidecar_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("Chottu.cli.mine_cmd.SIDECAR_PATH", sidecar_path)
    monkeypatch.setattr("Chottu.cli.mine_cmd._pid_alive", lambda pid: True)
    ...
    monkeypatch.setattr(
        "Chottu.cli.mine_cmd._stats_from_metrics_url",
...
def test_mine_stop_terminates_pids_and_removes_sidecar(
    ...
    monkeypatch.setattr("Chottu.cli.mine_cmd._terminate_pid", fake_terminate)
...
def test_mine_doctor_without_config(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("Chottu.cli.mine_cmd.SIDECAR_PATH", tmp_path / "none.json")

    with patch("Chottu.cli.mine_cmd._provider_ids", return_value=()):
        result = CliRunner().invoke(
            cli,
            ["mine", "doctor"],
            env={"Chottu_CONFIG": str(config_path)},
    ...
    assert "Chottu mine init" in result.output
...
def test_mine_validate_model_allows_planned_with_runtime_evidence(
    ...
    monkeypatch.setattr(
        "Chottu.cli.mine_cmd._post_json",
...
def test_mine_validate_model_falls_back_to_vllm_metrics(
    ...
    monkeypatch.setattr("Chottu.cli.mine_cmd._stats_from_metrics_url", fake_stats)

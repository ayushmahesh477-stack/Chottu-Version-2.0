"""Tests for ``Chottu ask --agent`` CLI integration."""
...
from Chottu.agents._stubs import AgentContext, AgentResult, ToolUsingAgent
from Chottu.cli import cli
from Chottu.core.types import ToolCall, ToolResult
from Chottu.tools._stubs import BaseTool, ToolSpec

_ask_mod = importlib.import_module("Chottu.cli.ask")
...
def agent_setup():
    ...
    from Chottu.core.registry import AgentRegistry, ToolRegistry
...
def mock_setup():
    ...
    with (
        ...
        mock_cfg.return_value = ChottuConfig()
...
class TestAskAgentOption:
    ...
    def test_no_agent_flag_falls_back_to_config_default_agent(
        ...
        The default ``ChottuConfig`` sets ``default_agent = "simple"``, so
        ``Chottu ask "..."`` should route through SimpleAgent rather than
...
class TestPersonaFilesReachModel:
    ...
    def test_soul_md_content_reaches_engine_in_simple_agent(
        ...
        soul.write_text("PERSONA_SENTINEL_zh_Chottu", encoding="utf-8")
        ...
        assert "PERSONA_SENTINEL_zh_Chottu" in joined

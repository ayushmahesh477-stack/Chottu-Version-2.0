...
from Chottu.cli._install_detect import InstallInfo, detect_install


def _patch_pkg_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point ``Chottu.__file__`` at ``tmp_path / Chottu / __init__.py``."""
    pkg_dir = tmp_path / "Chottu"
...
def test_uv_tool_install_detected(tmp_path, monkeypatch):
    fake = tmp_path / "share" / "uv" / "tools" / "Chottu" / "lib" / "python3.12"
    fake.mkdir(parents=True)
    _patch_pkg_file(fake, monkeypatch)

    info = detect_install()
    assert info.kind == "uv-tool"
    assert info.upgrade_command == "uv tool upgrade Chottu"
...
def test_pypi_install_detected(tmp_path, monkeypatch):
    ...
    assert info.upgrade_command == "pip install --upgrade Chottu"
...
def test_missing_Chottu_file_falls_back_to_pypi(monkeypatch):
    """Chottu unimportable / no __file__ — still get a sane default."""
    with patch("Chottu.cli._install_detect.Path") as mock_path:

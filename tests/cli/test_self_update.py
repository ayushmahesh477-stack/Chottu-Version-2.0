"""Smoke tests for `Chottu self-update`.
...
from Chottu.cli._install_detect import InstallInfo
...
def _mock_info(kind: str = "pypi") -> InstallInfo:
    return InstallInfo(
        kind=kind,
        upgrade_command={
            "pypi": "pip install --upgrade Chottu",
            "uv-tool": "uv tool upgrade Chottu",
            "editable-git": "cd /tmp/repo && git pull && uv sync",
            "unknown": "pip install --upgrade Chottu",
        }[kind],
    )
...
def test_check_flag_prints_command_and_exits_clean():
    with patch(
        "Chottu.cli.self_update_cmd.detect_install",
        return_value=_mock_info("pypi"),
    ):
        runner = CliRunner()
        result = runner.invoke(self_update, ["--check"])
    assert result.exit_code == 0
    assert "pip install --upgrade Chottu" in result.output
...
def test_check_does_not_invoke_subprocess():
    with patch(
        "Chottu.cli.self_update_cmd.detect_install",
        return_value=_mock_info("pypi"),
    ), patch("Chottu.cli.self_update_cmd.subprocess.run") as mock_run:
...
def test_yes_skips_confirmation_and_runs():
    ...
    with patch(
        ...
        "Chottu.cli.self_update_cmd.subprocess.run",
    ...
    assert args[0] == ["pip", "install", "--upgrade", "Chottu"]

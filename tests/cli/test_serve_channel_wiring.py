"""Tests for ChottuSystem.wire_channel() — channel → agent routing.

These tests exercise wire_channel() on ChottuSystem directly. The serve.py
...
def _make_system(engine=None, agent_name="", tmp_path=None) -> ChottuSystem:
    """Build a minimal ChottuSystem with mock engine for testing wire_channel."""
    config = ChottuConfig()
...
# ---------------------------------------------------------------------------
# Tests via ChottuSystem.wire_channel()
# ---------------------------------------------------------------------------
...
class TestChannelToolLoading:
    """ChottuSystem receives tools when agent accepts them."""

    def test_tool_using_agent_receives_tools(self, tmp_path):
        """ChottuSystem built with a tool list passes tools to the agent via ask()."""
    ...
    def test_non_tool_agent_receives_empty_tools(self, tmp_path):
        """ChottuSystem with no tools list results in empty tools — simple agent
        unaffected."""
        config = ChottuConfig()
        config.sessions.db_path = str(tmp_path / "sessions.db")

"""Tests for the ``Chottu eval`` CLI commands."""
...
class TestEvalCLI:
    ...
    def test_eval_group_exists(self):
        """``Chottu eval --help`` shows run/compare/report/list subcommands."""
    ...
    def test_eval_list_benchmarks(self):
        """``Chottu eval list`` exits 0 and outputs benchmark names."""
        ...
        assert "Chottu-direct" in result.output
        assert "Chottu-agent" in result.output

    def test_eval_run_missing_args(self):
        """``Chottu eval run`` without required args fails gracefully."""

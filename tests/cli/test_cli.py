...
import Chottu
from Chottu.cli import cli, main


class TestMainEntryPoint:
    """Tests for the ``Chottu`` console script entry point."""
    ...
    def test_windows_reconfigures_stdout_to_utf8(self) -> None:
        ...
        with (
            ...
            mock.patch("Chottu.cli.cli") as cli_mock,
        ):
            main()
...
class TestCLI:
    def test_help(self) -> None:
        ...
        assert "Chottu" in result.output

    def test_version(self) -> None:
        result = CliRunner().invoke(cli, ["--version"])
        assert result.exit_code == 0
        assert Chottu.__version__ in result.output
...
class TestStartupResilience:
    ...
    down every `Chottu` command — including `Chottu serve` — because the CLI
    ...
    def test_importing_cli_does_not_import_numpy(self) -> None:
        ...
        assert result.returncode == 0, (
            "importing Chottu.cli pulled in numpy (a broken numpy would then "
            f"crash `Chottu serve`):\nstdout={result.stdout}\nstderr={result.stderr}"
        )

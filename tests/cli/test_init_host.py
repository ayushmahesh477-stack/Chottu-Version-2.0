"""Tests for ``Chottu init --host`` option."""
...
from Chottu.core.config import generate_default_toml, generate_minimal_toml
...
class TestInitHost:
    def test_init_host_writes_to_config(self, tmp_path: Path) -> None:
        """Chottu init --host writes the host into config.toml."""
    ...
    def test_init_host_with_vllm(self, tmp_path: Path) -> None:
        """Chottu init --host applies to the selected engine."""
        config_dir = tmp_path / ".Chottu"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
    ...
    def test_init_host_probes_and_reports(self, tmp_path: Path) -> None:
        """Chottu init --host shows reachability status."""
        config_dir = tmp_path / ".Chottu"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("Chottu.cli.init_cmd.PrivacyScanner"),
            mock.patch("Chottu.cli.init_cmd.httpx") as mock_httpx,
        ):
    ...
    def test_init_without_host_still_works(self, tmp_path: Path) -> None:
        """Chottu init without --host still produces valid config."""
        config_dir = tmp_path / ".Chottu"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),

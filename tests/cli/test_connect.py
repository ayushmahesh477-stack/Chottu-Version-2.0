"""Tests for ``Chottu connect`` CLI command."""
...
def test_connect_list_no_connectors() -> None:
    ...
    runner = CliRunner()
    with mock.patch(
        "Chottu.cli.connect_cmd.connect.__wrapped__"
        if hasattr(cli, "__wrapped__")
        else "Chottu.core.registry.ConnectorRegistry.items",
        return_value=(),
    ):
        with mock.patch(
            "Chottu.core.registry.ConnectorRegistry.items",
            return_value=(),
        ):
...
def test_connect_specific_source(tmp_path: object) -> None:
    ...
    with (
        mock.patch(
            "Chottu.core.registry.ConnectorRegistry.contains",
            return_value=True,
        ),
        mock.patch(
            "Chottu.core.registry.ConnectorRegistry.get",

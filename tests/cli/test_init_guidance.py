"""Tests for ``Chottu init`` next-steps guidance."""
...
from Chottu.cli.init_cmd import _next_steps_text
...
class TestInitShowsNextSteps:
    def test_init_shows_next_steps(self, tmp_path: Path) -> None:
        ...
        config_dir = tmp_path / ".Chottu"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("Chottu.cli.init_cmd.PrivacyScanner"),
        ):
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "Getting Started" in result.output
        assert "Chottu ask" in result.output
        assert "Chottu doctor" in result.output
...
class TestNextStepsOllama:
    def test_next_steps_ollama(self) -> None:
        ...
        assert "Chottu ask" in text
        assert "Chottu doctor" in text
...
class TestMinimalConfig:
    def test_init_generates_minimal_by_default(self, tmp_path: Path) -> None:
        """Default Chottu init produces a short config."""
        config_dir = tmp_path / ".Chottu"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
        ...
        assert "Chottu init --full" in content

    def test_init_full_generates_verbose_config(self, tmp_path: Path) -> None:
        """Chottu init --full produces the full reference config."""
        config_dir = tmp_path / ".Chottu"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
...
class TestInitEmptyModelFallback:
    def test_init_no_model_shows_warning(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("Chottu.cli.init_cmd.recommend_model", return_value=""),
            mock.patch("Chottu.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitDownloadDispatch:
    def test_init_ollama_download_calls_ollama_pull(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch(
                "Chottu.cli.init_cmd.ollama_pull",
                return_value=True,
            ) as mock_pull,
            mock.patch("Chottu.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitPrivacyHook:
    def test_init_shows_privacy_summary(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("Chottu.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("Chottu.cli.init_cmd.PrivacyScanner") as MockScanner,
        ):
            from Chottu.cli.scan_cmd import ScanResult
            ...
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "Chottu scan" in result.output

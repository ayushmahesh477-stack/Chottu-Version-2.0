...
from Chottu.core.types import Message, Role, TelemetryRecord
from Chottu.telemetry.aggregator import AggregatedStats, TelemetryAggregator
...
_bench_mod = importlib.import_module("Chottu.cli.bench_cmd")
...
def _energy_config(tmp_path, gpu_metrics=True):
    """Build a ChottuConfig with energy monitoring enabled."""
    cfg = ChottuConfig()
    ...
    # `Chottu ask` (no --agent) now falls back to ``agent.default_agent``
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "Chottu.telemetry.energy_monitor.create_energy_monitor",
...
class TestSdkWiring:
    ...
    def test_energy_monitor_stored(self, tmp_path):
        """Energy monitor is created and stored on Chottu instance."""
        from Chottu.sdk import Chottu
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "Chottu.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "Chottu.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "Chottu.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "Chottu.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path

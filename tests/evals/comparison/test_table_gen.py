"""Tests for Chottu.evals.comparison.table_gen."""
...
from Chottu.evals.comparison.table_gen import (  # noqa: E402
    MixedCommitError,
    ResultsFrame,
    load_results,
    main,
...
class TestRenderBooktabs:
    def test_emits_valid_tabular(self) -> None:
        from Chottu.evals.comparison.table_gen import _render_booktabs
        ...
        assert "hermes" in fragment and "Chottu" in fragment
...
class TestT1Builder:
    def test_t1_builds_from_synthetic_results(self) -> None:
        from Chottu.evals.comparison.table_gen import (
            ResultsFrame,
            _build_t1,
        )
        ...
        assert "Chottu-distilled" in fragment.lower() or "OJ-distilled" in fragment
...
class TestT3to7Builders:
    ...
    def test_builder_emits_tabular(self, builder_name: str) -> None:
        import Chottu.evals.comparison.table_gen as m

        builder = getattr(m, builder_name)
        rows = []
        for fwk in ["hermes", "Chottu"]:
...
class TestTableBuilderRegistry:
    def test_registry_has_all_seven(self) -> None:
        from Chottu.evals.comparison.table_gen import _TABLE_BUILDERS
...
class TestTableGenCLI:
    def test_cli_writes_fragment_and_preview(self, tmp_path: Path) -> None:
        ...
        from Chottu.evals.comparison.table_gen import main

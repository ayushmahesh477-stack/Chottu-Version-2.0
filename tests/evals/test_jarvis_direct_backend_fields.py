"""Verify ChottuDirectBackend.generate_full returns the spec §6.2 extended fields."""
...
class TestChottuDirectExtendedFields:
    def test_generate_full_includes_framework_and_commit(self) -> None:
        ...
        # engine); set required attrs directly. ChottuDirectBackend.generate_full
        # calls ``self._system.engine.generate(messages, ...)`` so we mock the
        # whole ``_system`` chain.
        backend = ChottuDirectBackend.__new__(ChottuDirectBackend)
        ...
        assert result["framework"] == "Chottu"

"""Tests for Chottu.mining.apple_mps_pearl."""
...
_AVAIL = "Chottu.mining._install.pearl_packages_available"
...
def test_detect_requires_macos(hopper_hw):
    from Chottu.mining.apple_mps_pearl import AppleMpsPearlProvider
...
def test_detect_supported_when_torch_mps_available(apple_hw):
    ...
    with (
        patch("Chottu.mining._install.pearl_packages_available", return_value=True),
        patch(
            "Chottu.mining.apple_mps_pearl._torch_mps_available",
...
def test_start_uses_mps_miner_module(mps_config, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("Chottu.mining.cpu_pearl._sidecar_path", lambda: sidecar)
    monkeypatch.setattr("Chottu.mining.cpu_pearl._log_dir", lambda: log_dir)
    monkeypatch.setattr("Chottu.mining.apple_mps_pearl._log_dir", lambda: log_dir)
    ...
    target = "Chottu.mining.apple_mps_pearl.PearlSubprocessLauncher"
    ...
    assert (
        ...
        == "Chottu.mining._mps_miner_loop_main"
    )
...
def test_mps_noisy_gemm_plain_proof_verifies_when_mps_available():
    ...
    from Chottu.mining._mps_miner_loop_main import (
        MpsNoisyGemmAdapter,

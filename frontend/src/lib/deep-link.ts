/**
 * deep-link.ts — Handler for Chottu:// deep links in the Tauri desktop app.
 *
 ...
 *   Chottu://research/{session_id}   → open a research session
 *   Chottu://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `Chottu://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `Chottu://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `Chottu:` scheme.
 *
 * @example
 * parseDeepLink("Chottu://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("Chottu://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "Chottu:") return null;

    // The URL constructor treats "Chottu://research/abc123" such that

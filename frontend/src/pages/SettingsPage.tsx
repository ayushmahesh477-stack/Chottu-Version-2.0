...
export function SettingsPage() {
  ...
  const [memoryEnabled, setMemoryEnabled] = useState(() => {
    try { return localStorage.getItem('Chottu-memory-enabled') !== 'false'; } catch { return true; }
  });
  const [memoryBackend, setMemoryBackend] = useState(() => {
    try { return localStorage.getItem('Chottu-memory-backend') || 'sqlite'; } catch { return 'sqlite'; }
  });
  const [memoryTopK, setMemoryTopK] = useState(() => {
    try { return parseInt(localStorage.getItem('Chottu-memory-top-k') || '5'); } catch { return 5; }
  });
  const [memoryMinScore, setMemoryMinScore] = useState(() => {
    try { return parseFloat(localStorage.getItem('Chottu-memory-min-score') || '0.1'); } catch { return 0.1; }
  });
  const [memoryMaxTokens, setMemoryMaxTokens] = useState(() => {
    try { return parseInt(localStorage.getItem('Chottu-memory-max-tokens') || '2048'); } catch { return 2048; }
  });
  ...
  const handleExport = () => {
    const data = localStorage.getItem('Chottu-conversations') || '{}';
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Chottu-export-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (data.version === 1) {
            localStorage.setItem('Chottu-conversations', JSON.stringify(data));
  ...
  const handleClear = () => {
    ...
    localStorage.removeItem('Chottu-conversations');
    useAppStore.getState().loadConversations();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section title="Appearance">
            <SettingRow label="Theme" description="Choose how Chottu looks">
          ...
          <Section title="Models">
            ...
            <SettingRow label="Cloud providers" description="Green dot means API key is configured">
              <div className="flex flex-wrap gap-3">
                <CloudProviderStatus label="OpenAI" storageKey="Chottu-openai-key" />
                <CloudProviderStatus label="Anthropic" storageKey="Chottu-anthropic-key" />
                <CloudProviderStatus label="Google" storageKey="Chottu-gemini-key" />
                <CloudProviderStatus label="OpenRouter" storageKey="Chottu-openrouter-key" />
              </div>
            </SettingRow>
          </Section>
          ...
          <Section title="API Keys">
            <SettingRow label="OpenAI" description="GPT-4, GPT-3.5, etc.">
              <ApiKeyInput storageKey="Chottu-openai-key" placeholder="sk-..." />
            </SettingRow>
            <SettingRow label="Anthropic" description="Claude models">
              <ApiKeyInput storageKey="Chottu-anthropic-key" placeholder="sk-ant-..." />
            </SettingRow>
            <SettingRow label="Google" description="Gemini models">
              <ApiKeyInput storageKey="Chottu-gemini-key" placeholder="AI..." />
            </SettingRow>
            <SettingRow label="OpenRouter" description="Multi-provider routing">
              <ApiKeyInput storageKey="Chottu-openrouter-key" placeholder="sk-or-..." />
            </SettingRow>
          </Section>
          ...
          <Section title="Tools">
            <SettingRow label="Web Search" description="SerpAPI or Tavily key for web search tool">
              <ApiKeyInput storageKey="Chottu-search-key" placeholder="API key..." />
            </SettingRow>
          </Section>
            ...
            <SettingRow label="Use memory context" description="Automatically inject relevant memories into conversations">
              <button
                onClick={() => {
                  ...
                  try { localStorage.setItem('Chottu-memory-enabled', String(next)); } catch {}
                  showSaved();
                }}
            ...
            <SettingRow label="Memory backend" description="Which retrieval engine to use">
              <select
                ...
                onChange={(e) => {
                  ...
                  try { localStorage.setItem('Chottu-memory-backend', e.target.value); } catch {}
                  showSaved();
                }}
                className="text-sm px-3 py-1.5 rounded-lg outline-none cursor-pointer"
                style={{
            ...
            <SettingRow label="Results to inject" description={`${memoryTopK}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setMemoryTopK(v);
                  try { localStorage.setItem('Chottu-memory-top-k', String(v)); } catch {}
                  showSaved();
                }}
            ...
            <SettingRow label="Min relevance score" description={`${memoryMinScore}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setMemoryMinScore(v);
                  try { localStorage.setItem('Chottu-memory-min-score', String(v)); } catch {}
                  showSaved();
                }}
                className="w-32 cursor-pointer accent-[var(--color-accent)]"
              />
            ...
            <SettingRow label="Max context tokens" description={`${memoryMaxTokens}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  try { localStorage.setItem('Chottu-memory-max-tokens', String(v)); } catch {}
                  showSaved();
                }}
                className="w-32 cursor-pointer accent-[var(--color-accent)]"
              />
          ...
          <Section title="Speech">
            ...
            {!speechBackendAvailable && speechBackendAvailable !== null && (
              <div className="text-xs mt-2 px-1" style={{ color: 'var(--color-text-tertiary)' }}>
                ...
                See the <a href="https://Chottu.github.io/Chottu/user-guide/tools/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-accent)' }}>documentation</a> for details.
              </div>
            )}
          </Section>
          ...
          <Section title="About">
            <div className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
              <p className="mb-2">
                <span className="font-semibold" style={{ color: 'var(--color-text)' }}>Chottu</span> — Programming abstractions for on-device AI.
              </p>
              ...
              <div className="flex gap-3 mt-3 text-xs">
                <a
                  href="https://scalingintelligence.stanford.edu/blogs/Chottu/"
                  target="_blank"
                ...
                <a
                  href="https://Chottu.github.io/Chottu/"
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: 'var(--color-accent)' }}
                >

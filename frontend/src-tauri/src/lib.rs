...
fn find_project_root() -> Option<std::path::PathBuf> {
    ...
    let direct = [
        format!("{home}/Chottu"),
        format!("{home}/projects/hazy/Chottu"),
        format!("{home}/projects/Chottu"),
        format!("{home}/src/Chottu"),
        format!("{home}/Documents/Chottu"),
        format!("{home}/Desktop/Chottu"),
        format!("{home}/Developer/Chottu"),
        format!("{home}/dev/Chottu"),
        format!("{home}/Code/Chottu"),
        format!("{home}/code/Chottu"),
        format!("{home}/repos/Chottu"),
        format!("{home}/github/Chottu"),
    ];
    for p in &direct {
    ...
    // 4. Shallow scan: look for Chottu one level inside common parent dirs.
    //    This catches clones like ~/Documents/my-stuff/Chottu without
    ...
    for parent in &scan_parents {
        ...
        if let Ok(entries) = std::fs::read_dir(&parent_path) {
            for entry in entries.flatten() {
                let candidate = entry.path().join("Chottu");
                if candidate.join("pyproject.toml").exists() {
                    return Some(candidate);
                }
                // Also check if the entry itself is Chottu (case-insensitive match)
                if let Some(name) = entry.file_name().to_str() {
                    if name.eq_ignore_ascii_case("Chottu")
...
// ---------------------------------------------------------------------------
// BackendManager — owns the Ollama + Chottu server child processes
// ---------------------------------------------------------------------------
...
async fn wait_for_chottu_health(
    ...
    {
        Ok(c) => c,
        Err(_) => return ChottuStartResult::Timeout,
    };
    let deadline = tokio::time::Instant::now() + timeout;
    loop {
        ...
        match client.get(url).send().await {
            Ok(resp) => {
                ...
                if status.is_success() {
                    return ChottuStartResult::Ready;
                }
                if status == reqwest::StatusCode::SERVICE_UNAVAILABLE {
                    ...
                    return ChottuStartResult::ServiceUnavailable(body);
                }
        ...
        if tokio::time::Instant::now() >= deadline {
            return ChottuStartResult::Timeout;
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
    }
}
...
async fn boot_backend(backend: SharedBackend, status: SharedStatus) {
    ...
    if project_root.is_none() {
        ...
        let target_path = std::path::PathBuf::from(home_dir()).join("Chottu");
    ...
    match chottu_child {
        ...
        Err(e) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Could not start Chottu server: {}. \
                 Make sure uv is installed (https://astral.sh/uv) and the Chottu repo is cloned at {}",
                e,
                root.display(),
            ));
            return;
    ...
    match wait_for_chottu_health(&server_url, Duration::from_secs(600), &backend).await {
        ...
        ChottuStartResult::EarlyExit { code, stderr } => {
            ...
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Chottu server exited (code {}) before becoming ready.\n\n\
                     No stderr output. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The Chottu repo is at {}\n\
        ...
        ChottuStartResult::Timeout => {
            ...
            s.error = Some(if stderr.is_empty() {
                format!(
                    "Chottu server did not become ready within 10 minutes. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The Chottu repo is at {}\n\
                ...
                format!(
                    "Chottu server did not become ready within 10 minutes.\n\nStderr:\n{}",
                    stderr,
                )
            });
            return;
...
async fn run_chottu_command(args: Vec<String>) -> Result<String, String> {
    ...
    let output = tokio::process::Command::new(&uv_bin)
        ...
        .output()
        .await
        .map_err(|e| format!("Failed to launch Chottu: {}", e))?;
...
mod native_overlay {
    ...
    pub unsafe fn create(html: &str, api_port: u16) {
        ...
        if Class::get("ChottuOverlayPanel").is_none() {
            let sup = Class::get("NSPanel").unwrap();
            let mut decl = ClassDecl::new("ChottuOverlayPanel", sup).unwrap();
        ...
        if Class::get("ChottuOverlayNavDelegate").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("ChottuOverlayNavDelegate", sup).unwrap();
        ...
        if Class::get("ChottuOverlayMsgHandler").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("ChottuOverlayMsgHandler", sup).unwrap();
        ...
        let cls = Class::get("ChottuOverlayPanel").unwrap();
        ...
        let hcls = Class::get("ChottuOverlayMsgHandler").unwrap();
        ...
        let nav_cls = Class::get("ChottuOverlayNavDelegate").unwrap();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let health = MenuItemBuilder::with_id("health", "Health: starting...")
                .enabled(false)
                .build(app)?;
            let quit = MenuItemBuilder::with_id("quit", "Quit Chottu").build(app)?;
        ...
        .expect("error while building Chottu Desktop")
        .run(move |_app, event| {

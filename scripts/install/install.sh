#!/usr/bin/env bash
# install.sh — Chottu curl-pipe-bash installer.
#
# Usage:
...
#   Chottu_HOME        Install dir (default: $HOME/.Chottu)
#   Chottu_REPO_URL    git repo URL (default: https://github.com/Chottu/Chottu.git)
#   Chottu_FORCE_WSL   Set 1 to force WSL detection (testing)
...
# uv/git tooling installs to Windows paths the rest of Chottu can't
...
Chottu runs on Windows via WSL2. Two paths:
  ...
  2. Desktop app — download the .exe from the Releases page:
     https://github.com/Chottu/Chottu/releases

See the WSL2 install guide for the full walkthrough:
  https://Chottu.github.io/Chottu/getting-started/wsl2/
EOF
        exit 1
        ;;
esac
...
Chottu installs to $HOME/.Chottu, not /usr/local. Re-run as your
...
Two ways forward:
  ...
  2. Pre-authenticate sudo before piping (caches credentials for 5 min):
       sudo -v && curl -fsSL https://Chottu.github.io/Chottu/install.sh | bash
EOF
        exit 1
    fi
    local sudo=""
...
Chottu_REPO_URL="${Chottu_REPO_URL:-https://github.com/Chottu/Chottu.git}"
SRC_DIR="$Chottu_HOME/src"
VENV_DIR="$Chottu_HOME/.venv"
STATE_DIR="$Chottu_HOME/.state"
SCRIPTS_DIR="$Chottu_HOME/.scripts"
STATE_FILE="$STATE_DIR/install-state.json"

mkdir -p "$Chottu_HOME" "$STATE_DIR" "$SCRIPTS_DIR"

# ---- WSL detection ----
WSL=0
if [[ "${Chottu_FORCE_WSL:-0}" == "1" ]]; then
    WSL=1
...
# Chottu team can see where users drop off during install.
...
#
ANALYTICS_HOST="${Chottu_ANALYTICS_HOST:-https://34.231.106.201.sslip.io}"
ANALYTICS_KEY="${Chottu_ANALYTICS_KEY:-phc_ysKu72QaxzYNmDpHFcesD2ZZAe68zkdWJEKoYYkc5e3n}"
ANON_ID_FILE="$Chottu_HOME/anon_id"
...
clone_repo() {
    ...
    git clone --depth 1 "$Chottu_REPO_URL" "$SRC_DIR"
}
...
write_config() {
    "$VENV_DIR/bin/Chottu" _bootstrap --write-config \
...
install_symlinks() {
    ...
    ln -sf "$SCRIPTS_DIR/Chottu-wrapper.sh" "$HOME/.local/bin/Chottu"
    ln -sf "$SCRIPTS_DIR/Chottu-uninstall.sh" "$HOME/.local/bin/Chottu-uninstall"
}

# Tracks whether the user needs to source ~/.bashrc / ~/.zshrc / open a
# new terminal before `Chottu` will resolve. Set only when ensure_path
...
ensure_path() {
    ...
    if grep -q "Chottu" "$rc" 2>/dev/null; then
...
from Chottu.core.config import detect_hardware, recommend_model
hw = detect_hardware()
...
# ---- run ----
echo "Chottu installer"
echo "  install dir: $Chottu_HOME"
echo "  WSL2:        $WSL"
echo
...
step clone_repo         "Clone Chottu repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install Chottu"    editable_install
...
#
#   PATH ok + model ok   -> "type Chottu"
#   PATH new + model ok  -> "source rc && Chottu  (or open new terminal)"
#   PATH ok + model bad  -> "model still downloading; Chottu doctor"
#   PATH new + model bad -> "source rc && Chottu doctor; chat works once download finishes"
#
# `Chottu` and `Chottu doctor` need PATH equally, so the source/restart
# guidance goes first when PATH was modified.
NEXT_CMD="Chottu"
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    NEXT_CMD="Chottu doctor"
fi
...
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    cat <<EOF
NOTE: the qwen3.5:2b model didn't finish downloading. 'Chottu doctor'
...
Background work continues silently:
  ...
  Run 'Chottu doctor' to check status anytime.
EOF

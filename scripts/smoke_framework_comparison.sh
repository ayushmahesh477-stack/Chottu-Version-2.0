...
#   Chottu_MOCK_LLM_URL - OpenAI-compatible endpoint (Ollama or vLLM)
#
# Optional:
#   Chottu_ALLOW_COMMIT_DRIFT=1  - bypass commit-pin enforcement

set -euo pipefail

: "${HERMES_AGENT_PATH:?must be set}"
: "${OPENCLAW_PATH:?must be set}"
: "${Chottu_MOCK_LLM_URL:?must be set (e.g. http://localhost:11434/v1)}"
...
uv run python -c "
from Chottu.evals.comparison.third_party import (
...
SMOKE_FRAMEWORKS=(hermes openclaw Chottu)
SMOKE_MODEL="qwen-9b"

for fwk in "${SMOKE_FRAMEWORKS[@]}"; do
  for bench in "${SMOKE_BENCHES[@]}"; do
    uv run python -m Chottu.evals.comparison.make_configs \
      --framework "$fwk" \
      --model "$SMOKE_MODEL" \
      --benchmark "$bench" \
    ...
    Chottu_BACKEND_BASE_URL="$Chottu_MOCK_LLM_URL" uv run python - "$config" "$run_dir" <<'PY'
...
PY
    echo "  ▸ $fwk × $bench"
    mkdir -p "$run_dir"
    Chottu_BACKEND_BASE_URL="$Chottu_MOCK_LLM_URL" \
    Chottu_BACKEND_API_KEY="${Chottu_BACKEND_API_KEY:-smoke}" \
    uv run python -m Chottu.evals run --config "$config" \
      || echo "    FAILED (continuing)"
  done
done

echo "==> Generating T1 from smoke results"
uv run python -m Chottu.evals.comparison.table_gen \

"""Single point of contact between Python and the Rust ``Chottu_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``Chottu_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    ...
    """
    import Chottu_rust  # type: ignore[import-untyped]

    return Chottu_rust


def _detect_rust() -> bool:
    """Return ``True`` if the compiled ``Chottu_rust`` extension is importable.
...
def scan_result_from_json(json_str: str) -> object:
    ...
    from Chottu.security.types import (
        ScanFinding,
        ScanResult,
        ThreatLevel,
    )
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from Chottu.security.injection_scanner import (
        InjectionScanResult,
    )
    from Chottu.security.types import ScanFinding, ThreatLevel
...
def retrieval_results_from_json(json_str: str) -> list:
    ...
    from Chottu.tools.storage._stubs import RetrievalResult

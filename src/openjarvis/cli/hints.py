...
def hint_no_config() -> str:
    ...
    return (
        ...
        "  Run [bold]Chottu init[/bold] to detect hardware and generate "
        "[cyan]~/.Chottu/config.toml[/cyan].\n"
        "  Or run [bold]Chottu quickstart[/bold] for a guided setup."
    )
...
def hint_no_engine(engine_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "  Run [bold]Chottu doctor[/bold] to check all engines.\n"
        "  Run [bold]Chottu quickstart[/bold] for guided setup.\n"
        "\n"
        "  [dim]To use a remote engine:[/dim]\n"
        f"    [cyan]Chottu config set engine.{name}.host http://<remote-ip>:<port>[/cyan]\n"
...
def hint_no_model(model_name: Optional[str] = None) -> str:
    ...
    if model_name:
        return (
            ...
            "  Run [bold]Chottu model list[/bold] to see available models."
        )
    return (

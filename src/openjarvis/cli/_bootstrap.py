...
Used by both ``install.sh`` (via ``Chottu _bootstrap --write-config``)
and ``Chottu init`` (so there is a single source of truth for the
...
import Chottu
from Chottu.core import config as _cfg
...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``Chottu _bootstrap --write-config``)
    and ``Chottu init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        f"# Chottu configuration\n"
        f"{hardware_line}{gpu_comment}\n"
        f"# Full reference config: Chottu init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command — invoked by install.sh, hidden from `Chottu --help`
# ---------------------------------------------------------------------------

"""``Chottu quickstart`` — guided 5-step setup for new users."""
...
def _check_engine_health(engine_key: str) -> bool:
    ...
    try:
        import Chottu.engine  # noqa: F401 — trigger registration
...
def _test_query(engine_key: str) -> str:
    ...
    try:
        ...
        j = Chottu(engine_key=engine_key)
...
def quickstart(force: bool) -> None:
    ...
    if not _check_engine_health(engine_key):
        ...
        if fallbacks:
            ...
            console.print("  Run [bold]Chottu doctor[/bold] for detailed diagnostics.")
            raise SystemExit(1)
    else:
    ...
    console.print()
    console.print(
        '[bold green]Setup complete![/bold green] Try: [bold]Chottu ask "Hello"[/bold]'
    )

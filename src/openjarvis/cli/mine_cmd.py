...
from Chottu.core.config import HardwareInfo, detect_hardware, load_config
...
from Chottu.mining._models import (
    get_pearl_model_spec,
    iter_pearl_model_specs,
    pearl_variant_for_base_model,
)
from Chottu.mining._stubs import Sidecar
from Chottu.mining.vllm_pearl import ensure_registered as ensure_vllm_registered
...
def _ensure_providers_registered() -> None:
    ...
    from Chottu.mining.cpu_pearl import ensure_registered as ensure_cpu_registered

    ensure_cpu_registered()
    try:
        from Chottu.mining.apple_mps_pearl import (
...
def doctor() -> None:
    ...
    if mining_cfg is None:
        _row("RPC", False, "no [mining] config - run `Chottu mine init`")
    else:
...
def init(
        ...
        if local_model_path is None:
            ...
            if not cap.supported:
                raise click.ClickException(
                    ...
                    "See `Chottu mine models` and `Chottu mine doctor` for details."
                )
    ...
    if pearld_password_env not in os.environ:
        click.echo(
            ...
            "Set it before `Chottu mine start`.",
            err=True,
        )

    from Chottu.core.config import DEFAULT_CONFIG_PATH

    config_path = Path(os.environ.get("Chottu_CONFIG", DEFAULT_CONFIG_PATH))
...
"""
    ...
    click.echo("Done. Run `Chottu mine start` to begin mining.")
...
def start() -> None:
    ...
    if cfg is None:
        raise click.ClickException(
            "no [mining] section in config - run `Chottu mine init`"
        )
    provider = MinerRegistry.get(cfg.provider)()
    asyncio.run(provider.start(cfg))
    click.echo(f"Started {cfg.provider}. Run `Chottu mine status` for live stats.")
...
def validate_model(
    ...
    if sidecar is None:
        record(
            "Sidecar",
            False,
            "absent - run `Chottu mine start` first",
        )
    else:
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        launcher._container = client.containers.get("Chottu-pearl-miner")
    except Exception as exc:  # noqa: BLE001

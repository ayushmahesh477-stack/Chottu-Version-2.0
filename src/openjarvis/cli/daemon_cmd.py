"""``Chottu start|stop|restart|status`` — daemon management commands."""
...
from Chottu.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the Chottu server daemon."""
...
def start(
    ...
    """Start the Chottu server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'Chottu stop' to stop it first, or 'Chottu restart'.")
        sys.exit(1)
    ...
    # Build command to run Chottu serve
    cmd = [sys.executable, "-m", "Chottu.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]Chottu server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running Chottu server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the Chottu server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the Chottu server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:

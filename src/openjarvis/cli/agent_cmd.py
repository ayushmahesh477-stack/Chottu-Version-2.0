"""``Chottu agents`` — persistent agent lifecycle management."""
...
def list_agents() -> None:
    ...
    try:
        ...
        if not agents:
            console.print(
                "[dim]No agents found. Create one with: Chottu agents create[/dim]"
            )
            return
...
def _get_system():
    """Build a ChottuSystem for CLI commands that need scheduler/executor."""
...
def _get_scheduler_and_executor(system=None):
    """Get scheduler + executor from a ChottuSystem instance."""
    if system is None:
        system = _get_system()
...
def launch():
    ...
    from Chottu.agents.manager import AgentManager as _AM
...
def run_agent(agent_id):
    ...
    if not instruction and not pending:
        click.echo(
            ...
            f"'Chottu agents ask {agent_id} <message>' to set one.",
            err=True,
        )
        raise SystemExit(1)

...
def _get_discovered_dir() -> Path:
    ...
    return Path("~/.Chottu/skills/discovered/").expanduser()


def _get_overlay_dir() -> Path:
    """Return the directory where optimization overlays are stored."""
    return Path("~/.Chottu/learning/skills/").expanduser()
...
def _get_skill_paths() -> List[Path]:
    ...
    user_dir = Path("~/.Chottu/skills/").expanduser()
...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from Chottu.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from Chottu.skills.sources.openclaw import OpenClawResolver
    ...
    if source == "github":
        ...
        cache = _Path(
            "~/.Chottu/skill-cache/github/" + url.rstrip("/").rsplit("/", 1)[-1]
        ).expanduser()
...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``Chottu skill install hermes:apple-notes``
    """
    console = Console()
    ...
    from Chottu.skills.importer import SkillImporter
    from Chottu.skills.parser import SkillParser
    from Chottu.skills.tool_translator import ToolTranslator
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.Chottu/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: Chottu skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.Chottu/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="Chottu-discover-dryrun-"))
        try:

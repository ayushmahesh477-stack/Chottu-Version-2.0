"""``Chottu serve`` — OpenAI-compatible API server."""
...
def serve(
    ...
    if _has_cloud and engine_name != "cloud":
        try:
            from Chottu.engine.cloud import CloudEngine
            from Chottu.engine.multi import MultiEngine
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    import Chottu.tools  # noqa: F401  # trigger registration
    ...
    api_key = _os.environ.get("Chottu_API_KEY", "")
    if not api_key:
        try:
            import tomllib

            _cfg_path = str(
                __import__("pathlib").Path.home() / ".Chottu" / "config.toml"
            )
    ...
    from Chottu.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from Chottu.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    console.print(
        f"[green]Starting Chottu API server[/green]\n"

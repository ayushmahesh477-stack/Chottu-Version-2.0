"""``Chottu deep-research-setup`` — auto-detect local sources, ingest, and chat.
...
from Chottu.connectors.pipeline import IngestionPipeline
from Chottu.connectors.store import KnowledgeStore
from Chottu.connectors.sync_engine import SyncEngine
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from Chottu.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from Chottu.connectors.imessage import IMessageConnector

        return IMessageConnector(db_path=config.get("db_path", ""))
    elif connector_id == "obsidian":
        from Chottu.connectors.obsidian import ObsidianConnector

        return ObsidianConnector(vault_path=config.get("vault_path", ""))
    elif connector_id == "gmail_imap":
        from Chottu.connectors.gmail_imap import GmailIMAPConnector

        return GmailIMAPConnector()
    elif connector_id == "outlook":
        from Chottu.connectors.outlook import OutlookConnector

        return OutlookConnector()
    elif connector_id == "slack":
        from Chottu.connectors.slack_connector import SlackConnector

        return SlackConnector()
    elif connector_id == "notion":
        from Chottu.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from Chottu.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.Chottu/sync_state.db`` when empty.

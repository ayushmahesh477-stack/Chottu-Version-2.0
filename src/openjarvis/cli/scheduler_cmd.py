"""``Chottu scheduler`` -- task scheduling commands."""
...
def scheduler_run_task(agent_name: str, dry_run: bool) -> None:
    ...
    when Chottu is not running as a persistent daemon.

    Example (launchd plist ProgramArguments):
        Chottu scheduler run-task proactive
    """
    console = Console()
    store = _get_store()
    try:
        ...
        if match is None:
            console.print(
                ...
                "Register it first with 'Chottu scheduler create'.[/yellow]"
            )
            return

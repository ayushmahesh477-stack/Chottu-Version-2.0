"""Check for newer Chottu releases on PyPI."""
...
_CACHE_PATH = Path("~/.Chottu/version-check.json").expanduser()
_CACHE_TTL = 86400  # 24 hours
_PYPI_API = "https://pypi.org/pypi/Chottu/json"


def _config_path() -> Path:
    """Resolve the config path, honoring ``Chottu_CONFIG`` like core.config."""
    override = os.environ.get("Chottu_CONFIG")
    if override:
        return Path(override).expanduser()
    return Path("~/.Chottu/config.toml").expanduser()
...
# - ``Chottu_NO_UPDATE_CHECK=1`` — project-specific
# - ``CI=true`` — set by every major CI provider, suppresses by default
_OPT_OUT_ENV_VARS = ("Chottu_NO_UPDATE_CHECK",)
...
def _check_disabled() -> bool:
    ...
    # ``Chottu_NO_UPDATE_CHECK=0`` if they want the nudge anyway.
...
def check_for_updates(command_name: str) -> None:
    ...
    Honors ``Chottu_NO_UPDATE_CHECK=1`` and ``CI=true`` — any
...
def _do_check() -> None:
    ...
    try:
        if Version(latest) > Version(current):
            from Chottu.cli._install_detect import detect_install

            cmd = detect_install().upgrade_command
            sys.stderr.write(
                f"\033[33mA new version of Chottu is available "
                f"(v{current} → v{latest})\n"
                f"Update: {cmd}\n"
                f"Or run: Chottu self-update\033[0m\n\n"
            )

"""``Chottu doctor`` — run diagnostic checks on the Chottu installation."""
...
from Chottu.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `Chottu init` to generate a config file.",
    )
...
def _check_engines() -> List[CheckResult]:
    ...
    from Chottu.engine import _discovery
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "Chottu[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "Chottu[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "Chottu[energy-amd]", "AMD energy monitoring"),
        ("colbert", "Chottu[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "Chottu[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your Chottu installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]Chottu Doctor[/bold]")
    console.print()
    ...
    from Chottu.cli._bg_state import get_status
    ...
    if bg.rust_extension == "ready":
        ...
        console.print(
            "    retry: ~/.Chottu/.scripts/install-rust.sh && "
            "~/.Chottu/.scripts/build-extension.sh"
        )
        bg_failed = True
    else:
        console.print(
    ...
    for model_id, state in bg.models.items():
        if state == "ready":
            ...
            console.print(f"    retry: ~/.Chottu/.scripts/pull-model.sh {model_id}")
            bg_failed = True
        else:

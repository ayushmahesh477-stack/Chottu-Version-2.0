"""``Chottu compose`` — unified composition CLI for discrete agents and operators."""
...
@click.group()
def compose() -> None:
    """Compose, run, benchmark, and deploy Chottu configurations.
...
def compose_list(kind: Optional[str]) -> None:
    ...
    try:
        from Chottu.recipes.loader import discover_recipes

        recipes = discover_recipes(kind=kind)
        if not recipes:
            console.print("[dim]No compositions found.[/dim]")
            console.print(
                "[dim]Place TOML recipes in src/Chottu/recipes/data/ "
                "or ~/.Chottu/recipes/[/dim]"
            )
            return
...
def compose_show(name: str) -> None:
    ...
    console = Console(stderr=True)
    try:
        from Chottu.recipes.loader import resolve_recipe
...
def compose_stop(name: str) -> None:
    ...
    try:
        ...
        from Chottu.recipes.loader import _PROJECT_OPERATORS_DIR

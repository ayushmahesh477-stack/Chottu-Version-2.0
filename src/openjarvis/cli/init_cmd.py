"""``Chottu init`` — detect hardware, generate config, write to disk."""
...
from Chottu.cli._bootstrap import detect_cloud_keys
from Chottu.cli.model import find_model_spec, hf_download, ollama_pull
from Chottu.cli.scan_cmd import PrivacyScanner
...
# Engines supported by ``Chottu init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     Chottu ask "Hello"\n'
            "\n"
            "  Run `Chottu doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]Chottu scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-Chottu first-run guard.",
)
@click.pass_context
def init(
    ctx: click.Context,
    ...
    from_bare_Chottu: bool = False,
) -> None:
    """Detect hardware and generate ~/.Chottu/config.toml."""
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "Chottu" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]Chottu doctor[/bold] to verify."
        )
        return
    ...
    if engine is None and config is None:
        ...
        # Bare-Chottu cold path: use the recommended engine non-interactively.
        if from_bare_Chottu:
            engine = recommended
        else:
            console.print()
...
"""
        ...
        console.print(
            ...
            "Run [bold]Chottu connect gdrive[/bold] to connect "
            "Google services, then [bold]Chottu digest --fresh[/bold]."
        )
    ...
    if not soul_path.exists():
        soul_path.write_text(
            "# Agent Persona\n\nYou are Chottu, a helpful personal AI assistant.\n"
        )
    ...
    if not model:
        ...
        if not no_download and not from_bare_Chottu and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]Chottu model pull {model}[/bold]"
                )

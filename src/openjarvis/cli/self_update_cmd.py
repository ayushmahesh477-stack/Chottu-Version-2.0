"""`Chottu self-update` — upgrade Chottu to the latest release.

Runs the right upgrade command for how the user installed Chottu:

- PyPI installs get ``pip install --upgrade Chottu``.
- uv-tool installs get ``uv tool upgrade Chottu``.
...
@click.command(
    "self-update",
    help=(
        "Upgrade Chottu to the latest release. Detects how you "
...
def self_update(check: bool, yes: bool) -> None:
    ...
    click.echo(f"Current Chottu version: v{current}")
    ...
    click.echo("\nUpgrade complete. Re-run `Chottu --version` to confirm.")

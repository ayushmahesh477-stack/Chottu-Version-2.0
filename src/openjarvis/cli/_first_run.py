"""Bare-`Chottu` first-run guard.

When the user types ``Chottu`` with no subcommand, route them to the
chat command if a config exists, otherwise into the init wizard with
the ``--from-bare-Chottu`` flag (which lets init suppress the
...
def check_and_route(ctx: click.Context) -> None:
    ...
    (the user typed something specific like ``Chottu ask``).
    """
    if ctx.invoked_subcommand is not None:
        return

    # Late imports to avoid circular import with cli/__init__.py.
    from Chottu.cli.chat_cmd import chat as chat_cmd
    from Chottu.cli.init_cmd import init as init_cmd

    if _cfg.DEFAULT_CONFIG_PATH.exists():
        ctx.invoke(chat_cmd)
    else:
        ctx.invoke(init_cmd, from_bare_Chottu=True)

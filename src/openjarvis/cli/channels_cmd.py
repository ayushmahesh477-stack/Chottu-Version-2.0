"""``Chottu channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from Chottu.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "Chottu channels imessage-start <contact>",
        )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "Chottu.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from Chottu.connectors.retriever import (
            TwoStageRetriever,
        )
        from Chottu.connectors.store import KnowledgeStore
        from Chottu.engine.ollama import OllamaEngine
        from Chottu.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from Chottu.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from Chottu.channels.imessage_daemon import stop_daemon

"""``Chottu eval`` — evaluation framework CLI commands."""
...
@click.option(
    ...
    type=click.Choice(
        ["Chottu-direct", "Chottu-agent", "hermes", "openclaw", "terminalbench-native"]
    ),
    help=(
        "Inference backend. For hermes/openclaw, also pass --base-url and "
        "--api-key (or set Chottu_BACKEND_BASE_URL/Chottu_BACKEND_API_KEY)."
    ),
)
@click.option(
    "--base-url",
...
@click.option(
    ...
    "api_key",
    default=None,
    help=("API key for the hermes/openclaw endpoint (env: Chottu_BACKEND_API_KEY)."),
)
@click.option(
    "--agent",
    "agent_name",
...
def eval_run(
    ...
    if config_path is not None:
        ...
        try:
            from Chottu.evals.cli import _run_single
        except ImportError:
    ...
    config = RunConfig(
        ...
        # so users can also set Chottu_BACKEND_BASE_URL/Chottu_BACKEND_API_KEY.

...
class OAuthProvider:
    ...
    # Filenames in ~/.Chottu/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.Chottu/connectors/`` and falls
    back to environment variables ``Chottu_{NAME}_CLIENT_ID`` and
    ``Chottu_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"Chottu_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to Chottu.</p>"
                    b"</body></html>"
                )

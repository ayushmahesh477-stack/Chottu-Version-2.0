...
Uses OAuth 2.0 tokens stored locally (see :mod:`Chottu.connectors.oauth`).
...
from Chottu.connectors._stubs import BaseConnector, Document, SyncStatus
from Chottu.connectors.google_auth import (
    GoogleAuthError,
)
from Chottu.connectors.google_auth import (
    call_with_refresh as _call_with_refresh,
)
from Chottu.connectors.oauth import (
    GOOGLE_ALL_SCOPES,
...
class GmailConnector(BaseConnector):
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `Chottu connect gmail` short-circuit with
    ...
    def _refresh_token(self) -> str:
        ...
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed — re-run `Chottu connect gmail`"
            )
        return new

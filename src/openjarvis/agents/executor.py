...
from Chottu.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a ChottuSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after ChottuSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from Chottu.agents import AgentRegistry
        ...
        # Resolve engine + model from ChottuSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in ChottuSystem")
        model = (
            config.get("model")
        ...
        # agent's own ToolExecutor. Set by CLI paths like `Chottu agents ask`
        ...
        if self._system is not None:
            ...
            # agents, mirroring the one-shot `Chottu ask` path so they no
    ...
    def _save_trace(
        ...
        from Chottu.core.types import StepType, Trace, TraceStep

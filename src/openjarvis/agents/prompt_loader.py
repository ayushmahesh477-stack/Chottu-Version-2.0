"""Load system prompt and few-shot overrides from $Chottu_HOME.
...
- System prompts: ``$Chottu_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$Chottu_HOME/agents/{name}/few_shot.json``
...
def _Chottu_home() -> Path:
    """Resolve $Chottu_HOME, defaulting to ~/.Chottu."""
    return Path(os.environ.get("Chottu_HOME", "~/.Chottu")).expanduser()


def load_system_prompt_override(agent_name: str) -> str | None:
    """Return the override prompt for *agent_name*, or ``None``.

    Looks for ``$Chottu_HOME/agents/<agent_name>/system_prompt.md``.
    ``Chottu_HOME`` defaults to ``~/.Chottu`` when unset.
    """
    home = _Chottu_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$Chottu_HOME/agents/<agent_name>/few_shot.json``.

...
harness — each is registered as a standard Chottu agent so the rest
of the platform (SDK, CLI, distillation, evals) can use them like any
other agent. Results live under ``$Chottu_HYBRID_EXPERIMENTS_DIR``
(defaults to ``~/.Chottu/experiments/hybrid/``).
...
Items in parentheses are what the *paper* trains. These Chottu ports
...
`src/Chottu/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).
...
# 3. Run a smoke cell
.venv/bin/python -m Chottu.agents.hybrid.runner \
    --cell minions-gaia-qwen27b-opus-3
```

Outputs land in
`$Chottu_HYBRID_EXPERIMENTS_DIR/runs/<cell>/{results.jsonl,summary.json,config.json,logs/}`
(defaults to `~/.Chottu-hybrid/experiments/`). The schema matches the
...
```bash
src/Chottu/agents/hybrid/scripts/new_experiment.sh \
...
(`~/.Chottu/experiments/hybrid/docs/results.md`) at full N —
...
these Chottu cells should reproduce the harness numbers within

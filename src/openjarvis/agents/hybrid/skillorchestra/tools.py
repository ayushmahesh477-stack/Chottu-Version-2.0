...
both forced by the Chottu environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. Chottu
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — Chottu scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    Chottu substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip() + "\n\n"
        + f"Question: {problem}\nInstead of directly answering the question, "

...
from Chottu.agents.hybrid._base import (
    ANTHROPIC_WEB_SEARCH_TOOL,
    WEB_SEARCH_COST_PER_CALL,
    LocalCloudAgent,
)
from Chottu.agents.hybrid._prices import (
    PRICES,
    is_gpt5_family,
    supports_temperature,
)
from Chottu.agents.hybrid.mini_swe_agent import (
    _clone_repo,
    _extract_diff,
    run_swe_agent_loop,
)
...
# Map the orchestrator's `model` slot to a concrete Chottu worker spec.
...
def _call_tavily_search(query: str, max_results: int = 5) -> Tuple[str, int, int]:
    ...
    Token counts are reported as zero (no LLM was billed); the Chottu
...
_MODAL_APP_NAME = "Chottu-toolorchestra-sandbox"

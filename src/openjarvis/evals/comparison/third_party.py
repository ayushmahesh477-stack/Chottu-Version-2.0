...
``Chottu_ALLOW_COMMIT_DRIFT=1`` disables strict commit-pin enforcement.
"""
...
# python so its deps don't need to be installed in Chottu's venv); for
...
def verify_commit_pin(entry: ThirdPartyEntry) -> None:
    ...
    Raises:
        ...
        CommitDriftError: if HEAD doesn't match pinned_commit, unless
            ``Chottu_ALLOW_COMMIT_DRIFT=1`` is set (then logs a warning).
    """
    ...
    if not actual.startswith(pinned) and not pinned.startswith(actual):
        msg = (
            ...
            f"To bypass for this run, set Chottu_ALLOW_COMMIT_DRIFT=1."
        )
        if os.environ.get("Chottu_ALLOW_COMMIT_DRIFT") == "1":
            LOGGER.warning(msg)
            return

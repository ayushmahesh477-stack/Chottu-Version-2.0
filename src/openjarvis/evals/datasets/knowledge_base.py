...
from Chottu.evals.core.dataset import DatasetProvider
from Chottu.evals.core.types import EvalRecord
...
_RECORDS = [
    {
        "documents": "Chottu Architecture Guide (v2.9):\nChottu is organized around five composable primitives: Intelligence, Engine, Agents, Tools, and Learning. The Intelligence primitive handles model definition and catalog management via ModelRegistry. The Engine primitive provides inference through backends including Ollama, vLLM, SGLang, llama.cpp, MLX, and LM Studio. All engines implement the InferenceEngine ABC with generate(), stream(), list_models(), and health() methods.",
        "question": "What are the five primitives of Chottu and what does the Engine primitive provide?",
    ...
    },
    {
        ...
        "question": "How does Chottu integrate with external MCP tools?",
    ...
    {
        "documents": "Workflow Engine Documentation:\nThe workflow engine uses DAG-based WorkflowGraph with cycle detection and topological sort. Parallel stages execute via ThreadPoolExecutor. WorkflowBuilder provides a fluent API for graph construction. WorkflowEngine executes workflows against a ChottuSystem instance. Workflows can be defined in TOML files. Supported node types: agent, tool, condition, parallel, loop, and transform.",
    ...
    {
        "documents": "Speech Subsystem Technical Reference:\nChottu supports speech-to-text with pluggable backends. SpeechBackend ABC defines transcribe(), health(), and supported_formats() methods. Three backends are available: FasterWhisperBackend (local, CTranslate2, key 'faster-whisper'), OpenAIWhisperBackend (cloud, whisper-1, key 'openai'), and DeepgramSpeechBackend (cloud, nova-2, key 'deepgram'). Auto-discovery prioritizes local backends.",
    ...
    {
        "documents": "Channel System Architecture:\nChottu supports multi-platform messaging via the BaseChannel ABC. Available channels include WhatsAppBaileysChannel, LINEChannel, ViberChannel, MessengerChannel, RedditChannel, MastodonChannel, XMPPChannel, RocketChatChannel, ZulipChannel, TwitchChannel, and NostrChannel. All channels use @ChannelRegistry.register() for discovery and integrate with the EventBus.",
        "question": "How many messaging channels does Chottu support and how are they discovered?",
        "answer": "Chottu supports 11 messaging channels. They are discovered via the @ChannelRegistry.register() decorator.",
    },
    {
    ...
    {
        "documents": "Desktop Application Guide:\nThe Chottu desktop app uses Tauri 2.0. It includes 5 dashboard panels: EnergyDashboard (real-time power monitoring with recharts), TraceDebugger (timeline inspection with step-type color coding), LearningCurve (policy visualization for GRPO/bandit stats), MemoryBrowser (search and stats), and AdminPanel (health, agents, server control). Tauri commands proxy to the Chottu REST API.",
    ...
    {
        "documents": "API Server Reference:\nThe OpenAI-compatible server is started via 'Chottu serve'. Core endpoints: POST /v1/chat/completions, GET /v1/models, GET /health. Extended endpoints cover agents, memory, traces, telemetry, learning, skills, sessions, budget, and metrics. SSE streaming is supported on /v1/chat/completions with stream=true. WebSocket streaming is available at WS /v1/chat/stream.",
        "question": "How is the API server started and what streaming options are available?",
        "answer": "The API server is started via 'Chottu serve'. SSE streaming is available on /v1/chat/completions with stream=true, and WebSocket streaming is available at WS /v1/chat/stream.",
    },
    {
    ...
    {
        "documents": "Vault System Guide:\nThe vault provides encrypted credential storage at ~/.Chottu/vault.enc using Fernet encryption. The encryption key is auto-generated with 0o600 file permissions for security. CLI commands: 'Chottu vault set KEY' to store, 'Chottu vault get KEY' to retrieve, and 'Chottu vault list' to list stored keys.",
        "question": "What encryption does the vault use and where is it stored?",
        "answer": "The vault uses Fernet encryption and is stored at ~/.Chottu/vault.enc.",
    },
    {
    ...
    {
        "documents": "A2A Protocol Implementation:\nChottu implements the Google Agent-to-Agent spec using JSON-RPC 2.0. A2AServer supports tasks/send, tasks/get, and tasks/cancel operations. Agent discovery is available at /.well-known/agent.json. A2AClient enables calling external A2A agents. A2AAgentTool adapts A2A agents as tool-callable resources.",
    ...
    {
        "documents": "Deployment Guide:\nChottu provides three Docker variants: Dockerfile (Python 3.12-slim for CPU), Dockerfile.gpu (NVIDIA CUDA 12.4), and Dockerfile.gpu.rocm (AMD ROCm 7.2). The docker-compose.yml runs two services: Chottu on port 8000 and ollama on port 11434. SystemD and launchd service files are also provided for system-level deployment.",
        "question": "What Docker variants are available and what ports do the services use?",
        "answer": "Three Docker variants: CPU (Python 3.12-slim), NVIDIA (CUDA 12.4), and AMD (ROCm 7.2). Chottu runs on port 8000 and Ollama on port 11434.",
    },
    {
    ...
    {
        "documents": "Hardware Detection System:\nChottu auto-detects GPU vendor, model, and VRAM using platform-specific tools: nvidia-smi for NVIDIA GPUs, rocm-smi for AMD GPUs, system_profiler for Apple Silicon, and /proc/cpuinfo for CPU features. Based on detection, it recommends the appropriate inference engine (vLLM for NVIDIA, MLX for Apple, llama.cpp for CPU-only).",
        "question": "How does Chottu detect hardware and what engine is recommended for Apple Silicon?",
        "answer": "Chottu detects hardware using nvidia-smi, rocm-smi, system_profiler, and /proc/cpuinfo. MLX is recommended for Apple Silicon.",
    },
    {
    ...
    {
        "documents": "MCP Quick-Add Feature:\nThe 'Chottu add' command provides quick MCP server setup with 8 built-in templates: github, filesystem, slack, postgres, brave-search, memory, puppeteer, and google-maps. Configuration is saved as JSON to ~/.Chottu/mcp/. Each template includes the server command, arguments, and required environment variables.",
        "question": "How many MCP server templates are available via 'Chottu add' and where is configuration saved?",
        "answer": "8 MCP server templates are available. Configuration is saved to ~/.Chottu/mcp/.",
    },
    {
        "documents": "Composition Layer Documentation:\nSystemBuilder provides a fluent builder pattern that produces ChottuSystem instances. The builder wires together engine, model, agent, tools, telemetry, traces, workflow, sessions, and capability policy. ChottuSystem exposes ask() for queries and close() for resource cleanup. The SDK (Chottu class) wraps SystemBuilder with a simplified sync API.",
        "question": "What is the relationship between SystemBuilder, ChottuSystem, and the Chottu SDK class?",
        "answer": "SystemBuilder is a fluent builder that produces ChottuSystem instances. The Chottu SDK class wraps SystemBuilder with a simplified synchronous API.",
    },
]

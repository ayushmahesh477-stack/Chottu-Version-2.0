...
class SkillBenchmarkConfig:
    ...
    skills_dir: Path = field(
        default_factory=lambda: Path("~/.Chottu/skills/").expanduser()
    )
    overlay_dir_dspy: Path = field(
        default_factory=lambda: Path("~/.Chottu/learning/skills-dspy/").expanduser()
    )
    overlay_dir_gepa: Path = field(
        default_factory=lambda: Path("~/.Chottu/learning/skills-gepa/").expanduser()
    )
...
class SkillBenchmarkRunner:
    ...
    def __init__(self, config: SkillBenchmarkConfig) -> None:
        ...
        # the default ~/.Chottu/learning/skills/ tree.
    ...
    def _backend_kwargs_for_condition(self, condition: str) -> Dict[str, Any]:
        """Return the kwargs to pass to ChottuAgentBackend for *condition*.
    ...
    def _build_backend_for_condition(self, condition: str) -> Any:
        """Construct a ChottuAgentBackend for *condition*.
    ...
    def _run_single_seed(
        ...
        try:
            judge_backend = ChottuDirectBackend(
                engine_key=self._config.engine,
            )

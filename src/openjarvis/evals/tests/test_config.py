...
from Chottu.evals.core.config import EvalConfigError, expand_suite, load_eval_config
...
class TestDataclassDefaults:
    ...
    def test_benchmark_config_defaults(self):
        ...
        assert b.backend == "Chottu-direct"
...
class TestExpandSuite:
    ...
    def test_backend_from_benchmark(self):
        suite = EvalSuiteConfig(
            models=[ModelConfig(name="m1")],
            benchmarks=[BenchmarkConfig(name="gaia", backend="Chottu-agent")],
        )
        configs = expand_suite(suite)
        assert configs[0].backend == "Chottu-agent"
...
class TestCLIConfig:
    def test_run_missing_benchmark_and_config(self):
        ...
        from Chottu.evals.cli import main
    ...
    def test_run_config_loads_and_prints_suite_info(self, tmp_path):
        ...
        runner = CliRunner()
        with patch("Chottu.evals.cli._run_single", side_effect=Exception("mock")):

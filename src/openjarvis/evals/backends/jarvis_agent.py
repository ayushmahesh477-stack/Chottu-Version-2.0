"""Chottu Agent backend — agent-level inference with tool calling."""
...
from Chottu.evals.backends._commit_util import Chottu_commit
from Chottu.evals.core.backend import InferenceBackend


class ChottuAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + ChottuSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "Chottu-agent"
    framework_name = "Chottu"
    ...
    def __init__(
        ...
        # Override the agent's per-run turn budget. ChottuConfig.agent.max_turns
    ...
    def framework_commit_value(self) -> str:
        """Chottu repo HEAD commit (for telemetry tagging)."""
        from Chottu.evals.backends._commit_util import Chottu_commit

        return Chottu_commit()
    ...
    def generate_full(
        ...
        *,
        ...
        return {
            ...
            "framework": "Chottu",
            "framework_commit": Chottu_commit(),
            "error": None,
        }

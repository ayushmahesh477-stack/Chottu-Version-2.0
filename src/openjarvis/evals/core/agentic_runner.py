...
from Chottu.evals.core.event_recorder import AgentEvent, EventRecorder, EventType
...
class AgenticRunner:
    ...
    async def _run_single_query(
        ...
        # ToolExecutor publishes to ChottuSystem.bus; we relay those into
        ...
        def _run_body() -> None:
            ...
            with ctx:
                ...
                if task_env is not None and hasattr(task_env, "run_agent_loop"):
                    ...
                    if hasattr(agent, "ask"):
                        # SystemBuilder-based agent (ChottuSystem)
        ...
        for turn in turns:
            if turn.cost_usd is None and (
                ...
                from Chottu.evals.core.pricing import compute_turn_cost

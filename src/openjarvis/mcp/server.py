"""MCP Server — wraps Chottu tools as MCP-discoverable tools."""
...
from Chottu.mcp.protocol import (
    INTERNAL_ERROR,
    INVALID_PARAMS,
    METHOD_NOT_FOUND,
    MCPRequest,
...
class MCPServer:
    """MCP server that exposes Chottu tools via JSON-RPC.
    ...
    def _auto_discover_tools() -> List[BaseTool]:
        ...
        try:
            ...
            pass
        try:
            from Chottu.tools.web_search import WebSearchTool

            _tool_classes.append(WebSearchTool)
        except ImportError:
            pass
        try:
            from Chottu.tools.code_interpreter import CodeInterpreterTool

            _tool_classes.append(CodeInterpreterTool)
        except ImportError:
            pass
        try:
            from Chottu.tools.repl import ReplTool
        ...
        # Storage MCP tools
        try:
            from Chottu.tools.storage_tools import (
                MemoryIndexTool,
                MemoryRetrieveTool,
                MemorySearchTool,
                MemoryStoreTool,
        ...
        # Channel MCP tools
        try:
            from Chottu.tools.channel_tools import (
                ChannelListTool,
                ChannelSendTool,
                ChannelStatusTool,
            )

...
from Chottu.engine._base import InferenceEngine
...
def _make_engine(key: str, config: ChottuConfig) -> InferenceEngine:
...
def _maybe_register_mining_sidecar_engine() -> None:
    ...
    from Chottu.engine._openai_compat import _OpenAICompatibleEngine
...
def discover_engines(config: ChottuConfig) -> List[Tuple[str, InferenceEngine]]:
...
def get_engine(
    config: ChottuConfig, engine_key: str | None = None

...
must NEVER be inside the Chottu source tree. ``resolve_spec_search_root``
walks up from this module's ``__file__`` looking for a ``pyproject.toml`` that
identifies the Chottu source root, then refuses to operate if the resolved
root is inside it. Defense in depth — if a user accidentally points
``Chottu_HOME`` at the repo, the system fails loudly instead of silently
...
def _find_source_root() -> Path | None:
    """Walk upward from this module to find the Chottu source root.

    Returns the directory containing the Chottu ``pyproject.toml``, or
    ...
    for candidate in (here, *here.parents):
        ...
        if py.exists():
            try:
                ...
                continue
            if 'name = "Chottu"' in content.lower():
                return candidate
    return None


def _resolve_Chottu_home() -> Path:
    """Resolve the Chottu_HOME directory (env var or default)."""
    env = os.environ.get("Chottu_HOME")
    if env:
        return Path(env).expanduser().resolve()
    return (Path.home() / ".Chottu").resolve()


def resolve_spec_search_root() -> Path:
    """Return the absolute path of the spec-search root directory.

    The root is ``$Chottu_HOME/learning`` (or ``~/.Chottu/learning``
    by default). Raises ``ConfigurationError`` if the resolved path lies
    inside the Chottu source tree, to prevent dev artifacts from leaking
    into the repo.
    """
    home = _resolve_Chottu_home()
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"Chottu_HOME ({home}) is inside the source tree "
                f"({source_root}). Spec search refuses to write runtime "
                "artifacts inside the Chottu repo. Set Chottu_HOME "
                "to a directory outside the repo (default: ~/.Chottu)."
            )

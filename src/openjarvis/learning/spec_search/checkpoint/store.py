...
A thin wrapper over a local git repository at ``<Chottu_home>/.git``.
...
from Chottu.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.Chottu/`` (or ``$Chottu_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the Chottu source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"Chottu source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@Chottu.local")
            self._git("config", "user.name", "Chottu Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new Chottu home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",

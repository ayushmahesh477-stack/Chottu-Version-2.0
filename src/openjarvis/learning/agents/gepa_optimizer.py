...
Uses GEPA's adapter pattern to bridge Chottu traces into GEPA's
...
from Chottu.core.config import GEPAOptimizerConfig
from Chottu.core.registry import LearningRegistry
from Chottu.learning._stubs import AgentLearningPolicy
...
class ChottuGEPAAdapter:
    """Implements GEPA's adapter protocol for Chottu agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'Chottu[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = ChottuGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "ChottuGEPAAdapter"]

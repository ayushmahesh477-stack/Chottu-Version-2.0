...
The wrapper adapts Chottu traces into ACE's
...
sidecar overlay under ``~/.Chottu/learning/ace/<task>/`` for the
...
from Chottu.core.config import ACEOptimizerConfig
...
def _default_save_dir(task_name: str) -> Path:
    return Path.home() / ".Chottu" / "learning" / "ace" / task_name


class _TraceDataProcessor:
    """Adapter that exposes Chottu traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert Chottu trace records to ACE's sample dict shape."""

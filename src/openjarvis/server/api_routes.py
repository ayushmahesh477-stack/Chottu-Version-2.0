...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import Chottu.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from Chottu.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from Chottu.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from Chottu.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from Chottu.tools.agent_tools import AgentSendTool
...
def _get_memory_backend(request: Request):
    ...
    if backend is None:
        try:
            from Chottu.tools.storage.sqlite import SQLiteMemory
...
async def telemetry_stats(request: Request):
    ...
    try:
        ...
        from Chottu.telemetry.aggregator import TelemetryAggregator
...
async def list_skills(request: Request):
    ...
    try:
        from Chottu.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.Chottu/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from Chottu.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP Chottu_requests_total Total requests processed",
            "# TYPE Chottu_requests_total counter",
            f"Chottu_requests_total {stats.get('total_requests', 0)}",
            "# HELP Chottu_tokens_total Total tokens generated",
            "# TYPE Chottu_tokens_total counter",
            f"Chottu_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP Chottu_latency_avg_ms Average latency in milliseconds",
            "# TYPE Chottu_latency_avg_ms gauge",
            f"Chottu_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
async def websocket_chat_stream(websocket: WebSocket):
    ...
    """
    from Chottu.server.auth_middleware import websocket_authorized
...
async def learning_stats(request: Request):
    ...
    # Skill discovery
    try:
        from Chottu.learning.agents.skill_discovery import SkillDiscovery
...
def include_all_routes(app) -> None:
    ...
    from Chottu.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from Chottu.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    try:
        ...
        from Chottu.server.ws_bridge import create_ws_router

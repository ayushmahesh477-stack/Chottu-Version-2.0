...
from Chottu.server.models import (
    ChatCompletionChunk,
        ...
        try:
            ...
            if query_text:
                ...
                if len(enriched) > len(messages):
                    from Chottu.server.models import ChatMessage
            ...
            logging.getLogger("Chottu.server").debug(
...
def _handle_direct(
    ...
    if bus:
        from Chottu.telemetry.wrapper import instrumented_generate
...
def _handle_agent(
    ...
    if audio_path:
        ...
        from Chottu.server.models import AudioMeta
...
async def _handle_stream_tools(
    ...
    """
    from Chottu.server.cloud_router import is_cloud_model
    ...
    async def generate():
        ...
        try:
            ...
            logging.getLogger("Chottu.server").error(
                "Tool stream error: %s",
                exc,
                exc_info=True,
            )
...
async def _handle_stream(
    ...
    from Chottu.server.cloud_router import (
        is_cloud_model,
        stream_cloud,
        stream_local,
    )
...
async def list_models(request: Request) -> ModelListResponse:
    ...
    """
    from Chottu.server.cloud_router import is_cloud_model, list_local_models
...
async def reload_cloud_engine(request: Request):
    ...
    # Re-read ~/.Chottu/cloud-keys.env and update the running process env.
    keys_path = Path.home() / ".Chottu" / "cloud-keys.env"
    if keys_path.exists():

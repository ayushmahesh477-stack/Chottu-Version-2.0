"""FastAPI application factory for the Chottu API server."""
...
from Chottu.server.analytics_routes import router as analytics_router
from Chottu.server.api_routes import include_all_routes
from Chottu.server.comparison import comparison_router
from Chottu.server.connectors_router import create_connectors_router
from Chottu.server.dashboard import dashboard_router
from Chottu.server.digest_routes import create_digest_router
from Chottu.server.research_router import router as research_router
from Chottu.server.routes import router
from Chottu.server.upload_router import router as upload_router
        ...
        for agent in mgr.list_agents():
            ...
            for b in bindings:
                ...
                if bridge and hasattr(bridge, "_channels"):
                    ...
                    from Chottu.server.channel_bridge import ChannelBridge
                    from Chottu.server.session_store import SessionStore
...
def create_app(
    ...
    *,
    ...
    config:
        Optional ChottuConfig for other settings.
    """
    app = FastAPI(
        title="Chottu API",
        description="OpenAI-compatible API server for Chottu",
        version="0.1.0",
    )
    ...
    try:
        from Chottu.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from Chottu.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from Chottu.server.webhook_routes import (

...
logger = logging.getLogger("Chottu.server.agent_manager")
...
def _resolve_memory_backend(config: Any) -> Any:
    ...
    if config is None or not getattr(config.agent, "context_from_memory", False):
        return None
    try:
        import Chottu.tools.storage  # noqa: F401
        from Chottu.core.registry import MemoryRegistry
...
class _LightweightSystem:
    ...
    full ChottuSystem (which picks a random model from Ollama)."""
...
def _make_lightweight_system(
    ...
    """
    try:
        from Chottu.engine.ollama import OllamaEngine
        ...
        try:
            from Chottu.core.events import get_event_bus
            from Chottu.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
...
def _ensure_registries_populated() -> None:
    ...
    from Chottu.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import Chottu.channels  # noqa: F401
    except Exception:
        pass

    try:
        import Chottu.tools  # noqa: F401
    except Exception:
        pass

    # Also try to import browser tools (not included in Chottu.tools.__init__)
    for _browser_mod in ("Chottu.tools.browser", "Chottu.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("Chottu.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("Chottu.tools.")
    ...
    if not any(ToolRegistry.contains(n) for n in _BROWSER_SUB_TOOLS):
        for _browser_mod in (
            "Chottu.tools.browser",
            "Chottu.tools.browser_axtree",
        ):
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from Chottu.core.credentials import TOOL_CREDENTIALS
    from Chottu.core.registry import ChannelRegistry, ToolRegistry
...
def _build_managed_system_prompt(system_prompt: str, app_config: Any) -> str:
    ...
    """
    from Chottu.prompt.builder import SystemPromptBuilder
...
def _replay_history_messages(
    ...
    """
    from Chottu.core.types import Message, Role, ToolCall
...
def _build_deep_research_tools(
    ...
    if not knowledge_db_path:
        from Chottu.core.config import DEFAULT_CONFIG_DIR
    ...
    from Chottu.connectors.retriever import TwoStageRetriever
    from Chottu.connectors.store import KnowledgeStore
    from Chottu.tools.knowledge_search import KnowledgeSearchTool
    from Chottu.tools.knowledge_sql import KnowledgeSQLTool
    from Chottu.tools.scan_chunks import ScanChunksTool
    from Chottu.tools.think import ThinkTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from Chottu.mcp.transport import StdioTransport, StreamableHTTPTransport
    from Chottu.tools.mcp_adapter import MCPToolProvider
...
async def _stream_managed_agent(
    *,
    ...
    from Chottu.core.types import Message, Role
    ...
    if agent_type == "deep_research" and app_state is not None:
        ...
        if dr_tools:
            ...
            async def generate_deep_research():
                ...
                from Chottu.agents.deep_research import DeepResearchAgent
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from Chottu.core.types import ToolCall as MsgToolCall
                ...
                for tc in sorted_tcs:
                    ...
                    try:
                        ...
                        if mcp_adapter is not None:
                            ...
                            from Chottu.core.registry import ToolRegistry
                            from Chottu.tools._stubs import (
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from Chottu.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            from Chottu.server.agent_manager_routes import (
                                _build_deep_research_tools,
                            )

                            tools = _build_deep_research_tools(engine=engine, model="")
                            if tools:
                                from Chottu.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from Chottu.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from Chottu.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from Chottu.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from Chottu.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from Chottu.channels.slack_daemon import (
    ...
    def trigger_learning(agent_id: str):
        if not manager.get_agent(agent_id):
            raise HTTPException(status_code=404, detail="Agent not found")
        from Chottu.core.events import EventType, get_event_bus
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            ...
            config = load_config()
            store = TraceStore(config.traces.db_path or "~/.Chottu/traces.db")
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from Chottu.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from Chottu.core.credentials import get_credential_status

...
User configuration lives at ``~/.Chottu/config.toml``.  ``load_config()``
...
if TYPE_CHECKING:
    # Only used by type-checkers (mypy/pyright) for the ``ChottuConfig.mining``
...
DEFAULT_CONFIG_DIR = Path.home() / ".Chottu"
DEFAULT_CONFIG_PATH = DEFAULT_CONFIG_DIR / "config.toml"


def _ensure_config_dir() -> Path:
    """Ensure the config directory exists with restrictive permissions."""
    from Chottu.security.file_utils import secure_mkdir
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from Chottu.intelligence.model_catalog import BUILTIN_MODELS
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e Chottu[learning-ace]`` once the
    ...
    # most Chottu users have credentials for.
    ...
    # Empty string defaults to ``~/.Chottu/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "Chottu"
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.Chottu/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through Chottu. You are not a cloud service. Respond "
...
class AnalyticsConfig:
    ...
    usage events sent to the Chottu team's PostHog instance to
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.Chottu/whatsapp_auth
    assistant_name: str = "Chottu"
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "Chottu-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.Chottu/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.Chottu/operators"
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.Chottu/SOUL.md"
    memory_path: str = "~/.Chottu/MEMORY.md"
    user_path: str = "~/.Chottu/USER.md"
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.Chottu/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/Chottu/skill-index.git"
    index_dir: str = "~/.Chottu/skill-index/"
    max_depth: int = 5
...
@dataclass
class ChottuConfig:
    """Top-level configuration for Chottu."""
...
# Sections that users may set via ``Chottu config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(ChottuConfig.__dataclass_fields__.keys()) - {
    "hardware",
    "mining",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``ChottuConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import Chottu.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "ChottuConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> ChottuConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``Chottu_CONFIG`` when set,
        otherwise ``~/.Chottu/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = ChottuConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("Chottu_CONFIG"):
        config_path = Path(os.environ["Chottu_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``Chottu init``)
# ---------------------------------------------------------------------------
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.Chottu/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.Chottu/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.Chottu/whatsapp_auth

...
from Chottu.skills.dependency import validate_dependencies
from Chottu.skills.executor import SkillExecutor, SkillResult
from Chottu.skills.loader import discover_skills
from Chottu.skills.tool_adapter import SkillTool
from Chottu.skills.types import SkillManifest
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.Chottu/learning/skills/ if config can't be loaded.
            try:
                ...
                pass
            if overlay_dir is None:
                overlay_dir = Path("~/.Chottu/learning/skills/").expanduser()
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.Chottu.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from Chottu.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("Chottu", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["Chottu"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.Chottu.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("Chottu", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.Chottu/skills/discovered/``).  Returns a list of dicts with
        ...
        if output_dir is None:
            output_dir = Path("~/.Chottu/skills/discovered/").expanduser()
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "Chottu (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        """
        if roots is None:
            roots = [Path("~/.Chottu/skills/").expanduser(), Path("./skills")]
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from Chottu.skills.loader import load_skill_directory

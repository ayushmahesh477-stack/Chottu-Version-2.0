...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.Chottu.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "Chottu_meta" the value is
# stored under manifest.metadata["Chottu"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("Chottu_meta", "platforms"),
    "prerequisites": ("Chottu_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.Chottu.original_frontmatter,
        and merges metadata.Chottu.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize Chottu namespace
        oj_meta = dict(raw_metadata.get("Chottu") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.Chottu.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.Chottu.* into canonical fields (these override
        # top-level mappings since they are explicit Chottu-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.Chottu.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the Chottu metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["Chottu"] = oj_meta

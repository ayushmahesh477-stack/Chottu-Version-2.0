"""Config-driven fluent builder that wires up a ChottuSystem."""
...
from Chottu.core.config import ChottuConfig, load_config
from Chottu.core.events import EventBus, get_event_bus
from Chottu.engine._stubs import InferenceEngine
from Chottu.system.core import ChottuSystem
from Chottu.tools._stubs import BaseTool, ToolExecutor

logger = logging.getLogger(__name__)


class SystemBuilder:
    """Config-driven fluent builder for ChottuSystem."""

    def __init__(
        self,
        config: Optional[ChottuConfig] = None,
        *,
    ...
    def build(self) -> ChottuSystem:
        """Construct a fully wired ChottuSystem."""
        config = self._config
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            try:
                from Chottu.telemetry.energy_monitor import (
            ...
            if energy_monitor is None:
                try:
                    from Chottu.telemetry.gpu_monitor import GpuMonitor
        ...
        from Chottu.security import setup_security
        ...
        if config.agent_manager.enabled:
            try:
                ...
                am_db = config.agent_manager.db_path or str(
                    Path("~/.Chottu/agents.db").expanduser()
                )
        ...
        if speech_enabled:
            try:
                from Chottu.speech._discovery import get_speech_backend
    ...
    def _resolve_engine(self, config: ChottuConfig):
        from Chottu.engine._discovery import get_engine
    ...
    def _resolve_model(self, config: ChottuConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _resolve_memory(self, config):
        try:
            import Chottu.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import Chottu.channels  # noqa: F401 -- trigger registration
            from Chottu.core.registry import ChannelRegistry
            from Chottu.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from Chottu.sandbox.runner import ContainerRunner
    ...
    def _setup_scheduler(self, config, bus):
        ...
        try:
            ...
            from Chottu.scheduler.scheduler import TaskScheduler
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: ChottuConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from Chottu.core.config import DEFAULT_CONFIG_DIR
            from Chottu.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from Chottu.learning.training.lora import LoRATrainingConfig
            from Chottu.traces.store import TraceStore

...
Recipes are the universal composition format for Chottu.  Each recipe
...
_USER_RECIPES_DIR = Path.home() / ".Chottu" / "recipes"
_USER_OPERATORS_DIR = Path.home() / ".Chottu" / "operators"
...
class Recipe:
    ...
    def to_builder_kwargs(self) -> Dict[str, Any]:
        """Convert recipe fields to kwargs for SystemBuilder/Chottu.

        Returns a dict with only the non-None fields, keyed to match
        the SystemBuilder fluent API or Chottu constructor parameters.
        """
        kwargs: Dict[str, Any] = {}
    ...
    def to_eval_suite(
        ...
        """
        from Chottu.recipes.composer import recipe_to_eval_suite
    ...
    def to_operator_manifest(self) -> Any:
        ...
        from Chottu.recipes.composer import recipe_to_operator
...
def discover_recipes(
    ...
    *,
    ...
    3. User ``~/.Chottu/recipes/`` directory
    4. User ``~/.Chottu/operators/`` directory

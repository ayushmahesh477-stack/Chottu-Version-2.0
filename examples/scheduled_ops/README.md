...
This tutorial demonstrates how to use Chottu to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Chottu` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `Chottu scheduler` CLI
...
Register each script as a recurring task with `Chottu scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | Chottu channel send slack
...
See `Chottu channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let Chottu auto-select from what is available.

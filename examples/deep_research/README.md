...
using Chottu. The assistant uses an orchestrator agent loop with web
...
- Chottu installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `Chottu eval`
runner.
...
User query
  |
  v
Chottu SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `Chottu agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `Chottu.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/Chottu/recipes/) -- composable TOML configs that

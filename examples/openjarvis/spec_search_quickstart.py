...
are local fakes, so you can ``python examples/Chottu/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/Chottu/examples/spec-search-quickstart.toml``
via the regular ``Chottu.core.config.load_config`` machinery — you can copy
that TOML to ``~/.Chottu/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    Chottu_HOME=/tmp/Chottu-spec-search-demo \\
...
from Chottu.core.config import SpecSearchLearningConfig
from Chottu.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
from Chottu.learning.spec_search.models import (
    BenchmarkSnapshot,
    FailureCluster,
)
from Chottu.learning.spec_search.multi_session import SpecSearchLoop
from Chottu.learning.spec_search.orchestrator import SpecSearchOrchestrator
from Chottu.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from Chottu.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = Chottu.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.Chottu/config.toml``.
    ...
    #
    # (after copying ``configs/Chottu/examples/spec-search-quickstart.toml``
    # to ``~/.Chottu/config.toml``)
    ...
    home = Path(
        os.environ.get("Chottu_HOME")
        or tempfile.mkdtemp(prefix="Chottu-spec-search-")
    )
    print(f"Chottu_HOME = {home}")
    ...
    with patch(
        "Chottu.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:

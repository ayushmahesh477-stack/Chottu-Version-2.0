...
A set of developer-focused scripts that use Chottu tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Chottu` instance with the `native_react` (ReAct) agent
...
1. **Install Chottu** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/Chottu/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/Chottu/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run Chottu eval list` or
inspect `src/Chottu/tools/` for the full registry.

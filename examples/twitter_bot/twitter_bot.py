#!/usr/bin/env python3
"""Chottu Twitter Bot — @ChottuAI reactive mention handler.
...
DEMO_TWEETS = [
    {
        ...
        "author": "alice_dev",
        "text": "@ChottuAI how do I add a new channel integration?",
    },
    {
        "id": "1000000000000000002",
        "author": "bob_user",
        "text": (
            "@ChottuAI bug: the memory_search tool crashes "
    ...
    },
    {
        ...
        "author": "carol_eng",
        "text": "@ChottuAI it would be great to have a built-in scheduler UI",
    },
    {
        "id": "1000000000000000004",
        "author": "dave_fan",
        "text": "@ChottuAI just discovered this project, absolutely love it!",
    },
    {
        "id": "1000000000000000005",
        "author": "spambot99",
        "text": "@ChottuAI BUY CRYPTO NOW 🚀🚀🚀 LINK IN BIO",
    },
]
...
# nomic-embed-text on the Chottu fixture corpus, relevant queries
...
def _build_question_grounded_prompt(
    ...
    return (
        "You are @ChottuAI. Someone asked a question. We retrieved "
...
def _build_question_deferral_prompt(author: str, tweet_id: str, text: str) -> str:
    ...
    """
    return (
        "You are @ChottuAI. Someone asked a question, but our docs "
...
def _build_bug_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @ChottuAI. Someone reported a bug.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
        "1. call http_request to create a github issue:\n"
        "   url: https://api.github.com/repos/Chottu/Chottu/issues\n"
        "   method: POST\n"
...
def _build_feature_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @ChottuAI. Someone requested a feature.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
        "1. call http_request to create a github issue:\n"
        "   url: https://api.github.com/repos/Chottu/Chottu/issues\n"
...
def _build_praise_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @ChottuAI. Someone said something nice.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
...
_INJECTION_PROMPT = (
    "Classify this tweet mentioning @ChottuAI as SAFE or MALICIOUS. "
...
def _classify_mention(text: str, Chottu) -> str:
    ...
    output.
    """
    llm_label = _classify_mention_llm(text, Chottu)
...
def _run_demo(model: str, engine_key: str) -> None:
    ...
    try:
        ...
        click.echo(
            "Error: Chottu is not installed. "
            "Install it with:  uv sync --extra dev",
            err=True,
        )
        sys.exit(1)

    click.echo("Chottu Twitter Bot — Demo Mode (reactive only)")
    click.echo(f"Model: {model}  |  Engine: {engine_key}")
    click.echo("=" * 60)

    try:
        j = Chottu(model=model, engine_key=engine_key)
    except Exception as exc:
        click.echo(
            f"Error: could not initialize Chottu — {exc}\n\n"
    ...
    try:
        for idx, tweet in enumerate(DEMO_TWEETS, 1):
            mention_type = _classify_mention(tweet["text"], Chottu=j)
            click.echo(
...
_SINCE_ID_STATE_PATH = Path.home() / ".Chottu" / "twitter_since_id.txt"
...
def _seed_since_id_to_newest(channel) -> Optional[str]:
    ...
    1. **Persisted state from a prior run** (``~/.Chottu/twitter_since_id.txt``).
...
def _run_live(
    ...
    *,
    ...
    try:
        from Chottu import Chottu
        from Chottu.channels._stubs import ChannelStatus
        from Chottu.channels.twitter_channel import TwitterChannel
        from Chottu.core.types import ToolResult
    except ImportError:
        click.echo(
            "Error: Chottu is not installed. "
            "Install it with:  uv sync --extra dev",
    ...
    click.echo(f"Chottu Twitter Bot — {mode_label} Mode")
    click.echo(f"Model: {model}  |  Engine: {engine_key}")
    click.echo("=" * 60)
    ...
    try:
        ...
        click.echo(f"Error: could not initialize Chottu — {exc}", err=True)
        sys.exit(1)
    ...
    http_restore = None
    if dry_run:
        from Chottu.tools.http_request import HttpRequestTool
        ...
        def _dry_http_execute(self, **params):  # noqa: ANN001
            ...
            return ToolResult(
                ...
                content=(
                    '{"number": 999, "html_url": '
                    '"https://github.com/Chottu/Chottu/issues/999"}'
                ),
            )
    ...
    click.echo(f"\n{mode_hint}")
    click.echo("Waiting for @ChottuAI mentions (poll every 60s). Ctrl+C to stop.\n")
    ...
    def _handle_mention(msg):  # noqa: ANN001
        ...
        if _detect_injection(msg.content, Chottu=j) == "MALICIOUS":
            click.echo(
        ...
        mention_type = _classify_mention(msg.content, Chottu=j)
...
def main(
    ...
    """Chottu Twitter bot — reactive @ChottuAI mention handler.

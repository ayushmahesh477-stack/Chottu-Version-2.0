...
Index a directory of documents into Chottu memory and answer questions
with context-augmented retrieval and citations.

## Requirements

- Chottu installed (`git clone https://github.com/Chottu/Chottu.git && cd Chottu && uv sync` or `uv sync --extra dev`)
...
1. **Index** -- Uses `Chottu.memory.index()` to chunk the documents at
...
Chottu SDK. Adjust `--chunk-size` and `--top-k` to tune the

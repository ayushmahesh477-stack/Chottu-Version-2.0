...
Chottu orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting Chottu to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- Chottu installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the Chottu channel docs).
...
Chottu supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/Chottu/config.toml` for the full list of channel and agent
options.
...
this via the Chottu scheduler:

```bash
Chottu scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/Chottu/recipes/data/operators/` for examples.

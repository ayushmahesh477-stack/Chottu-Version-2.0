...
def _run_channel(channel: str, model: str, engine_key: str) -> None:
    ...
    help_text = setup_help.get(
        ...
        f"Run: Chottu channel list   to see available channels.\n",
    )
    ...
    click.echo("  from Chottu import Chottu")
    click.echo(f'  j = Chottu(model="{model}", engine_key="{engine_key}")')
    click.echo("  # Listen for incoming messages on the channel")
    click.echo(f'  # See: Chottu channel status  (to verify "{channel}" is connected)')

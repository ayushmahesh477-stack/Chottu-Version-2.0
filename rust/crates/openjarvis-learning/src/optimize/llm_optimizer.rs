...
//!
//! Rust translation of `src/Chottu/learning/optimize/llm_optimizer.py`.
//!
...
/// Uses an LLM to propose optimal Chottu configurations.
///
...
impl LLMOptimizer {
    ...
    pub fn analyze_trial(
        ...
        if let Some(ref backend) = self.backend {
            let prompt = format!(
                "Analyze this Chottu evaluation result.\n\n\
                 ## Configuration\n{}\n\n\
                 ## Results\n- accuracy: {accuracy:.4}\n\
    ...
    fn build_initial_prompt(&self) -> String {
        ...
        lines.push("You are optimizing an Chottu AI system configuration.".into());

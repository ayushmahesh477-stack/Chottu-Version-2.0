...
use Chottu_core::{ChottuError, Trace};
...
impl TraceStore {
    pub fn new(db_path: &Path) -> Result<Self, ChottuError> {
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                ChottuError::Io(std::io::Error::other(e))
            })?;
        }

        let conn = Connection::open(db_path).map_err(|e| {
            ChottuError::Io(std::io::Error::other(e.to_string()))
        })?;
    ...
    pub fn in_memory() -> Result<Self, ChottuError> {
        Self::new(Path::new(":memory:"))
    }

    pub fn save(&self, trace: &Trace) -> Result<(), ChottuError> {
    ...
    pub fn get(&self, trace_id: &str) -> Result<Option<Trace>, ChottuError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
            ...
            .map_err(|e| {
                ChottuError::Io(std::io::Error::other(
                    e.to_string(),
                ))
            })?;
    ...
    pub fn count(&self) -> Result<usize, ChottuError> {
        let conn = self.conn.lock();
        let count: i64 = conn

...
use Chottu_core::config::ChottuConfig;
...
pub fn discover_engines(config: &ChottuConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, ChottuError> {
    ...
    match key.as_str() {
        ...
        other => Err(ChottuError::Engine(
            Chottu_core::error::EngineError::ModelNotFound(format!(
...
mod tests {
    ...
    fn test_get_engine_static_ollama() {
        let config = ChottuConfig::default();

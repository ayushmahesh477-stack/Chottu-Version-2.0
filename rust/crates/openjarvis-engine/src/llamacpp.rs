...
use Chottu_core::error::{EngineError, ChottuError};
use Chottu_core::{GenerateResult, Message, Usage};
...
impl InferenceEngine for LlamaCppEngine {
    ...
    fn generate(
        ...
        max_tokens: i64,
        _extra: Option<&Value>,
    ) -> Result<GenerateResult, ChottuError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|e| {
                ChottuError::Engine(EngineError::Connection(format!(
        ...
        if !resp.status().is_success() {
            ...
            return Err(ChottuError::Engine(EngineError::Http(format!(
                "llama.cpp returned {status}: {body}"
            ))));
        }

        let data: Value = resp.json().map_err(|e| {
            ChottuError::Engine(EngineError::Deserialization(e.to_string()))
        })?;
    ...
    async fn stream(
        ...
        max_tokens: i64,
        _extra: Option<&Value>,
    ) -> Result<TokenStream, ChottuError> {
        let prompt = Self::messages_to_prompt(messages);
        let payload = serde_json::json!({
            "prompt": prompt,
            "n_predict": max_tokens,
        ...
        let async_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                ChottuError::Engine(EngineError::Connection(e.to_string()))
            })?;
        ...
        let token_stream = byte_stream.filter_map(|chunk_result| async {
            match chunk_result {
                ...
                Err(e) => Some(Err(ChottuError::Engine(EngineError::Streaming(
                    e.to_string(),
                )))),
            }
        });
    ...
    fn list_models(&self) -> Result<Vec<String>, ChottuError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|_| {
                ChottuError::Engine(EngineError::Connection(

...
//!
//! Rust translation of `src/Chottu/core/config.py`.
...
/// Default config directory: `~/.Chottu/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".Chottu")
}

/// Default config file path: `~/.Chottu/config.toml`
...
fn default_sandbox_image() -> String { "Chottu-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.Chottu/operators".into() }
...
// ---------------------------------------------------------------------------
// Top-level ChottuConfig
// ---------------------------------------------------------------------------

/// Top-level configuration for Chottu.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct ChottuConfig {
    #[serde(skip)]
...
pub fn load_config(path: Option<&Path>) -> Result<ChottuConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: ChottuConfig = toml::from_str(&content)?;
        ...
        let mut cfg = ChottuConfig::default();
        cfg.engine.default = recommended_engine;
        cfg
    };
...
mode = "block"
"#;
        let cfg: ChottuConfig = toml::from_str(toml_str).unwrap();

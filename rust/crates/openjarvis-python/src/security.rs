...
pub struct PySecretScanner {
    inner: Chottu_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: Chottu_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: Chottu_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: Chottu_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: Chottu_security::GuardrailsEngine<Chottu_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => Chottu_security::RedactionMode::Redact,
            "block" => Chottu_security::RedactionMode::Block,
            _ => Chottu_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: Chottu_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<Chottu_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = Chottu_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: Chottu_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: Chottu_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: Chottu_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: Chottu_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: Chottu_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: Chottu_security::RateLimiter::new(
                Chottu_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
pub struct PyTaintSet {
    inner: Chottu_security::TaintSet,
}
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: Chottu_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => Chottu_security::TaintLabel::Pii,
            "secret" => Chottu_security::TaintLabel::Secret,
            "user_private" => Chottu_security::TaintLabel::UserPrivate,
            "external" => Chottu_security::TaintLabel::External,
            _ => Chottu_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &Chottu_security::TaintSet::from_labels(&[taint_label]),
        );
    }

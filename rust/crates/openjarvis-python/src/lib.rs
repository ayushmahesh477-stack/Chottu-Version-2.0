//! PyO3 bridge — exposes ~50 Rust classes to Python via `Chottu_rust`.
...
fn load_config(path: Option<&str>) -> PyResult<core::PyConfig> {
    ...
    let config = Chottu_core::load_config(p)
...
fn detect_hardware() -> PyResult<String> {
    let hw = Chottu_core::hardware::detect_hardware();
    Ok(serde_json::to_string(&hw).unwrap_or_default())
}

#[pyfunction]
fn check_ssrf(url: &str) -> Option<String> {
    Chottu_security::check_ssrf(url)
}

#[pyfunction]
fn is_sensitive_file(path: &str) -> bool {
    Chottu_security::is_sensitive_file(std::path::Path::new(path))
}

#[pyfunction]
fn register_builtin_models() {
    Chottu_core::model_catalog::register_builtin_models();
}

#[pyfunction]
fn classify_query(query: &str) -> &'static str {
    Chottu_learning::classify_query(query)
}

#[pymodule]
fn Chottu_rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // --- Core types ---

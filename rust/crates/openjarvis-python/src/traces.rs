...
pub struct PyTraceStore {
    pub inner: Arc<Chottu_traces::TraceStore>,
}
...
impl PyTraceStore {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        let inner = match path {
            Some(p) => Chottu_traces::TraceStore::new(std::path::Path::new(p)),
            None => Chottu_traces::TraceStore::in_memory(),
        }
...
pub struct PyTraceCollector {
    inner: Chottu_traces::TraceCollector,
}
...
impl PyTraceCollector {
    ...
    fn new(store: &PyTraceStore) -> Self {
        Self {
            inner: Chottu_traces::TraceCollector::new(Arc::clone(&store.inner)),
        }
    }
...
pub struct PyTraceAnalyzer {
    store: Arc<Chottu_traces::TraceStore>,
}
...
impl PyTraceAnalyzer {
    ...
    fn stats(&self) -> PyResult<String> {
        let analyzer = Chottu_traces::TraceAnalyzer::new(&self.store);
        let stats = analyzer
            .overall_stats()
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;

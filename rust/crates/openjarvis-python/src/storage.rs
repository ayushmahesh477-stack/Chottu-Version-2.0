...
use Chottu_tools::storage::MemoryBackend;
use pyo3::prelude::*;

#[pyclass(name = "SQLiteMemory")]
pub struct PySQLiteMemory {
    inner: Chottu_tools::storage::SQLiteMemory,
}
...
impl PySQLiteMemory {
    ...
    fn new(path: &str) -> PyResult<Self> {
        let inner = Chottu_tools::storage::SQLiteMemory::new(std::path::Path::new(path))
...
pub struct PyBM25Memory {
    inner: Chottu_tools::storage::BM25Memory,
}
...
impl PyBM25Memory {
    ...
    fn new(k1: f64, b: f64) -> Self {
        Self {
            inner: Chottu_tools::storage::BM25Memory::new(k1, b),
        }
    }
...
pub struct PyFAISSMemory {
    inner: Chottu_tools::storage::FAISSMemory,
}
...
impl PyFAISSMemory {
    ...
    fn new(path: &str, dim: usize) -> PyResult<Self> {
        let inner = Chottu_tools::storage::FAISSMemory::new(std::path::Path::new(path), dim)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self { inner })
    }
...
pub struct PyColBERTMemory {
    inner: Chottu_tools::storage::ColBERTMemory,
}
...
impl PyColBERTMemory {
    ...
    fn new(path: &str, token_dim: usize) -> PyResult<Self> {
        let inner =
            Chottu_tools::storage::ColBERTMemory::new(std::path::Path::new(path), token_dim)
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self { inner })
    }
...
pub struct PyHybridMemory {
    inner: Chottu_tools::storage::HybridMemory,
}
    ...
    fn new(backend_keys: Vec<String>) -> PyResult<Self> {
        ...
        for key in &backend_keys {
            let backend: Box<dyn MemoryBackend> = match key.as_str() {
                "sqlite" => {
                    let m = Chottu_tools::storage::SQLiteMemory::new(
                ...
                "bm25" => Box::new(Chottu_tools::storage::BM25Memory::default()),
                "faiss" => {
                    let m = Chottu_tools::storage::FAISSMemory::in_memory().map_err(|e| {
                        PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string())
                    })?;
                    Box::new(m)
                }
                "colbert" => {
                    let m =
                        Chottu_tools::storage::ColBERTMemory::in_memory().map_err(|e| {
                            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string())
                        })?;
                    Box::new(m)
                }
        ...
        Ok(Self {
            inner: Chottu_tools::storage::HybridMemory::new(backends),
        })
    }
...
pub struct PyKnowledgeGraphMemory {
    inner: Chottu_tools::storage::KnowledgeGraphMemory,
}
...
impl PyKnowledgeGraphMemory {
    ...
    #[pyo3(signature = (path=":memory:"))]
    fn new(path: &str) -> PyResult<Self> {
        let inner = Chottu_tools::storage::KnowledgeGraphMemory::new(std::path::Path::new(path))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self { inner })
    }

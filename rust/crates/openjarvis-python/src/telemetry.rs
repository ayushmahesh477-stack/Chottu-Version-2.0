...
pub struct PyTelemetryStore {
    pub inner: Arc<Chottu_telemetry::TelemetryStore>,
}
...
impl PyTelemetryStore {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        let inner = match path {
            Some(p) => Chottu_telemetry::TelemetryStore::new(std::path::Path::new(p)),
            None => Chottu_telemetry::TelemetryStore::in_memory(),
        }
...
pub struct PyTelemetryAggregator {
    store: Arc<Chottu_telemetry::TelemetryStore>,
}
...
impl PyTelemetryAggregator {
    ...
    fn stats(&self) -> PyResult<String> {
        let stats = Chottu_telemetry::TelemetryAggregator::stats(&self.store)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
...
pub struct PyInstrumentedEngine {
    inner: Chottu_telemetry::InstrumentedEngine<Chottu_engine::Engine>,
}
...
impl PyInstrumentedEngine {
    ...
    fn new(engine_key: &str, host: &str, store_path: Option<&str>, agent_name: &str) -> PyResult<Self> {
        ...
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?);
        Ok(Self {
            inner: Chottu_telemetry::InstrumentedEngine::new(
                engine,
                store,
...
pub struct PyTelemetrySessionCore {
    inner: Chottu_telemetry::session::TelemetrySessionCore,
}
...
impl PyTelemetrySessionCore {
    ...
    fn new(capacity: usize, sampling_interval_ms: u64) -> Self {
        Self {
            inner: Chottu_telemetry::session::TelemetrySessionCore::new(
                capacity,
    ...
    fn add_sample(&self, sample: &PyTelemetrySample) {
        let s = Chottu_telemetry::session::TelemetrySample {
...
impl PyItlStats {
    ...
    fn compute(token_timestamps_ms: Vec<f64>) -> PyResult<pyo3::Py<pyo3::types::PyDict>> {
        let stats = Chottu_telemetry::itl::compute_itl_stats(&token_timestamps_ms);
...
impl PyFlopsEstimator {
    ...
    fn estimate_flops(model: &str, input_tokens: u64, output_tokens: u64) -> (f64, f64) {
        Chottu_telemetry::flops::estimate_flops(model, input_tokens, output_tokens)
    }

    #[staticmethod]
    #[pyo3(signature = (flops, duration_s, gpu_name, num_gpus=1))]
    fn compute_mfu(flops: f64, duration_s: f64, gpu_name: &str, num_gpus: u32) -> f64 {
        Chottu_telemetry::flops::compute_mfu(flops, duration_s, gpu_name, num_gpus)
    }
}
...
impl PyPhaseMetrics {
    ...
    fn compute(
        ...
        let rust_samples: Vec<Chottu_telemetry::session::TelemetrySample> = samples
            .iter()
            .map(|s| Chottu_telemetry::session::TelemetrySample {
                timestamp_ns: s.timestamp_ns,
                gpu_power_w: s.gpu_power_w,
                cpu_power_w: s.cpu_power_w,
                gpu_energy_j: s.gpu_energy_j,
            ...
            .collect();
        let metrics =
            Chottu_telemetry::phase::compute_phase_metrics(&rust_samples, start_ns, end_ns, tokens);
        pyo3::Python::with_gil(|py| {
            let dict = pyo3::types::PyDict::new(py);
    ...
    fn split_at_ttft(
        ...
        let rust_samples: Vec<Chottu_telemetry::session::TelemetrySample> = samples
            ...
            .collect();
        let (prefill, decode) = Chottu_telemetry::phase::split_at_ttft(
            &rust_samples,
            start_ns,
            ttft_ns,
            end_ns,

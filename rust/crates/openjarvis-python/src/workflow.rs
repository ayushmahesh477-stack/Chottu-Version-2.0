...
pub struct PyWorkflowGraph {
    inner: Chottu_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Chottu_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => Chottu_workflow::NodeType::Tool,
            "condition" => Chottu_workflow::NodeType::Condition,
            "parallel" => Chottu_workflow::NodeType::Parallel,
            "loop" => Chottu_workflow::NodeType::Loop,
            "transform" => Chottu_workflow::NodeType::Transform,
            _ => Chottu_workflow::NodeType::Agent,
        };
        let node = Chottu_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = Chottu_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<Chottu_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(Chottu_workflow::WorkflowBuilder::new(name)),
        }
    }

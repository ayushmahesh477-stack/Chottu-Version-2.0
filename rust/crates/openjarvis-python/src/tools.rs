...
use Chottu_tools::traits::BaseTool;
use pyo3::prelude::*;
use std::sync::Arc;

#[pyclass(name = "ToolExecutor")]
pub struct PyToolExecutor {
    pub inner: Arc<Chottu_tools::ToolExecutor>,
}
...
impl PyToolExecutor {
    ...
    fn new() -> Self {
        Self {
            inner: Arc::new(Chottu_tools::ToolExecutor::new(None, None)),
        }
    }
...
impl PyCalculatorTool {
    ...
    fn execute(&self, expression: &str) -> PyResult<String> {
        let tool = Chottu_tools::builtin::calculator::CalculatorTool;
...
impl PyThinkTool {
    ...
    fn execute(&self, thought: &str) -> PyResult<String> {
        let tool = Chottu_tools::builtin::think::ThinkTool;
...
impl PyFileReadTool {
    ...
    fn execute(&self, path: &str) -> PyResult<String> {
        let tool = Chottu_tools::builtin::file_tools::FileReadTool;
...
impl PyFileWriteTool {
    ...
    fn execute(&self, path: &str, content: &str) -> PyResult<String> {
        let tool = Chottu_tools::builtin::file_tools::FileWriteTool;
...
impl PyShellExecTool {
    ...
    fn execute(&self, command: &str, cwd: Option<&str>) -> PyResult<String> {
        let tool = Chottu_tools::builtin::shell::ShellExecTool;
...
impl PyHttpRequestTool {
    ...
    fn execute(&self, url: &str, method: &str, body: Option<&str>) -> PyResult<String> {
        let tool = Chottu_tools::builtin::http_tools::HttpRequestTool;
...
impl PyGitStatusTool {
    ...
    fn execute(&self, cwd: Option<&str>) -> PyResult<String> {
        let tool = Chottu_tools::builtin::git_tools::GitStatusTool;
...
impl PyGitDiffTool {
    ...
    #[pyo3(signature = (cwd=None))]
    fn execute(&self, cwd: Option<&str>) -> PyResult<String> {
        let tool = Chottu_tools::builtin::git_tools::GitDiffTool;
        let mut params = serde_json::json!({});
        if let Some(cwd) = cwd {
            params["cwd"] = serde_json::Value::String(cwd.to_string());
        }
...
impl PyGitLogTool {
    ...
    fn execute(&self, cwd: Option<&str>, count: Option<u32>) -> PyResult<String> {
        let tool = Chottu_tools::builtin::git_tools::GitLogTool;
        let mut params = serde_json::json!({});
        if let Some(cwd) = cwd {
            params["cwd"] = serde_json::Value::String(cwd.to_string());
        }

...
use Chottu_core::error::{ChottuError, ToolError};
use Chottu_core::{EventBus, EventType, ToolResult};
use Chottu_security::capabilities::CapabilityPolicy;
use Chottu_security::taint::{TaintSet, check_taint};
...
impl ToolExecutor {
    ...
    pub fn execute(
    ...
    ) -> Result<ToolResult, ChottuError> {
        let tool = self.tools.get(tool_name).ok_or_else(|| {
            ChottuError::Tool(ToolError::NotFound(tool_name.to_string()))
        })?;
        ...
        if let (Some(policy), Some(aid)) = (&self.capability_policy, agent_id) {
            ...
            for cap in &spec.required_capabilities {
                if !policy.check(aid, cap, "") {
                    return Err(ChottuError::Tool(ToolError::CapabilityDenied(
                        aid.to_string(),
        ...
        if let Some(taint_set) = taint {
            if let Some(violation) = check_taint(tool_name, taint_set) {
                return Err(ChottuError::Tool(ToolError::TaintViolation(
                    tool_name.to_string(),
                    violation,
                )));
            }
        ...
        if elapsed > timeout {
            ...
            return Err(ChottuError::Tool(ToolError::Timeout(
                timeout.as_secs_f64(),
                tool_name.to_string(),
            )));
        }
...
mod tests {
    ...
    fn test_executor_tool_not_found() {
        ...
        let err = exec
            ...
            .unwrap_err();
        assert!(matches!(err, ChottuError::Tool(ToolError::NotFound(_))));
    }
}

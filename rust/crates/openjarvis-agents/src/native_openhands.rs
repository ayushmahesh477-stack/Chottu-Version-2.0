...
use Chottu_core::{AgentContext, AgentResult, ChottuError, ToolResult};
use Chottu_tools::executor::ToolExecutor;
use regex::Regex;
    ...
    async fn run(
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        Chottu_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        Chottu_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    ChottuError::Agent(Chottu_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<Chottu_engine::Engine>>;

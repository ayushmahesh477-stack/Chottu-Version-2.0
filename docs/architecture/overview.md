...
description: The five-primitive architecture behind Chottu — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
Chottu is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![Chottu Architecture](../assets/Chottu_Architecture.png)
...
The sandbox module (`Chottu.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in Chottu use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/Chottu/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           ChottuConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              Chottu ask -- query the assistant
        serve.py            Chottu serve -- start API server

    sdk.py              Chottu class -- high-level Python SDK

...
This page traces the end-to-end journey of a user query through the Chottu system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: Chottu ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: ChottuConfig (hardware, engine defaults)
...
Chottu supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
Chottu ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.Chottu/config.toml
```
...
- Returns a `ChottuConfig` dataclass with all settings
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.Chottu/telemetry.db`.
...
The `Chottu` class in `sdk.py` provides the same query flow through a Python API:

```python
from Chottu import Chottu

j = Chottu(model="qwen3:8b", engine_key="ollama")

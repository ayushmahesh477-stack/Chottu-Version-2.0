...
```python
from Chottu.agents.orchestrator import OrchestratorAgent
from Chottu.tools.calculator import CalculatorTool
...
!!! note "Backward compatibility"
    The old `from Chottu.agents.react import ReActAgent` import path still works via a backward-compat shim. The registry alias `"react"` also maps to `NativeReActAgent`.

```python
from Chottu.agents.native_react import NativeReActAgent
...
```python
from Chottu.agents.native_openhands import NativeOpenHandsAgent
...
```python
from Chottu.agents.rlm import RLMAgent
...
```python
from Chottu.agents.openhands import OpenHandsAgent
...
Wraps the `@anthropic-ai/claude-code` SDK via a bundled Node.js subprocess bridge. Unlike every other agent, inference is handled entirely by the Claude Agent SDK -- the Chottu inference engine is not used. This makes `ClaudeCodeAgent` a true external agent, similar in spirit to `OpenHandsAgent` but implemented via subprocess rather than an importable Python SDK.
...
1. On first call, copies the bundled `claude_code_runner/` to `~/.Chottu/claude_code_runner/` and runs `npm install --production` if `node_modules` is absent
...
4. Reads stdout and extracts the JSON payload between `---Chottu_OUTPUT_START---` and `---Chottu_OUTPUT_END---` sentinels
...
- `cleanup_orphans()` removes any stale containers labelled `Chottu-sandbox=true`
...
from Chottu.agents._stubs import BaseAgent

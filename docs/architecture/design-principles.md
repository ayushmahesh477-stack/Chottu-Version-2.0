...
Chottu follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in Chottu is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
Chottu is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Chottu(engine_key="ollama")  # Local Ollama server
...
Chottu **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `Chottu init` and used as the default engine:

```bash
Chottu init --force
# Detects hardware, writes ~/.Chottu/config.toml with:
# [engine]
...
```bash
Chottu telemetry stats          # Aggregated statistics
Chottu telemetry export --json  # Export all records
```
...
Chottu provides a **clean Python API** through the `Chottu` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`Chottu[server]`, `Chottu[memory-colbert]`, etc.)
...
The API server (`Chottu serve`) implements the **OpenAI chat completions API format**, making Chottu a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to Chottu:
...
Chottu requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of Chottu.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `Chottu[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `Chottu[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `Chottu[memory-bm25]` | `rank-bm25` |
| API server | `Chottu[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `Chottu[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `Chottu[inference-vllm]` | `vllm` |
| PDF ingestion | `Chottu[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `Chottu[channel-whatsapp-baileys]` | Node.js 22+ |

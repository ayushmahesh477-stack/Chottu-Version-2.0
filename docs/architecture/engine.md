...
The Engine primitive provides the **inference runtime** -- the layer that connects Chottu to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
Chottu automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from Chottu.engine import discover_engines
...
```python
from Chottu.engine import discover_engines, discover_models
...
Engine hosts and defaults are configured in `~/.Chottu/config.toml` using **nested per-engine sub-sections**:
...
```python
from Chottu.engine._base import messages_to_dicts
...
```python
from Chottu.engine import EngineConnectionError

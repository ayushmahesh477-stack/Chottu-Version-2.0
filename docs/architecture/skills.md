...
2. **Tolerant pass** — maps non-spec vendor fields to canonical locations via `FIELD_MAPPING` table. Unmapped fields are logged and preserved in `metadata.Chottu.original_frontmatter`
...
Sidecar storage for optimization output at `~/.Chottu/learning/skills/<name>/optimized.toml`:
...
SkillManager.get_few_shot_examples()
  → system._skill_few_shot_examples (stashed on ChottuSystem)

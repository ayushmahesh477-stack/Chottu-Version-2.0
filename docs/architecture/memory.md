...
- **Persistence:** Data persists across restarts in `~/.Chottu/memory.db`
...
from Chottu.tools.storage.faiss_backend import FAISSMemory
from Chottu.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from Chottu.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `Chottu.tools.storage.*`.
...
```python
from Chottu.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from Chottu.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from Chottu.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.Chottu/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:

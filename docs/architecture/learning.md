...
!!! note "Backward compatibility"
    The canonical locations are now `Chottu.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `Chottu.core.types` (for `RoutingContext`). The old `Chottu.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `Chottu.learning._stubs`.
...
```bash
Chottu ask --router learned "What is the capital of France?"
```
...
```python
from Chottu.learning.router import HeuristicRouter, build_routing_context
...
```python
from Chottu.learning.router import build_routing_context
...
```python
from Chottu.learning.trace_policy import TraceDrivenPolicy
from Chottu.traces.analyzer import TraceAnalyzer
...
```python
from Chottu.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from Chottu.learning.sft_policy import SFTPolicy
```
...
```python
from Chottu.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from Chottu.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from Chottu.learning.heuristic_reward import HeuristicRewardFunction
...
over Chottu's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `Chottu-learning`

---
title: Configuration
description: Complete reference for Chottu configuration
---

# Configuration

Chottu uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
Chottu creates the `~/.Chottu/` directory and populates it with a default config when you run `Chottu init`.
...
3. Writes `~/.Chottu/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `Chottu init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, Chottu automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, Chottu uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
Chottu ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.Chottu/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `Chottu serve`.
...
```bash
Chottu serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.Chottu/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.Chottu/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.Chottu/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.Chottu/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `Chottu init`, Chottu probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.Chottu/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure Chottu entirely from Python without a TOML file:

```python
from Chottu import Chottu
from Chottu.core.config import (
    AgentConfig,
    EngineConfig,
    IntelligenceConfig,
    ChottuConfig,
...
```python
j = Chottu(config_path="/path/to/my-config.toml")
```
...
Chottu respects the following environment variables:

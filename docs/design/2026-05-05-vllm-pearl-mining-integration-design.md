...
### In scope (v1)
- New `Chottu.mining` subsystem with `MiningProvider` ABC, `MinerRegistry`, `MiningCapabilities` / `MiningConfig` / `MiningStats` dataclasses
- `vllm-pearl` provider implementation: orchestrate Pearl's published `vllm-miner` Docker container
- `[mining]` TOML section in OJ config; `MiningConfig` field in `ChottuConfig`
- New CLI namespace: `Chottu mine init|start|stop|status|doctor|attach|logs`
- Runtime sidecar at `~/.Chottu/runtime/mining.json` for engine ↔ mining handoff
...
| Engine attachment | Runtime sidecar JSON at `~/.Chottu/runtime/mining.json` | Existing vLLM engine class stays untouched. Sidecar is the single source of truth tying mining lifecycle to engine resolution. Inspectable via `cat`. |
...
src/Chottu/cli/
    mine_cmd.py          # Chottu mine init|start|stop|status|doctor|attach|logs
...
`MinerRegistry` added to `src/Chottu/core/registry.py` as a peer to `EngineRegistry`, `AgentRegistry`, etc. `tests/conftest.py`'s autouse `_clean_registries` fixture is updated to include `MinerRegistry.clear()`.
...
[mining.extra]
docker_image_tag         = "Chottu/pearl-miner:<pinned-ref>"
...
`~/.Chottu/runtime/mining.json` (created on `mine start`, removed on `mine stop`):
...
1. `Chottu mine start` → `MinerRegistry.get("vllm-pearl").start(config)`.
...
4. `Chottu ask` and the SDK route to that endpoint transparently. The user's normal inference is the mining work.
...
Power users running their own Pearl container skip `Chottu mine start` and write the sidecar themselves via `Chottu mine attach --vllm-endpoint=... --gateway-url=...`. Decouples lifecycle from wiring.
...
| Command | Purpose |
|---|---|
| `Chottu mine init` | Interactive: hardware/Docker checks, prompt for wallet + pearld credentials, write `[mining]`, pull/build image. Does NOT start mining. Pre-checks `>=200 GB` free disk. |
| `Chottu mine start` | Launch container via the registered provider, write sidecar, print endpoint info. Idempotent if running. |
| `Chottu mine stop` | Stop container, remove sidecar. Idempotent if not running. |
| `Chottu mine status` | Read sidecar + query gateway `:8339/metrics`. Print `MiningStats`. |
| `Chottu mine doctor` | Capability matrix; every check ✓/✗ with reason. Works in any state. |
| `Chottu mine attach` | Manual mode: write sidecar without launching. |
| `Chottu mine logs [-f]` | Tail container logs through Docker SDK. |
...
Image
  Chottu/pearl-miner:<ref>   present (built 2026-04-30)   ✓
Pearl node
...
- Existing `com.Chottu.gateway` daemon is unchanged.
...
- launchd/systemd installation surface (`Chottu daemon install`) untouched.

### 6.5 Concurrency

POSIX `flock` on `~/.Chottu/runtime/mining.lock` prevents racing `mine start` invocations.

### 6.6 `Chottu ask` UX hint

When `[mining]` is configured but no sidecar exists, `cli/hints.py` emits one line: `"mining configured but not running — start it with \`Chottu mine start\`"`. One-line UX nudge, no new infrastructure.
...
| **Build-from-pin** | OJ git-clones Pearl at a pinned ref into `~/.Chottu/cache/pearl/`, then `docker buildx build` | v1 default (Pearl publishes nothing today) |
...
- **`auto_remove=False`** so a crashed container stays around for `Chottu mine logs` post-mortem.
...
- `Chottu mine init` triggers `ensure_image()`, streams build/pull output through the CLI with a clear time estimate (`"Building Pearl miner image — first run takes ~45 min on a fast machine"`).
- `Chottu mine doctor` reports `image: present (tag, age, sha)` or `image: missing (run mine init)`.
- `Chottu mine prune` (v1.x) cleans old `Chottu/pearl-miner:*` tags. Manual `docker image rm` works in v1.
...
- **v1: on-demand only.** `Chottu mine status` makes one HTTP GET per call (~10 ms). No background polling.
...
- `Chottu telemetry stats --mining` (v1.x) joins to the latest `MiningStats` snapshot and reports `tokens / share`, `joules / share`, `est. PRL / kWh`.
...
**1. `submit_target` parsed into a tagged union; only one variant works.** `SoloTarget` accepted at runtime in v1; `PoolTarget` raises `NotImplementedError("pool support is v2 — track Chottu#XYZ")`. Reachable only by users who edit their config to opt in.
...
| Concurrent `mine start` | POSIX `flock` on `~/.Chottu/runtime/mining.lock`; second invocation errors clearly | clear message |

...
**Goal:** Implement Spec A — the v1 vLLM-Pearl mining integration — as a new `Chottu.mining` subsystem that lets H100/H200 owners running vLLM solo-mine the Pearl PoUW chain with their normal LLM inference.

**Architecture:** New top-level subsystem `mining/` mirrors OJ's existing primitive pattern (peer to `engine/`, `agents/`). A `MiningProvider` ABC + `MinerRegistry` enables future hardware/engine paths (Apple Silicon, AMD, Ollama) without rewrite. The v1 `vllm-pearl` provider orchestrates Pearl's published Docker container; a runtime sidecar at `~/.Chottu/runtime/mining.json` decouples mining lifecycle from the existing vLLM engine class. Three deliberate seams (`submit_target` tagged-union, zero-valued `fee_bps`/`fees_owed`, reserved `mining/pools/`) leave room for v2 pool support without retrofit pain.
...
- Absolute imports only (`from Chottu.core.registry import ...`).
...
| Path | Responsibility |
|---|---|
| `src/Chottu/mining/__init__.py` | Package init; soft-imports providers via `try/except ImportError` |
| `src/Chottu/mining/_stubs.py` | `MiningProvider` ABC, `MiningCapabilities`, `MiningConfig`, `MiningStats`, `SoloTarget`/`PoolTarget` tagged union, `Sidecar` dataclass + read/write helpers |
| `src/Chottu/mining/_discovery.py` | `detect_providers()`, hardware/Docker/disk/pearld checks, wallet-format check |
| `src/Chottu/mining/_constants.py` | `PEARL_REPO`, `PEARL_PINNED_REF`, `PEARL_IMAGE_TAG`, default ports, default model, sidecar path constants |
| `src/Chottu/mining/_docker.py` | `PearlDockerLauncher` — `ensure_image()`, `start()`, `stop()`, `is_running()`, `get_logs()` |
| `src/Chottu/mining/_collector.py` | `MiningTelemetryCollector` — full impl, shipped unwired in v1; v1.x lights it up in the gateway daemon |
| `src/Chottu/mining/vllm_pearl.py` | `VllmPearlProvider` (`MiningProvider` impl); `_parse_gateway_metrics()`; `ensure_registered()` |
| `src/Chottu/mining/pools/__init__.py` | Reserved location for v2 `PoolClient` work — empty except for a docstring saying so |
| `src/Chottu/cli/mine_cmd.py` | `Chottu mine` Click group: `init`, `start`, `stop`, `status`, `doctor`, `attach`, `logs` |
...
| Path | Change |
|---|---|
| `src/Chottu/core/registry.py` | Add `MinerRegistry` class + entry in `__all__` |
| `src/Chottu/core/config.py` | Add `MiningConfig` field to `ChottuConfig`; add TOML parser for `[mining]` with `submit_target` tagged-union resolution |
| `src/Chottu/engine/_discovery.py` | Add sidecar-aware engine resolution: when `~/.Chottu/runtime/mining.json` exists, register a derived `vllm` engine pointing at `vllm_endpoint` |
| `src/Chottu/telemetry/store.py` | Add nullable `mining_session_id` column to inference rows; bump `PRAGMA user_version`; helper to tag rows when sidecar is present |
| `src/Chottu/cli/__init__.py` | Register the `mine` Click group |
| `src/Chottu/cli/hints.py` | Add hint: "mining configured but not running — start it with `Chottu mine start`" |
...
**Files:**
- Modify: `src/Chottu/core/registry.py`
...
def test_miner_registry_register_and_get():
    from Chottu.core.registry import MinerRegistry
...
class MinerRegistry(RegistryBase[Any]):
    ...
    ``Chottu.mining._stubs``. Registry keys are short lowercase strings
...
```bash
git add src/Chottu/core/registry.py tests/conftest.py tests/core/test_registry.py
...
**Files:**
- Create: `src/Chottu/mining/__init__.py`
- Create: `src/Chottu/mining/_constants.py`
- Create: `src/Chottu/mining/_stubs.py`
- Create: `src/Chottu/mining/pools/__init__.py`
...
from Chottu.core.config import GpuInfo, HardwareInfo
...
def test_mining_capabilities_default_unsupported():
    from Chottu.mining._stubs import MiningCapabilities
...
def test_solo_target_dataclass():
    from Chottu.mining._stubs import SoloTarget
...
def test_pool_target_dataclass():
    from Chottu.mining._stubs import PoolTarget
...
def test_mining_config_v1_defaults():
    from Chottu.mining._stubs import MiningConfig, SoloTarget
    cfg = MiningConfig(
        provider="vllm-pearl",
...
def test_mining_stats_v1_defaults():
    from Chottu.mining._stubs import MiningStats
...
def test_mining_provider_is_abstract():
    from Chottu.mining._stubs import MiningProvider
...
def test_sidecar_write_then_read_roundtrip(sidecar_path: Path, sample_sidecar_payload: dict):
    from Chottu.mining._stubs import Sidecar
...
```
Expected: ALL fail with `ImportError` on `Chottu.mining._stubs`.

- [ ] **Step 5: Create `_constants.py`**

```python
# src/Chottu/mining/_constants.py
...
PEARL_IMAGE_TAG = f"Chottu/pearl-miner:{PEARL_PINNED_REF}"
...
RUNTIME_DIR = Path.home() / ".Chottu" / "runtime"
...
PEARL_CACHE_DIR = Path.home() / ".Chottu" / "cache" / "pearl"
```

- [ ] **Step 6: Create `_stubs.py`**

```python
# src/Chottu/mining/_stubs.py
...
from Chottu.core.config import HardwareInfo
...
class MiningCapabilities:
    ...
    ``reason`` is human-readable and surfaced verbatim by ``Chottu mine doctor``
...
class MiningProvider(ABC):
    ...
    def detect(cls, hw: HardwareInfo, engine_id: str, model: str) -> MiningCapabilities:
        ...
        by ``Chottu mine doctor`` and ``Chottu mine init`` for fast capability
        reporting.
        """
...
class Sidecar:
    """Read/write helpers for ``~/.Chottu/runtime/mining.json``."""
...
from Chottu.mining._stubs import (
    MiningCapabilities,
    MiningConfig,
    MiningProvider,
    MiningStats,
...
try:
    from Chottu.mining import vllm_pearl  # noqa: F401
...
```python
# src/Chottu/mining/pools/__init__.py
...
```bash
git add src/Chottu/mining/ tests/mining/__init__.py tests/mining/conftest.py tests/mining/test_stubs.py
git commit -m "feat(mining): add ABC, dataclasses, constants, sidecar IO"
```

---

## Task 3 — `MiningConfig` integration in `ChottuConfig` + TOML parsing

**Files:**
- Modify: `src/Chottu/core/config.py`
...
provider       = "vllm-pearl"
wallet_address = "prl1qexampleaddress"
submit_target  = "pool:https://pool.Chottu.ai/submit"
...
def test_mining_config_absent_means_none(tmp_path):
    from Chottu.core.config import load_config
...
def test_mining_config_pool_parsed_as_pool_target(tmp_path):
    ...
    assert cfg.mining.submit_target.url == "https://pool.Chottu.ai/submit"
```
...
- [ ] **Step 4: Add `mining` field to `ChottuConfig`**

In `src/Chottu/core/config.py`, add an import near the top:

```python
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from Chottu.mining._stubs import MiningConfig
```

Add the field to `ChottuConfig` (insert in alphabetical order — after `memory_files`, before `operators`):
...
In `src/Chottu/core/config.py`, locate the existing `load_config()` function (around line 1541). Find the section that processes the loaded TOML data into the `ChottuConfig` (look for where it iterates the `data` dict and assigns to dataclass fields). Add a dedicated handler for `mining` because of the tagged-union parsing — it can't go through the generic walker.
...
def _parse_mining_section(data: dict) -> Optional["MiningConfig"]:
    ...
    from Chottu.mining._stubs import MiningConfig, PoolTarget, SoloTarget
...
```bash
git add src/Chottu/core/config.py tests/core/test_config.py tests/mining/fixtures/
git commit -m "feat(mining): integrate MiningConfig into ChottuConfig with TOML parsing"
```
...
**Files:**
- Create: `src/Chottu/mining/_discovery.py`
...
def test_detect_supported_on_h100(hopper_hw):
    from Chottu.mining._discovery import detect_for_engine_model
...
def test_check_docker_available_true():
    from Chottu.mining._discovery import check_docker_available
    with patch("Chottu.mining._discovery._docker_client") as fake:
...
def test_check_disk_free_passes(tmp_path):
    from Chottu.mining._discovery import check_disk_free
    with patch("Chottu.mining._discovery.shutil.disk_usage") as du:
        # 500 GB free
...
def test_check_pearld_reachable_true():
    from Chottu.mining._discovery import check_pearld_reachable
    with patch("Chottu.mining._discovery.httpx.post") as post:
...
def test_check_wallet_address_format_valid():
    from Chottu.mining._discovery import check_wallet_address_format
...
```python
# src/Chottu/mining/_discovery.py
...
verbatim by ``Chottu mine doctor``.
"""
...
def detect_for_engine_model(
    *,
    ...
    ``Chottu mine doctor`` and ``Chottu mine init``.
    """
...
# ---------------------------------------------------------------------------
# Doctor checks (one per row of `Chottu mine doctor` output)
# ---------------------------------------------------------------------------
...
def check_disk_free(path: Path) -> Tuple[bool, str]:
    from Chottu.mining._constants import MIN_FREE_DISK_GB
...
```bash
git add src/Chottu/mining/_discovery.py tests/mining/test_discovery.py
...
**Files:**
- Create: `src/Chottu/mining/_docker.py`
...
def test_ensure_image_already_local():
    from Chottu.mining._docker import PearlDockerLauncher
    fake = MagicMock()
    fake.images.get.return_value = MagicMock(id="sha256:abc", tags=["Chottu/pearl-miner:main"])
    launcher = PearlDockerLauncher(client=fake)
    out = launcher.ensure_image("Chottu/pearl-miner:main")
    assert out == "Chottu/pearl-miner:main"
    fake.images.get.assert_called_once_with("Chottu/pearl-miner:main")
...
def test_ensure_image_falls_back_to_build_for_default_tag():
    from Chottu.mining._docker import PearlDockerLauncher
    from Chottu.mining._constants import PEARL_IMAGE_TAG
    import docker.errors as derr
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
def test_ensure_image_errors_when_non_default_tag_missing():
    from Chottu.mining._docker import PearlDockerLauncher, ImageAcquisitionError
    import docker.errors as derr
    import pytest
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
from Chottu.mining._constants import (
    PEARL_CACHE_DIR,
    PEARL_IMAGE_TAG,
    PEARL_PINNED_REF,
    PEARL_REPO,
...
```bash
git add src/Chottu/mining/_docker.py tests/mining/test_docker.py
...
**Files:**
- Modify: `src/Chottu/mining/_docker.py`
...
def test_launcher_start_calls_run_with_expected_kwargs(_env_password):
    ...
    cfg = MiningConfig(
        ...
        submit_target=SoloTarget(pearld_rpc_url="http://localhost:44107"),
        extra={
            "docker_image_tag": "Chottu/pearl-miner:main",
            "model": "pearl-ai/Llama-3.3-70B-Instruct-pearl",
            "vllm_port": 8000,
...
def test_launcher_start_errors_when_password_env_missing():
    from Chottu.mining._docker import PearlDockerLauncher, ConfigurationError
    from Chottu.mining._stubs import MiningConfig, SoloTarget
    fake = MagicMock()
    launcher = PearlDockerLauncher(client=fake)
    cfg = MiningConfig(
    ...
    with pytest.raises(ConfigurationError) as ei:
        launcher.start(cfg, image="Chottu/pearl-miner:main")
...
Append to `src/Chottu/mining/_docker.py`:
...
class ConfigurationError(RuntimeError):
    ...
    def start(self, config: "MiningConfig", image: str) -> Any:
        ...
        """
        from Chottu.mining._stubs import MiningConfig  # noqa: F401  (typing only)
        ...
        if password is None:
            raise ConfigurationError(
                ...
                f"set it before running `Chottu mine start`"
            )
...
**Files:**
- Modify: `src/Chottu/mining/vllm_pearl.py` (creating in next task — for now create the parser as a helper)
- Create: `src/Chottu/mining/_metrics.py`
...
def test_parse_gateway_metrics_full():
    from Chottu.mining._metrics import parse_gateway_metrics
...
```bash
git add src/Chottu/mining/_metrics.py tests/mining/test_metrics.py tests/mining/fixtures/
...
**Files:**
- Create: `src/Chottu/mining/vllm_pearl.py`
...
def test_vllm_pearl_detect_supported_on_h100(hopper_hw):
    from Chottu.mining.vllm_pearl import VllmPearlProvider
...
async def test_vllm_pearl_start_writes_sidecar(tmp_path, monkeypatch):
    from Chottu.mining.vllm_pearl import VllmPearlProvider
    from Chottu.mining._stubs import MiningConfig, SoloTarget, Sidecar

    sidecar_path = tmp_path / "mining.json"
    monkeypatch.setattr(
        "Chottu.mining.vllm_pearl.SIDECAR_PATH", sidecar_path
    )
...
async def test_vllm_pearl_start_pool_target_raises_not_implemented(monkeypatch, tmp_path):
    from Chottu.mining.vllm_pearl import VllmPearlProvider
    from Chottu.mining._stubs import MiningConfig, PoolTarget
    ...
    cfg = MiningConfig(
        provider="vllm-pearl",
        wallet_address="prl1qaaa",
        submit_target=PoolTarget(url="https://pool.Chottu.ai/submit"),
        extra={"docker_image_tag": "Chottu/pearl-miner:main"},
    )
...
async def test_vllm_pearl_stop_removes_sidecar(tmp_path, monkeypatch, written_sidecar):
    from Chottu.mining.vllm_pearl import VllmPearlProvider
    monkeypatch.setattr(
        "Chottu.mining.vllm_pearl.SIDECAR_PATH", written_sidecar
    )
    fake_client = MagicMock()
    provider = VllmPearlProvider(docker_client=fake_client)
...
def test_vllm_pearl_stats_reads_gateway(monkeypatch, written_sidecar):
    ...
    with patch("Chottu.mining.vllm_pearl.httpx.get") as get:
...
def test_ensure_registered_is_idempotent():
    from Chottu.core.registry import MinerRegistry
    from Chottu.mining.vllm_pearl import (
        VllmPearlProvider,
        ensure_registered,
    )
    ensure_registered()
...
```python
# src/Chottu/mining/vllm_pearl.py
    ...
    async def start(self, config: MiningConfig) -> None:
        if isinstance(config.submit_target, PoolTarget):
            raise NotImplementedError(
                "pool support is v2 — see Chottu#XYZ. v1 only accepts "
                "submit_target='solo'."
            )
...
```bash
git add src/Chottu/mining/vllm_pearl.py tests/mining/test_vllm_pearl.py
...
**Files:**
- Modify: `src/Chottu/engine/_discovery.py`
...
Run:
```bash
sed -n '1,80p' src/Chottu/engine/_discovery.py
```
...
def test_engine_discovery_picks_up_mining_sidecar(tmp_path, monkeypatch, written_sidecar):
    ...
    """
    from Chottu.engine._discovery import discover_engines
    from Chottu.mining import _constants as mining_const
...
def _maybe_register_mining_sidecar_engine() -> None:
    ...
    """
    try:
        from Chottu.mining import Sidecar
        from Chottu.mining._constants import SIDECAR_PATH
    except ImportError:
        return
    payload = Sidecar.read(SIDECAR_PATH)
    if payload is None:
    ...
    from Chottu.core.registry import EngineRegistry
    from Chottu.engine.openai_compat_engines import OpenAICompatEngine  # adjust to actual class name
...
```bash
git add src/Chottu/engine/_discovery.py tests/conftest.py tests/mining/conftest.py tests/engine/test_discovery.py
...
**Files:**
- Create: `src/Chottu/mining/_collector.py`
...
async def test_collector_collect_once_returns_stats(written_sidecar):
    from Chottu.mining._collector import MiningTelemetryCollector

    sample = (
        "pearl_gateway_shares_submitted_total 50\n"
        "pearl_gateway_shares_accepted_total 49\n"
    )
    with patch("Chottu.mining._collector.httpx.get") as get:
        get.return_value.status_code = 200
        get.return_value.text = sample
        store = MagicMock()
...
from Chottu.mining._metrics import parse_gateway_metrics
from Chottu.mining._stubs import MiningStats, Sidecar
...
```bash
git add src/Chottu/mining/_collector.py tests/mining/test_collector.py
...
**Files:**
- Modify: `src/Chottu/telemetry/store.py`
...
Run:
```bash
grep -n "PRAGMA user_version\|CREATE TABLE\|ALTER TABLE\|migrate" src/Chottu/telemetry/store.py | head -20
```
...
def test_inference_row_has_mining_session_id_column(tmp_path):
    from Chottu.telemetry.store import TelemetryStore
    db = tmp_path / "tel.db"
...
In `src/Chottu/telemetry/store.py`:
...
```bash
git add src/Chottu/telemetry/store.py tests/telemetry/test_store.py
git commit -m "feat(telemetry): add mining_session_id + mining_stats table"
```

---

## Task 12 — `Chottu mine doctor` (highest user-facing value, build first)

**Files:**
- Create: `src/Chottu/cli/mine_cmd.py`
...
def test_mine_doctor_prints_capability_matrix(monkeypatch):
    ...
    with patch("Chottu.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("Chottu.cli.mine_cmd.check_docker_available", return_value=(True, "running 24.0.7")), \
         patch("Chottu.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("Chottu.cli.mine_cmd.check_pearld_reachable", return_value=(True, "block height 442107 (synced)")):
...
def test_mine_doctor_flags_unsupported_hardware():
    ...
    with patch("Chottu.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("Chottu.cli.mine_cmd.check_docker_available", return_value=(True, "ok")), \
         patch("Chottu.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("Chottu.cli.mine_cmd.check_pearld_reachable", return_value=(False, "connection refused")):
        result = runner.invoke(mine, ["doctor"])
...
```python
# src/Chottu/cli/mine_cmd.py
"""``Chottu mine`` command group.
...
from Chottu.core.config import HardwareInfo, load_config
from Chottu.mining._constants import (
    DEFAULT_PEARL_MODEL,
...
from Chottu.mining._discovery import (
    check_disk_free,
...
def mine() -> None:
    ...
    See https://Chottu.github.io/Chottu/user-guide/mining/ for the
    full guide.
    """
...
def doctor() -> None:
    ...
    if mining_cfg is not None:
        ...
        row("pearld", "RPC", False, "no [mining] config — run `Chottu mine init`")
...
```bash
git add src/Chottu/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): Chottu mine doctor"
```

---

## Task 13 — `Chottu mine init / start / stop`

**Files:**
- Modify: `src/Chottu/cli/mine_cmd.py`
...
def test_mine_start_runs_provider_start(monkeypatch):
    ...
    with patch("Chottu.cli.mine_cmd.MinerRegistry") as reg, \
         patch("Chottu.cli.mine_cmd.load_config") as load, \
         patch("Chottu.cli.mine_cmd.asyncio.run") as arun:
        from Chottu.mining._stubs import MiningConfig, SoloTarget
...
def test_mine_start_errors_when_no_mining_config():
    from Chottu.cli.mine_cmd import mine
    runner = CliRunner()
    with patch("Chottu.cli.mine_cmd.load_config") as load:
...
def init(
    ...
    if not cap.supported:
        raise click.ClickException(
            ...
            f"See `Chottu mine doctor` for details."
        )
    ...
    if pearld_password_env not in os.environ:
        click.echo(
            ...
            f"Set it before `Chottu mine start`.",
            err=True,
        )
    ...
    config_path = Path.home() / ".Chottu" / "config.toml"
...
"""
    ...
    click.echo(f"Done. Run `Chottu mine start` to begin mining.")
...
def start() -> None:
    ...
    if cfg is None:
        raise click.ClickException("no [mining] section in config — run `Chottu mine init`")
    ...
    async def _run():
        await provider.start(cfg)
    asyncio.run(_run())
    click.echo(f"Mining started. Run `Chottu mine status` for live stats.")
...
```bash
git add src/Chottu/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): Chottu mine init/start/stop"
```

---

## Task 14 — `Chottu mine status / attach / logs`
...
def test_mine_status_renders_stats(written_sidecar, monkeypatch):
    from Chottu.cli.mine_cmd import mine
    runner = CliRunner()
    monkeypatch.setattr("Chottu.cli.mine_cmd.SIDECAR_PATH", written_sidecar)
    sample = (
        "pearl_gateway_shares_submitted_total 100\n"
        "pearl_gateway_shares_accepted_total 99\n"
    ...
    with patch("Chottu.mining.vllm_pearl.httpx.get") as get, \
         patch("Chottu.cli.mine_cmd.MinerRegistry") as reg:
        get.return_value.status_code = 200
        get.return_value.text = sample
        from Chottu.mining.vllm_pearl import VllmPearlProvider
...
def test_mine_attach_writes_sidecar(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("Chottu.cli.mine_cmd.SIDECAR_PATH", sidecar)
...
def test_mine_logs_streams_container_output(monkeypatch):
    ...
    with patch("Chottu.cli.mine_cmd.PearlDockerLauncher",
...
def status() -> None:
    ...
    cfg = load_config().mining
    if cfg is None:
        raise click.ClickException("no [mining] section — run `Chottu mine init`")
    provider_cls = MinerRegistry.get(cfg.provider)
    provider = provider_cls()
    s = provider.stats()
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        container = docker.from_env().containers.get("Chottu-pearl-miner")
...
```bash
git add src/Chottu/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): Chottu mine status/attach/logs"
```
...
**Files:**
- Modify: `src/Chottu/cli/__init__.py`
- Modify: `src/Chottu/cli/hints.py`
...
```bash
sed -n '1,50p' src/Chottu/cli/__init__.py
grep -n "add_command\|@main.command\|main.add_command" src/Chottu/cli/__init__.py | head -20
```
...
def mining_not_running_hint(cfg, sidecar_present: bool) -> Optional[str]:
    if cfg is None or sidecar_present:
        return None
    return "mining configured but not running — start it with `Chottu mine start`"
```
...
```bash
git add src/Chottu/cli/__init__.py src/Chottu/cli/hints.py tests/cli/test_main.py tests/cli/test_hints.py
...
Chottu can mine the [Pearl](https://github.com/pearl-research-labs/pearl)
...
uv run Chottu mine init    # writes [mining] config + builds Docker image (30-60 min first time)
uv run Chottu mine start
uv run Chottu mine status
```

## Diagnosing problems

`Chottu mine doctor` prints one row per check with a clear ✓ or ✗ and reason.
...
The `Chottu.mining` subsystem uses a registry pattern identical to
...
1. Create `src/Chottu/mining/<provider>.py`.
2. Subclass `Chottu.mining.MiningProvider`.
...
5. Add a soft-import in `mining/__init__.py`:
   ```python
   try:
       from Chottu.mining import <provider>  # noqa: F401
...
```markdown
- New mining providers must register via `MinerRegistry` in `src/Chottu/core/registry.py` and expose an idempotent `ensure_registered()` per the autouse-clear test convention.
```
...
```bash
uv run pytest tests/mining/ --cov=Chottu.mining --cov-report=term-missing
```
Expected: ≥ 80% coverage on the new `mining/` package.

- [ ] **Step 4: Smoke `Chottu mine doctor` on a non-mining dev box**

```bash
uv run Chottu mine doctor
```
...
## Summary
- New `Chottu.mining` subsystem with `MiningProvider` ABC + `MinerRegistry`
- `vllm-pearl` provider wrapping Pearl's Docker container
- Runtime sidecar (`~/.Chottu/runtime/mining.json`) for engine ↔ mining handoff
- Telemetry: on-demand reads in v1; `MiningTelemetryCollector` shipped unwired for v1.x
- New CLI: `Chottu mine init|start|stop|status|doctor|attach|logs`
...
- [ ] Manual smoke: `uv run Chottu mine doctor` on a non-mining dev box prints honest output

...
Each script follows the same SDK pattern: create a `Chottu` instance, call `j.ask()` with an agent and tools, print the result, and close the instance. The schedule is managed externally by the Chottu scheduler daemon.
...
```mermaid
graph TD
    A[Chottu scheduler start] --> B[Scheduler Daemon]
...
Register each script as a recurring task using `Chottu scheduler create`:

```bash title="Terminal"
# Morning digest every day at 9 AM
Chottu scheduler create "Run daily news digest" \
    --type cron --value "0 9 * * *"

# Weekly code review every Monday at 8 AM
Chottu scheduler create "Run weekly code review" \
    --type cron --value "0 8 * * 1"

# Gym check on Monday, Wednesday, Friday at 6 AM
Chottu scheduler create "Check gym schedule" \
...
```bash title="Terminal"
Chottu scheduler list
```

!!! note "Cron expression syntax"
    Chottu uses standard five-field cron syntax: `minute hour day-of-month month day-of-week`. Install `croniter` (`uv add croniter`) for full expression support including ranges and step values. Without it, basic `hour:minute` patterns still work.
...
from Chottu.scheduler import TaskScheduler
...
j = Chottu()  # uses defaults from ~/.Chottu/config.toml
response = j.ask(
...
To route script output to Slack or any other supported channel, pipe stdout through `Chottu channel send`:

```bash title="Terminal"
uv run python examples/scheduled_ops/daily_digest.py \
    --topics "AI,finance" | Chottu channel send slack
```

Or add channel output inside the script:

```python title="In-script channel output"
from Chottu.channels import ChannelRegistry
...
!!! warning "Channel credentials"
    Live channel output requires channel-specific credentials. Run `Chottu add slack` (or the relevant provider) to set up the MCP server and credential store, then configure environment variables in your `.env` file before starting the scheduler daemon.
...
- **One-time tasks**: Use `--type once --value "2026-04-01T09:00:00"` with `Chottu scheduler create` for non-recurring tasks.

...
!!! tip "Prerequisites"
    ...
    - Chottu installed: run `uv sync --extra dev` from the repository root
...
The script creates a `Chottu` instance and delegates the research task to an `OrchestratorAgent` with five tools wired in. The orchestrator iterates through multiple tool-calling turns, deciding at each step whether to search, store, think, or synthesize.

```mermaid
sequenceDiagram
    participant U as User
    participant J as Chottu SDK
...
j = Chottu(model="qwen3:8b", engine_key="ollama")  # (1)!
try:
    response = j.ask(
...
1. Creates a `Chottu` instance targeting the local Ollama engine with `qwen3:8b`. Both parameters are optional — omitting them uses auto-detected defaults from `~/.Chottu/config.toml`.
...
from Chottu.recipes import load_recipe
...
This is useful when you want to version-control the research configuration, share it with collaborators, or feed it to the `Chottu eval` runner for benchmarking.
...
Run `Chottu agent info orchestrator` to see the full tool catalog.
...
- [Getting Started: Configuration](../getting-started/configuration.md) — how to configure engines and models in `~/.Chottu/config.toml`

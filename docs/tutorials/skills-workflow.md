...
!!! note "Before you begin"
    This tutorial assumes Chottu is installed with Ollama running and a model available (e.g., `qwen3.5:9b`). If you have not completed setup yet, start with the [Quick Start guide](../getting-started/quickstart.md).

## Step 1: Install Skills from Hermes Agent

Chottu can import skills from the [Hermes Agent](https://github.com/NousResearch/hermes-agent) skill library maintained by NousResearch. Let's install a few useful ones.

```bash
# Install individual skills
Chottu skill install hermes:arxiv
Chottu skill install hermes:github-pr-workflow

# Or bulk install an entire category
Chottu skill sync hermes --category research
```

The first install clones the Hermes repo to `~/.Chottu/skill-cache/hermes/` (one-time, ~5s). Subsequent installs reuse the cache.
...
```bash
cat ~/.Chottu/skills/hermes/arxiv/SKILL.md | head -40
```

The `.source` file records provenance:

```bash
cat ~/.Chottu/skills/hermes/arxiv/.source
```
...
```bash
Chottu ask "Use the math-solver skill to compute 17 * 23" \
  --engine ollama --model qwen3.5:9b
```
...
```bash
mkdir -p ~/.Chottu/skills/my-reviewer
```

Write a SKILL.md:

```bash
cat > ~/.Chottu/skills/my-reviewer/SKILL.md << 'EOF'
---
name: my-reviewer
...
```bash
Chottu ask "Use the my-reviewer skill to review this function: def login(user, pwd): return db.query(f'SELECT * FROM users WHERE name={user} AND pass={pwd}')" \
  --engine ollama --model qwen3.5:9b
```
...
Chottu ask "Use math-solver to compute 100 / 7"
Chottu ask "Use code-explainer to explain: lambda x: x**2"
Chottu ask "Use my-reviewer to review: def add(a,b): return a+b"
Chottu ask "Use math-solver to compute 2**10"
Chottu ask "Use code-explainer to explain: [x for x in range(10) if x % 2 == 0]"
```

Each query produces a trace in `~/.Chottu/traces.db` with skill metadata tags (`skill`, `skill_source`, `skill_kind`).
...
Chottu skill discover --dry-run --min-frequency 2

# Write discovered patterns as skill manifests
Chottu skill discover --min-frequency 2
```
...
This produces overlay files at `~/.Chottu/learning/skills/<skill-name>/optimized.toml` with improved descriptions and few-shot examples extracted from your best traces.

Inspect what was produced:

```bash
Chottu skill show-overlay math-solver
Chottu skill show-overlay code-explainer
```
...
For a hands-off experience, add this to `~/.Chottu/config.toml`:
...
|---------|-------------|
| **Installing skills** | `Chottu skill install hermes:arxiv` — imported from public sources |
| **Using skills** | `Chottu ask "Use the code-explainer skill..."` — agent invokes skills as tools |
...
| **Discovering patterns** | `Chottu skill discover` — mined traces for recurring tool sequences |
| **Optimizing skills** | `Chottu optimize skills --policy dspy` — improved descriptions + few-shot examples |
| **Benchmarking** | `Chottu bench skills` — measured the impact across 4 conditions |

...
1. Both `model` and `engine_key` are optional. Omitting them uses auto-detected defaults from `~/.Chottu/config.toml`.
...
4. The tool list is passed directly. Any registered tool name is valid — run `Chottu agent info native_react` to see all available tools.

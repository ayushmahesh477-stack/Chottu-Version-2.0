...
Chottu provides a command-line interface through the `Chottu` command. Built on [Click](https://click.palletsprojects.com/), it offers subcommands for querying models, managing memory, running benchmarks, and serving an OpenAI-compatible API.

## Global Options

```bash
Chottu --version   # Print the Chottu version
Chottu --help      # Show top-level help with all subcommands
```

## `Chottu init`

Detect local hardware (CPU, GPU, RAM) and generate a configuration file at `~/.Chottu/config.toml`.

```bash
Chottu init           # Interactive — refuses to overwrite existing config
Chottu init --force   # Overwrite existing config without prompting
```
...
```bash
Chottu ask "Explain quantum computing"
```

**Agent mode** routes the query through an agent that can use tools and manage multi-turn interactions:

```bash
Chottu ask --agent orchestrator "What is 2+2?"
Chottu ask --agent orchestrator --tools calculator,think "Calculate sqrt(144) + 3^2"
Chottu ask --agent simple "Hello"
```
...
```bash
# Basic query
Chottu ask "What is machine learning?"

# Specify a model
Chottu ask -m qwen3:8b "Summarize this concept"

# Use the orchestrator agent with tools
Chottu ask --agent orchestrator --tools calculator "What is 15% of 340?"

# Get JSON output
Chottu ask --json "Hello"

# Disable memory context injection
Chottu ask --no-context "Tell me about Python"

# Set maximum token generation
Chottu ask --max-tokens 2048 "Write a detailed essay about AI"
```
...
Access Pearl's native node, wallet, and RPC tools from the Chottu CLI.

```bash
Chottu pearl doctor
Chottu pearl node -- <pearld args>
Chottu pearl wallet -- <oyster args>
Chottu pearl ctl -- <prlctl args>
Chottu pearl address
```

All Pearl wrapper commands use the `Chottu pearl <command>` shape. The
pass-through commands map to Pearl's native binaries:

| Chottu command | Pearl binary | Use |
|--------------------|--------------|-----|
| `Chottu pearl doctor` | n/a | Check whether `pearld`, `oyster`, and `prlctl` are discoverable |
| `Chottu pearl node` | `pearld` | Run the Pearl full node |
| `Chottu pearl wallet` | `oyster` | Run the Oyster wallet daemon |
| `Chottu pearl ctl` | `prlctl` | Query Pearl node or wallet RPC |
| `Chottu pearl address` | `prlctl --wallet getnewaddress` | Generate a wallet address from Oyster |
...
### `Chottu memory index <path>`

Index documents from a file or directory into the memory store.

```bash
Chottu memory index ./docs/
Chottu memory index ./notes.md
Chottu memory index ./data/ --chunk-size 256 --chunk-overlap 32
Chottu memory index ./docs/ --backend sqlite
```
...
### `Chottu memory search <query>`

Search the memory store for relevant document chunks.

```bash
Chottu memory search "machine learning basics"
Chottu memory search -k 10 "neural networks"
Chottu memory search --backend faiss "embeddings"
```
...
```bash
Chottu memory stats
Chottu memory stats --backend sqlite
```
...
```bash
Chottu telemetry stats
Chottu telemetry stats -n 5    # Show top 5 models
```
...
### `Chottu telemetry export`

Export raw telemetry records in JSON or CSV format.

```bash
Chottu telemetry export                          # JSON to stdout
Chottu telemetry export --format csv             # CSV to stdout
Chottu telemetry export --format json -o data.json  # JSON to file
Chottu telemetry export -f csv -o metrics.csv    # CSV to file
```
...
### `Chottu telemetry clear`

Delete all telemetry records from the database.

```bash
Chottu telemetry clear         # Interactive confirmation
Chottu telemetry clear --yes   # Skip confirmation
```
...
```bash
Chottu bench run                               # Run all benchmarks, 10 samples
Chottu bench run -n 20                         # 20 samples per benchmark
Chottu bench run -b latency                    # Only the latency benchmark
Chottu bench run -b throughput -n 50 --json    # Throughput, 50 samples, JSON output
Chottu bench run -o results.jsonl              # Write JSONL results to file
Chottu bench run -m qwen3:8b -e ollama         # Specific model and engine
```
...
```bash
Chottu channel send slack "Hello from Chottu!"
Chottu channel send discord "Build complete"
```
...
```bash
Chottu serve                                 # Default host/port from config
Chottu serve --port 8000                     # Custom port
Chottu serve --host 0.0.0.0 --port 9000      # Bind to all interfaces
Chottu serve --model qwen3:8b                # Specify default model
Chottu serve --agent orchestrator            # Route requests through an agent
```
...
`Chottu` subcommand for it. Construct a `SpecSearchOrchestrator`
directly from `Chottu.learning.spec_search.orchestrator` and call
`.run(trigger)` with a trigger from
`Chottu.learning.spec_search.triggers`. See

...
Chottu has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.Chottu/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.Chottu/telemetry.db")
...
Chottu telemetry stats -n 5          # Top 5 models only

# Export records
Chottu telemetry export              # JSON to stdout
Chottu telemetry export -f csv       # CSV to stdout
Chottu telemetry export -o data.json # JSON to file
Chottu telemetry export -f csv -o metrics.csv

# Clear all records
Chottu telemetry clear               # With confirmation prompt
Chottu telemetry clear --yes         # Without confirmation
```

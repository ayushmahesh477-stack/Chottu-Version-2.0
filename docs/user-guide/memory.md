...
- **Persistence:** SQLite database file (default: `~/.Chottu/memory.db`)
...
```python
from Chottu.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from Chottu.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `Chottu.tools.storage.*` imports.
...
[Source: docs/intro.md] Chottu is a modular AI framework...
...
Chottu memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
Chottu memory search "machine learning"

...
Chottu has two types of integrations:
...
```json
{
    "display_information": { "name": "Chottu" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "Chottu", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
- You DM @Chottu in Slack → Socket Mode receives the event in real-time
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "Chottu" |
...
3. **Connect in Chottu:**
   ...
   - CLI: `uv run Chottu connect gmail_imap`
...
Slack serves two purposes in Chottu:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [Chottu icon](https://github.com/Chottu/Chottu/blob/main/assets/Chottu-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **Chottu** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "Chottu"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @Chottu` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @Chottu` in the channel |
...
3. **Connect in Chottu:**
   ...
   - CLI: `uv run Chottu connect notion`
...
3. **Connect in Chottu:**
   ...
   - CLI: `uv run Chottu connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the Chottu desktop app
...
3. **Connect in Chottu:**
   ...
   - CLI: `uv run Chottu connect outlook`
...
2. **Connect in Chottu:**
   ...
   - CLI: `uv run Chottu connect obsidian --path /path/to/vault`
...
4. **Connect in Chottu:**
   ...
   - CLI: `uv run Chottu connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run Chottu deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.Chottu/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.Chottu/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.

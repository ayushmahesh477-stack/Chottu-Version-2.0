...
!!! note "Optional component"
    The scheduler is a standalone module (`Chottu.scheduler`). It is not wired into the default `Chottu` / `ChottuSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `Chottu scheduler start`.
...
The `Chottu scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
Chottu scheduler list --status active

# Paused tasks
Chottu scheduler list --status paused
```
...
Chottu scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
Chottu scheduler resume a3f9b12c4d8e
```
...
Chottu scheduler cancel a3f9b12c4d8e
```
...
Chottu scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.Chottu/scheduler.db")  # (1)!

# Wire in a ChottuSystem for task execution
...
4. Starts a daemon thread named `"Chottu-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.Chottu/config.toml`.

```toml title="~/.Chottu/config.toml"
[scheduler]
enabled = false
db_path = "~/.Chottu/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.Chottu/scheduler.db`     | SQLite database path                       |

...
Chottu includes a thin `Chottu pearl` wrapper for Pearl's native command
...
`Chottu pearl` looks for `pearld`, `oyster`, and `prlctl` on `PATH`, then under
`$PEARL_HOME/bin`.
...
```bash
Chottu pearl node -- --help
Chottu pearl wallet -- --help
Chottu pearl ctl -- --help
```

These map directly to:

| Chottu command | Pearl binary |
|---|---|
| `Chottu pearl node` | `pearld` |
| `Chottu pearl wallet` | `oyster` |
| `Chottu pearl ctl` | `prlctl` |

The command format is always `Chottu pearl <command>`. Pearl-native arguments
...
`Chottu mine` is the Chottu mining lifecycle. `Chottu pearl` is an escape

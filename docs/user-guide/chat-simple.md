...
A lightweight conversational AI with no tools and no agent overhead. This is the simplest possible Chottu setup: just Ollama and a local model. Ideal for general-purpose chat, Q&A, brainstorming, and getting started quickly.
...
### 2. Install and initialize Chottu

```bash
git clone https://github.com/Chottu/Chottu.git
cd Chottu
uv sync
Chottu init --preset chat-simple
```

### 3. Ask a question

```bash
Chottu ask "What is quantum computing?"
```
...
```bash
# Single question
Chottu ask "Explain the difference between TCP and UDP"
...
Chottu ask -m qwen3.5:9b "Explain general relativity"

# Adjust temperature (0.0 = deterministic, 1.0 = creative)
Chottu ask -t 0.2 "List the planets in our solar system"

# Output raw JSON
Chottu ask --json "What is 2+2?"
```
...
To switch models, either edit `~/.Chottu/config.toml` or override per-query:

```bash
Chottu ask -m qwen3.5:35b "Write a detailed comparison of REST and GraphQL"
```
...
1. Start the backend: `Chottu serve` (or `./scripts/quickstart.sh`)
2. Download and open the desktop app from the [releases page](https://github.com/Chottu/Chottu/releases)
...
${EDITOR:-nano} ~/.Chottu/config.toml
...
Chottu ask -m deepseek-r1:14b "Hello"
```

**Use an environment variable:**

```bash
Chottu_MODEL=qwen3.5:9b Chottu ask "Hello"
```
...
**Browser app not loading** -- Make sure both the backend (`Chottu serve`) and frontend are running. The `./scripts/quickstart.sh` script starts both automatically.

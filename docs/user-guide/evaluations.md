...
The Chottu evaluation framework (`Chottu-evals`) measures model **correctness and accuracy** on academic datasets. It is a separate package from the main Chottu library and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    Chottu has two distinct measurement systems that complement each other:

    | System | Package | Measures | Entry Point |
    |--------|---------|----------|-------------|
    | **Evaluations** | `Chottu-evals` | Correctness on academic datasets (accuracy, pass rate) | `Chottu-eval` |
    | **Benchmarks** | `Chottu` | Engine performance (latency, throughput) | `Chottu bench` |
...
The evaluation framework is a standalone package in the `evals/` directory. Install it alongside Chottu:

```bash
uv sync --extra eval
```

This installs the `Chottu-eval` CLI entry point and all required dependencies (`datasets`, `huggingface-hub`, `tqdm`, `rich`).
...
These benchmarks evaluate models on practical tasks that mirror real Chottu use cases.
...
```bash
uv run python -m Chottu.evals --config src/Chottu/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run python -m Chottu.evals --config src/Chottu/evals/configs/use_case_v2_local.toml
```
...
|---------|-----|-------------|
| **Chottu-direct** | `Chottu-direct` | Engine-level inference via `SystemBuilder`. Works for local (Ollama, vLLM, llama.cpp) and cloud models. |
| **Chottu-agent** | `Chottu-agent` | Agent-level inference with tool calling. Uses `ChottuSystem.ask()` with the specified agent and tools. |

Use `Chottu-direct` for most evaluations. Use `Chottu-agent` when the benchmark requires tool use — for example, GAIA tasks that reference files that must be read with `file_read`, or arithmetic tasks that benefit from `calculator`.
...
| `--backend` | | choice | `Chottu-direct` | `Chottu-direct` or `Chottu-agent` |
| `--model` | `-m` | str | required* | Model identifier (e.g., `qwen3:8b`, `gpt-4o`) |
| `--engine` | `-e` | str | auto | Engine key (`ollama`, `vllm`, `cloud`, ...) |
| `--agent` | | str | `orchestrator` | Agent name for `Chottu-agent` backend |
...
```bash
Chottu-eval run-all -m qwen3:8b

# With options
Chottu-eval run-all -m gpt-4o -n 100 --output-dir results/gpt4o/
```
...
```bash
Chottu-eval summarize results/supergpqa_qwen3-8b.jsonl
```
...
```bash
Chottu-eval run --config src/Chottu/evals/configs/full-suite.toml
```
...
| `backend` | str | `"Chottu-direct"` | Inference backend: `Chottu-direct` or `Chottu-agent` |
...
| `agent` | str | `None` | Agent name for `Chottu-agent` backend (e.g., `"orchestrator"`) |
| `tools` | list[str] | `[]` | Tool names for `Chottu-agent` backend |
...
Chottu-eval run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Agents](agents.md) — Configure the `OrchestratorAgent` used by `Chottu-agent` backend
- [Tools](tools.md) — Available tools for agent-backed evaluations
- [Python SDK](python-sdk.md) — Programmatic access to Chottu inference and agents

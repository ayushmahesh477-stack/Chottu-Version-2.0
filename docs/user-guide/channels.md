...
The channels module lets Chottu send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from Chottu.channels.telegram import TelegramChannel
...
from Chottu.channels._stubs import ChannelMessage
from Chottu.channels.discord_channel import DiscordChannel
...
from Chottu.channels.slack import SlackChannel
...
from Chottu.core.events import EventBus, EventType
from Chottu.channels.telegram import TelegramChannel
...
The `Chottu channel` subcommand group provides quick access to channel operations.
...
Chottu channel send telegram "Build completed successfully"
```
...
When `Chottu serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.Chottu/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.Chottu/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from Chottu!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  Chottu
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. Chottu sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since Chottu runs locally, you need a tunnel so SendBlue can reach your webhook:
...
from Chottu.channels.sendblue import SendBlueChannel
...
# Send a message
ok = channel.send("+15551234567", "Hello from Chottu!")
```
...
Chottu channel status --channel-type sendblue

# Send a message
Chottu channel send sendblue "+15551234567" "Hello from Chottu!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `Chottu serve` starts:
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.Chottu/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
from Chottu.channels.whatsapp_baileys import WhatsAppBaileysChannel

channel = WhatsAppBaileysChannel(
    assistant_name="Chottu",           # (1)!
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.Chottu/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.Chottu/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
| `assistant_name`           | `str`      | `"Chottu"`  | Display name for the assistant                         |
...
auth_dir = "/home/user/.Chottu/whatsapp_baileys_bridge/auth"
assistant_name = "Chottu"
...
- [API Reference: Channels](../api-reference/Chottu/channels/index.md) -- full class and type signatures

...
Chottu can mine the Pearl Proof-of-Useful-Work chain through local LLM
...
Chottu falls back to building from the pinned Pearl source checkout. First
builds can take 30-60 minutes.

On a shared NVIDIA host, restrict the miner to idle GPUs:

```bash
uv run Chottu mine init --cuda-visible-devices 0
```
...
- `Chottu mine models` lists Pearl model support status.
- `Chottu mine inspect-model` checks a Pearl model artifact before GPU launch.
- `Chottu mine doctor` prints hardware, Docker, Pearl node, wallet, provider,
  and session checks.
- `Chottu mine init` writes the local mining config and resolves the image.
- `Chottu mine start` launches the Pearl miner container and writes the runtime
  sidecar.
- `Chottu mine stop` stops the provider and removes the sidecar.
- `Chottu mine status` reads live gateway metrics.
- `Chottu mine attach` writes a sidecar for a miner you launched manually.
- `Chottu mine logs` prints the Docker container log tail.
- `Chottu mine validate-model` probes the active vLLM miner and gateway before
...
Chottu only lists Pearl-compatible models published by the Pearl Research
...
pass the Chottu H100/H200 validation run.

When validating a Pearl org model on a mining host, run:

```bash
Chottu mine inspect-model \
  --model pearl-ai/Gemma-4-31B-it-pearl \
  --allow-planned

Chottu mine validate-model \
  --model pearl-ai/Gemma-4-31B-it-pearl \
  --allow-planned \

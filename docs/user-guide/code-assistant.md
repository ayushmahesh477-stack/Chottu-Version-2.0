...
This writes a pre-configured `~/.Chottu/config.toml` for the code assistant.
...
```bash
Chottu ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
Chottu ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
Chottu ask "Read main.py and explain the architecture"

# Fix a bug
Chottu ask "Find and fix the bug in test_utils.py"

# Run tests
Chottu ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
Chottu ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
The preset writes this to `~/.Chottu/config.toml`:
...
```bash
# Write a new script
Chottu ask "Write a Python script that converts YAML to JSON"

# Explain existing code
Chottu ask "Read src/Chottu/core/events.py and explain the EventBus pattern"

# Debug a failing test
Chottu ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
Chottu ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
Chottu ask "Read src/Chottu/tools/calculator.py and write unit tests for it"

# Shell tasks
Chottu ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`Chottu chat`).
- For stronger isolation, use the sandboxed agent: `Chottu ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `Chottu`. Use `cd /path && command` in your prompt if needed, or run `Chottu` from the project directory.

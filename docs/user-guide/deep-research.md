...
This writes a pre-configured `~/.Chottu/config.toml` for the deep research agent.
...
Chottu chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
Chottu ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
Chottu ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"

# Index more documents
Chottu memory index ~/Downloads/reports/
Chottu memory index ./notes.md

# Search memory directly
Chottu memory search "project timeline"
Chottu memory search -k 20 "budget estimates"
...
Chottu ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
Chottu ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
Chottu ask "What meetings did I have with Alice last month?"

# Extract structured data
Chottu ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
Chottu ask "Compare our internal benchmarks with the latest published results"
```
...
# Single file
Chottu memory index ./report.pdf

# Custom chunk size for long documents
Chottu memory index ./paper.pdf --chunk-size 1024 --chunk-overlap 128
```
...
```bash
Chottu memory index ~/Papers/*.pdf
```
...
```bash
curl -s https://example.com/article | Chottu memory index --stdin --source "example.com"
```

### Code repositories

```bash
Chottu memory index ./src/ --chunk-size 256
```
...
**"No results found"** -- Make sure you have indexed documents first with `Chottu memory index`. Check indexed content with `Chottu memory stats`.

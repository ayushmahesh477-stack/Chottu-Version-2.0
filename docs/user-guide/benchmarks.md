...
Chottu ships with two benchmarks:
...
from Chottu.bench._stubs import BenchmarkResult
...
from Chottu.bench.latency import LatencyBenchmark
from Chottu.bench.throughput import ThroughputBenchmark
...
Chottu bench run -n 50

# Run only the latency benchmark
Chottu bench run -b latency

# Run only the throughput benchmark with 20 samples
Chottu bench run -b throughput -n 20

# Specify model and engine
Chottu bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
Chottu bench run --json

# Write JSONL results to a file
Chottu bench run -o results.jsonl

# Combine options
Chottu bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from Chottu.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
Chottu bench run -b context_length
```

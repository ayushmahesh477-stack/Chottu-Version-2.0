...
from Chottu.agents._stubs import AgentContext, AgentResult
...
!!! note "Backward compatibility"
    The registry alias `"react"` maps to `NativeReActAgent`. The old import `from Chottu.agents.react import ReActAgent` also still works.
...
```bash
# Simple agent
Chottu ask --agent simple "What is the capital of France?"

# Orchestrator with tools
Chottu ask --agent orchestrator --tools calculator,think "What is sqrt(256)?"

# NativeReActAgent
Chottu ask --agent native_react --tools calculator "What is 2+2?"

# ReAct alias (same as native_react)
Chottu ask --agent react --tools calculator,think "Solve step by step: 15% of 340"

# NativeOpenHandsAgent
Chottu ask --agent native_openhands --tools calculator,web_search "Summarize example.com"

# RLMAgent
Chottu ask --agent rlm "Summarize this long document"

# OpenHands SDK agent
Chottu ask --agent openhands "Fix the bug in test_utils.py"
```
...
!!! warning "Requirements"
    Requires Node.js 22+ on `PATH` and an `ANTHROPIC_API_KEY` environment variable (or pass `api_key=` directly). The bundled runner is auto-installed to `~/.Chottu/claude_code_runner/` on first use via `npm install`.

**How it works:**

1. On first call, copies the bundled `claude_code_runner/` to `~/.Chottu/claude_code_runner/` and runs `npm install --production` if `node_modules` is missing.
...
4. The Python side parses the output between `---Chottu_OUTPUT_START---` and `---Chottu_OUTPUT_END---` markers, extracting content, tool results, and metadata.
...
**When to use:** For software engineering tasks where the Claude Agent SDK's built-in tools (code editing, bash execution, file operations) provide capabilities beyond what Chottu tool-calling agents support.

```python
from Chottu.agents.claude_code import ClaudeCodeAgent
...
```bash
# Via CLI
Chottu ask --agent claude_code "Refactor the tests to use pytest fixtures"
```

!!! info "accepts_tools = False"
    `ClaudeCodeAgent` does not accept Chottu tools via `--tools`. Tool access for the Claude agent is configured separately via the `allowed_tools` constructor parameter, which passes tool names understood by the Claude Agent SDK itself.

---

## OpenCodeAgent

The `OpenCodeAgent` delegates coding tasks to [opencode](https://opencode.ai), the open-source coding agent, running it **on your local engine**. opencode handles the agentic loop, file edits, and tool use; Chottu supplies the model — keeping coding-agent work local-first.

!!! warning "Requirements"
    Requires the `opencode` binary on `PATH` (`npm i -g opencode-ai` or `brew install anomalyco/tap/opencode`). It is **not** bundled; `run()` returns a clear error if it is missing. No `ANTHROPIC_API_KEY` needed — inference goes through your Chottu engine.

**How it works:**

1. Derives an OpenAI-compatible base URL from the `engine` (e.g. Ollama/vLLM/llama.cpp at `<host>/v1`) and writes an `opencode.json` in the workspace registering it as an `@ai-sdk/openai-compatible` provider (`Chottu/<model>`).
...
| `provider_id`       | `str`             | `"Chottu"`   | opencode provider id to register/use                     |
...
```python
from Chottu.agents.opencode import OpenCodeAgent
...
Chottu ask --agent opencode "Refactor the parser to use a state machine"
```
...
```python
from Chottu.agents.operative import OperativeAgent
...
```bash
# Via CLI
Chottu ask --agent operative "Check system status"
```
...
```python
from Chottu.agents.monitor_operative import MonitorOperativeAgent
...
```bash
# Via CLI
Chottu ask --agent monitor_operative "Analyze the security audit findings"
```
...
```python
from Chottu.sandbox import ContainerRunner, SandboxedAgent
from Chottu.agents.simple import SimpleAgent

runner = ContainerRunner(
    image="Chottu-sandbox:latest",
    timeout=60,
    mount_allowlist_path="/etc/Chottu/mount_allowlist.json",
)
...
| Parameter              | Type   | Default                      | Description                                    |
|------------------------|--------|------------------------------|------------------------------------------------|
| `image`                | `str`  | `"Chottu-sandbox:latest"`| Docker image to run                            |
...
```python
from Chottu.core.registry import AgentRegistry

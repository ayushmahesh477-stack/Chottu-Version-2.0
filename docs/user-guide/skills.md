...
| **Overlay** | Sidecar file at `~/.Chottu/learning/skills/` storing optimized descriptions and few-shot examples |
...
Chottu skill install hermes:apple-notes

# Bulk install a category
Chottu skill sync hermes --category research

# Run a skill directly
Chottu skill run math-solver -a expression="41 + 82"

# See skill details
Chottu skill info research-and-summarize
```
...
Chottu skill sync hermes --category research
Chottu skill sync hermes --category coding
Chottu skill sync hermes  # everything (~150 skills)
```
...
Chottu skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
Chottu skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
Chottu skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```
...
```bash
Chottu skill install github:Xquik-dev/hermes-tweet/skills/hermes-tweet --url https://github.com/Xquik-dev/hermes-tweet
```

### Config-Driven Auto Import

Add sources to `~/.Chottu/config.toml` for automatic syncing:
...
Chottu can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
Chottu skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.Chottu/skills/discovered/
Chottu skill discover
```

Discovered skills land in `~/.Chottu/skills/discovered/` and automatically appear in `Chottu skill list` on the next session.
...
Chottu optimize skills --dry-run

# Run DSPy optimization
Chottu optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
Chottu optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
Chottu skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.Chottu/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
# Single condition
Chottu bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with Chottu | Implicit trust | Full access within declared capabilities |
...
```bash
Chottu skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.Chottu/skills/"  # where skills are installed
...
overlay_dir = "~/.Chottu/learning/skills/"
```
...
2. **User** `~/.Chottu/skills/`
3. **Bundled** (shipped with Chottu)

## CLI Reference

| Command | Description |
|---------|-------------|
| `Chottu skill list` | List installed skills |
| `Chottu skill info <name>` | Show detailed skill information |
| `Chottu skill run <name> [-a key=value]` | Execute a skill directly |
| `Chottu skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `Chottu skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `Chottu skill sources` | List configured skill sources |
| `Chottu skill update` | Pull latest from configured sources |
| `Chottu skill remove <name>` | Remove an installed skill |
| `Chottu skill search <query>` | Search the skill index |
| `Chottu skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `Chottu skill show-overlay <name>` | Inspect optimization output for a skill |
| `Chottu optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `Chottu bench skills [--condition C]` | Run the PinchBench skills benchmark |

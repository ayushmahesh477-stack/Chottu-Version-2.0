...
```bash
python examples/Chottu/spec_search_quickstart.py
```
...
`Chottu.learning.spec_search.composite_reward.score_batch`.
...
`configs/Chottu/examples/spec-search-quickstart.toml`. Copy it to
`~/.Chottu/config.toml` (or set `Chottu_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `Chottu.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `Chottu.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `Chottu.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `Chottu.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `Chottu.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/Chottu/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `Chottu.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from Chottu.evals.datasets.adp import ADPDataset
from Chottu.learning.spec_search.external_adapter import (
...
store = TraceStore("~/.Chottu/traces.db")
...
`src/Chottu/evals/backends/Chottu_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.Chottu/traces.db` regardless of caller
...
- `examples/Chottu/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/Chottu/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/Chottu/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/Chottu/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/Chottu/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/Chottu/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/Chottu/learning/spec_search/external_adapter.py` — corpus → trace adapter.

...
The Chottu Python SDK provides a high-level interface for interacting with local inference engines, managing memory, and running agent workflows. The primary entry point is the `Chottu` class.
...
|---------------|------------------|---------|----------------------------------------------------------------|
| `config`      | `ChottuConfig`   | `None`  | Provide a pre-built configuration object                       |
...
If no `config` or `config_path` is provided, the SDK loads configuration from the default location (`~/.Chottu/config.toml`), falling back to built-in defaults.
...
|-----------|----------------|-----------------------------------|
| `config`  | `ChottuConfig` | The active configuration object   |
| `version` | `str`          | The Chottu version string     |
...
The `Chottu.memory` attribute provides a `MemoryHandle` for document indexing, search, and statistics. The memory backend is lazily initialized on first use.
...
Release all resources held by the `Chottu` instance, including the memory backend, telemetry store, and engine connection.
...
!!! tip "Context Manager Pattern"
    While `Chottu` does not implement `__enter__`/`__exit__` directly, you should always call `close()` when done to free database connections and other resources:

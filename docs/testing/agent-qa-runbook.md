...
| Chottu initialized | `uv run Chottu doctor` all green |
| Rust extension built | `uv run maturin develop -m rust/crates/Chottu-python/Cargo.toml` |
| Desktop app running | `uv run Chottu serve` + `cd frontend && npm run dev` |
...
|---|----------|-------|-----------------|------|
| 1 | Template launch | `Chottu agents launch`, pick research_monitor | Agent created, config printed with curated tools | [ ] |
| 2 | Manual run | `Chottu agents run <id>` | Output shows reasoning + tool calls, status -> idle | [ ] |
| 3 | Immediate ask | `Chottu agents ask <id> "summarize recent AI news"` | Synchronous response in terminal | [ ] |
| 4 | Queued instruct | `Chottu agents instruct <id> "focus on diffusion"`, then `Chottu agents run <id>` | Queued -> delivered, response in `Chottu agents messages <id>` | [ ] |
| 5 | Status check | `Chottu agents status` after 3+ runs | total_runs, total_cost, last_run_at populated | [ ] |
| 6 | Pause/resume | `Chottu agents pause <id>`, verify skipped, `Chottu agents resume <id>`, verify fires | Status toggles correctly | [ ] |
| 7 | Daemon scheduling | `Chottu agents daemon` with interval agent (60s) | 3+ ticks fire on schedule, memory accumulates | [ ] |
| 8 | Budget exhaustion | Set max_cost=0.001, run until exceeded | Status becomes budget_exceeded | [ ] |
| 9 | Error recovery | Kill Ollama mid-tick, then `Chottu agents recover <id>` | Error -> recover -> idle with checkpoint | [ ] |
| 10 | Channel binding | `Chottu agents bind <id> --slack #test`, run tick | Agent sends to Slack | [ ] |
| 11 | Multi-agent | Launch 3 agents, different intervals, run daemon | All fire independently | [ ] |
| 12 | Template tools | Create from each template, `Chottu agents info <id>` | All curated default tools listed | [ ] |

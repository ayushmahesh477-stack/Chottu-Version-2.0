...
Chottu supports [ACE](https://github.com/ace-agent/ace) as a third
...
ACE is **not on PyPI** as of Chottu v1.0.1, and the upstream
...
```bash
# 1. Clone ACE somewhere outside your Chottu checkout
git clone https://github.com/ace-agent/ace.git ~/code/ace
cd ~/code/ace
curl -LsSf https://astral.sh/uv/install.sh | sh   # if you don't have uv
uv sync

# 2. Make ACE's src/ importable from your Chottu venv
...
# 4. Verify the import resolves from Chottu's venv
python -c "from Chottu.learning.agents.ace_optimizer import HAS_ACE; print(HAS_ACE)"
# True
```

If `HAS_ACE` prints `False`, the `.pth` file isn't being picked up —
verify the path matches `site.getsitepackages()[0]` for the same
Python interpreter you're using to run Chottu.

## Configuration

ACE is configured under `[learning.agent.ace]` in your Chottu
config TOML:
...
task_name = "Chottu"
save_dir = ""               # default: ~/.Chottu/learning/ace/<task>/
...
in `src/Chottu/learning/agents/ace_optimizer.py` — it does a
...
- **Single provider per run.** ACE assigns the same `api_provider` to
  all three roles. To mix providers, run ACE outside Chottu and

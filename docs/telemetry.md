...
Chottu ships **anonymous usage telemetry** by default so the team can
...
- **Self-hosted backend** on the Chottu team's PostHog instance —
...
[`src/Chottu/analytics/events.py`](../src/Chottu/analytics/events.py).
...
1. [`src/Chottu/analytics/redaction.py`](../src/Chottu/analytics/redaction.py) — value-level pattern matching (20+ regexes for PII).
2. [`src/Chottu/analytics/events.py`](../src/Chottu/analytics/events.py) — structural allowlist (event name + property name + type validator).
...
- The leaderboard / contest opt-in (`OptInModal.tsx`) is a separate,
  ...
  the Chottu leaderboard. It is **not** the same as analytics and

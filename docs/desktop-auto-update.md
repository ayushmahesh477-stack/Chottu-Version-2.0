...
The Chottu desktop app ships with [Tauri's updater
...
on launch / every 30 min
        │
        ▼
GET https://github.com/Chottu/Chottu/releases/download/desktop-latest/latest.json
        │
        ▼
...
For frontend development, set `VITE_Chottu_NO_UPDATER=1` in your
...
```bash
export VITE_Chottu_NO_UPDATER=1
npm run tauri dev
```

This is purely a dev escape hatch — it has no effect on production
builds (where `import.meta.env.VITE_Chottu_NO_UPDATER` will be
...
curl -fsSL https://github.com/Chottu/Chottu/releases/download/desktop-latest/latest.json | jq .

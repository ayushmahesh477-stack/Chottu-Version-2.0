...
The `Chottu.mining` subsystem follows the same registry pattern as engines,
agents, tools, memory, and channels. New mining paths should be provider
modules, not special cases in the CLI or engine layer.

## Provider Contract

Every provider implements `Chottu.mining.MiningProvider`:
...
The runtime sidecar lives at `~/.Chottu/runtime/mining.json`. Engine
...
- `MiningStats`
- `Sidecar`
- `Chottu mine doctor` capability iteration

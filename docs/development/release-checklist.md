...
2. **`Chottu` (no args)** drops into a chat session within 2 s.
...
6. **`Chottu doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`Chottu-uninstall`** removes `~/.Chottu/` and `~/.local/bin/Chottu*`. Verify with `ls`.
...
- Delete `~/.Chottu/config.toml` — verify bare `Chottu` re-runs init.
- Delete `~/.Chottu/.venv` — verify re-running curl heals it.

...
```bash
uv run Chottu --version   # Should print 0.1.0
uv run Chottu --help      # Show all subcommands
```
...
Chottu uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=Chottu --cov-report=html
```
...
Chottu uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/Chottu/`:
...
src/Chottu/
    ...
    core/                       # Shared infrastructure
        config.py               # ChottuConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # Chottu ask
        init_cmd.py             # Chottu init
        model.py                # Chottu model list/info
        memory_cmd.py           # Chottu memory index/search/stats
        telemetry_cmd.py        # Chottu telemetry stats/export/clear
        bench_cmd.py            # Chottu bench run
        serve.py                # Chottu serve
```
...
try:
    import Chottu.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```

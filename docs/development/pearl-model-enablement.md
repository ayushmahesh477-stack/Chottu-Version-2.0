...
through Pearl's vLLM miner and Chottu.

Chottu can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
Chottu only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and Chottu-specific conversion
...
`Chottu mine start`, vLLM `/v1/models`, Chottu inference routing, Pearl
gateway template refresh, and `Chottu mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in Chottu until we have clean
...
2. Convert the target model.
   ...
   Chottu includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `Chottu mine inspect-model` and
   `Chottu mine validate-model` pass on H100/H200 hardware.
...
3. Validate the Pearl vLLM plugin path.
   - Run `Chottu mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `Chottu mine status` parses those metrics.

5. Promote the model in Chottu.
...
```text
src/Chottu/mining/_models.py
```

`Chottu mine models` renders that registry. Planned models are visible to users
...
- `Chottu mine inspect-model --model <pearl-model-id> --allow-planned`
- `Chottu mine init --model <pearl-model-id>`
- `Chottu mine start`
- `curl http://127.0.0.1:8000/v1/models`
- `Chottu ask "Say hello in one sentence."`
- `Chottu mine status`
- `Chottu mine validate-model --model <pearl-model-id> --allow-planned --prompt
...
Attach the JSON artifact from `Chottu mine validate-model --output` to the issue.

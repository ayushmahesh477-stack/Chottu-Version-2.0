...
prove Chottu wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through Chottu.
...
- Docker container `Chottu-pearl-miner` starts.
- `~/.Chottu/runtime/mining.json` is written.
...
```bash
docker ps --filter name=Chottu-pearl-miner
cat ~/.Chottu/runtime/mining.json
uv run Chottu mine logs --tail 200
...
## Verify Chottu Inference Uses Mining Endpoint

Run:

```bash
uv run Chottu mine doctor
uv run Chottu ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/Chottu/mining/_metrics.py`.
...
```bash
uv run Chottu mine stop
docker ps --filter name=Chottu-pearl-miner
test ! -e ~/.Chottu/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `Chottu ask` no longer routes through `vllm-pearl-mining` unless another
...
- Chottu inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run Chottu mine doctor
uv run Chottu mine status || true
uv run Chottu mine logs --tail 300 || true
docker inspect Chottu-pearl-miner || true

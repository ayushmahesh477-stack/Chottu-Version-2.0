...
Chottu provides Docker images for both CPU-only and GPU-accelerated deployments, along with a Docker Compose configuration that bundles the API server with an Ollama inference backend.
...
cd deploy/docker
cp .env.example .env
echo "Chottu_API_KEY=$(Chottu auth generate-key)" > .env   # or paste your own
```
...
`docker compose` reads `Chottu_API_KEY` from `.env` (or your shell
...
1. **Builder stage** -- installs `uv` and the `Chottu[server]` package (which includes FastAPI, uvicorn, and all server dependencies) from the project source.
...
```bash
docker build -t Chottu:latest .
```

Run it standalone:

```bash
docker run -d -p 8000:8000 Chottu:latest
```
...
```bash
docker build -f Dockerfile.gpu -t Chottu:gpu .
```
...
```bash
docker run -d --gpus all -p 8000:8000 Chottu:gpu
```
...
The `docker-compose.yml` defines a complete deployment with the Chottu API server and an Ollama backend:
...
services:
  Chottu:
    ...
    ports:
      - "8000:8000"
    environment:
      - Chottu_ENGINE_DEFAULT=ollama
      - Chottu_OLLAMA_HOST=http://ollama:11434
    depends_on:
      - ollama
...
The `Chottu` service is configured through environment variables:

| Variable                      | Description                                             | Default                    |
|-------------------------------|---------------------------------------------------------|----------------------------|
| `Chottu_ENGINE_DEFAULT`   | Inference engine backend to use                         | `ollama`                   |
| `Chottu_OLLAMA_HOST`      | URL of the Ollama server (uses Docker service name)     | `http://ollama:11434`      |
...
The `Chottu` service declares `depends_on: ollama`, ensuring the Ollama container starts before the API server. Both services use `restart: unless-stopped` to automatically recover from crashes.

## Custom Configuration

### Mounting a Configuration File

To use a custom `config.toml`, mount it into the container at the expected path (`~/.Chottu/config.toml`, which is `/root/.Chottu/config.toml` in the container):
...
services:
  Chottu:
    ...
    ports:
      - "8000:8000"
    volumes:
      - ./my-config.toml:/root/.Chottu/config.toml:ro
    environment:
      - Chottu_ENGINE_DEFAULT=ollama
      - Chottu_OLLAMA_HOST=http://ollama:11434
    depends_on:
...
To persist telemetry data, memory databases, and trace records across container restarts, mount the entire Chottu data directory:
...
The entrypoint is `Chottu` and the default command is `serve --host 0.0.0.0 --port 8000`. Override the command to change server options:

```bash
docker run -d -p 9000:9000 Chottu:latest \
...
### Available CLI Options for `Chottu serve`

...
Chottu includes an OpenAI-compatible API server built on FastAPI and uvicorn. It exposes chat completion, model listing, and health check endpoints, making it a drop-in replacement for the OpenAI API when working with local models.
...
The server reads defaults from `~/.Chottu/config.toml` and auto-detects available engines and models. Override any option via CLI flags:
...
!!! warning "Server dependency check"
    If the `[server]` extra is not installed, `Chottu serve` exits with a clear error message explaining how to install the required dependencies.
...
The `[server]` section of `~/.Chottu/config.toml` controls default server behavior:
...
CLI flags override config file values. For example, `Chottu serve --port 9000` overrides the `port` setting in the config file.
...
For production deployments, run Chottu behind a reverse proxy like Nginx or Caddy for TLS termination, rate limiting, and authentication.
...
server {
    listen 443 ssl;
    server_name Chottu.example.com;

    ssl_certificate /etc/ssl/certs/Chottu.pem;
    ssl_certificate_key /etc/ssl/private/Chottu.key;
...
```bash
Chottu serve --host 127.0.0.1 --port 8000
```
